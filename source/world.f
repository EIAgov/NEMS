...
aa=91
      !if (fdbk_sw.eq.0) then
              do iY=FFY-1989,aa
                  XSTART_PRICE(iY)=XSTART_PRICE(iY)/mc_jpgdp(36)
        end do
              do iY=FFY-1989,MNUMYR
                  START_PRICE(iY)=START_PRICE(iY)/mc_jpgdp(36)
        end do
      !endif

write(DBGUNIT, '(//,A)') "Start Pricesa this is a new test AEO2017"
write (DBGUNIT,1234) (START_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)

write(DBGUNIT, '(//,A)') "XStart Pricesa"
write (DBGUNIT,12345) (XSTART_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)

!write(DBGUNIT, '(//,A)') "Brent Pricesa"
!write (DBGUNIT,1234) (BRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)
1234          format (2x, 43F10.2)

!write(DBGUNIT, '(//,A)') "XBrent Pricesa"
!write (DBGUNIT,12345) (XBRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)
12345          format (2x, 73F10.2)
      ...
      do Ncol=2,MNUMYR+Max_LFMM_FYR-FFY+1991
          ...
          if ((FFY+Ncol-2) .LE. (1989+MNUMYR)) then
            ...
            do iY=FFY-1989,aa
                XWTI_PRICE(iY)=XWTI_PRICE(iY)/mc_jpgdp(36)
      end do
            do iY=FFY-1989,MNUMYR
                WTI_PRICE(iY)=WTI_PRICE(iY)/mc_jpgdp(36)
            end do
            !XWTI_PRICE(:) = XWTI_PRICE(:)/mc_jpgdp(36)      
...
!end do
 !*************************************
      ...
      D_Diff(1989+CURIYR) = 1000*(RFQTDCRD(16,CURIYR) + RFIMCR(MNUMPR,CURIYR) + RFCRDOTH(MNUMPR,CURIYR)) - Q_Net_Crude_REF(1989+CURIYR) + 1000.0*RFIMTP(MNUMPR,CURIYR) - Q_Net_Product_Imp_REF(1989+CURIYR)
	  !D_Diff(1989+CURIYR) = 1000*(sum(RFQTDCRD(1:13,CURIYR)) + RFIMCR(MNUMPR,CURIYR) + RFCRDOTH(MNUMPR,CURIYR)) - Q_Net_Crude_REF(1989+CURIYR) + 1000.0*RFIMTP(MNUMPR,CURIYR) - Q_Net_Product_Imp_REF(1989+CURIYR)
      ...
      !START_PRICE(CURIYR) = P_Eql(1989+CURIYR)/mc_jpgdp(36)
...
write (DBGUNIT,1122) (BRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)
1122          format (2x, 43F10.2)

write(DBGUNIT, '(//,A)') "XBrent Prices1"
write (DBGUNIT,3333) (XBRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)
...
write (DBGUNIT,1122) (START_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)

write(DBGUNIT, '(//,A)') "XStart Prices1"
write (DBGUNIT,3333) (XSTART_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)

write(DBGUNIT, '(//,A)') "WTI Prices1"
write (DBGUNIT,11222) (WTI_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)
11222          format (2x, 43F10.2)

write(DBGUNIT, '(//,A)') "XWTI Prices1"
write (DBGUNIT,33333) (XWTI_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)
33333          format (2x, 73F10.2)

end if


      !BRENT_PRICE(CURIYR) = BRENT_PRICE(CURIYR)/mc_jpgdp(36)
      !START_PRICE(CURIYR) = START_PRICE(CURIYR)/mc_jpgdp(36)
      !WTI_PRICE(CURIYR) = WTI_PRICE(CURIYR)/mc_jpgdp(36)
...
!end do
      ...
      do iSt = 1, CRSTEP
        P_Total_Crude(iSt, 1989+CURIYR) = P_Total_Crude(iSt, 1989+CURIYR)/mc_jpgdp(36)
      end do
...
!end do
 !******************************
      ...
      do c=1,MNCRUD
        do t = 1, CISTEP
            P_FOREIGN_CRUDE(c,Max_Crude_Source,t,1989+CURIYR) = P_FOREIGN_CRUDE(c,Max_Crude_Source,t,1989+CURIYR)/mc_jpgdp(36)
        end do
      end do
...
!*******************************************************************************
      ...
      do c=1,MNCRUD
        P_NON_US_DEMAND(c, Max_Crude_Source, Max_NonUS_Demand_Steps,1989+CURIYR)=P_NON_US_DEMAND(c, Max_Crude_Source, Max_NonUS_Demand_Steps,1989+CURIYR)/mc_jpgdp(36)
      end do
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$    
        ...
        if(EXP_P(iPr,1989+CURIYR) .NE. 0 ) then
           ...
           do t=1,INTLSTEP
            ...
            P_Start = ( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1-BP(t))
            P_End   = ( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1-BP(t+1))

            if (t .EQ.1) then
                Q_Start = 0.0
            else
                Q_Start = EXP_Q(iPr,1989+CURIYR)*(P_Start/( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**(D_E(1989+CURIYR)-0.15)
            end if

            Q_End   = EXP_Q(iPr,1989+CURIYR)*(P_End/( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**(D_E(1989+CURIYR)-0.15)
      ...
      do iPr=1,MNPROD !for each product
       do t=1,INTLSTEP !for each step
        Product_Export_P(iPr,1,t,CURIYR)=Product_Export_P(iPr,1,t,CURIYR)/mc_jpgdp(36)
...
8484          format (2x, 43F10.2)      
      ...
      do iPr=1,MNPROD !for each product
       do t=1,INTLSTEP !for each step
         ...
         P_Start = ( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1+BP(t))
         P_End   = ( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1+BP(t+1))

         Q_Start = IMP_Q(iPr,1989+CURIYR)*(P_Start/( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**(S_E(1989+CURIYR)+0.15)
         Q_End   = IMP_Q(iPr,1989+CURIYR)*(P_End/( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**(S_E(1989+CURIYR)+0.15)
  ...
  !       end if          
     ...
     !Change Product_Import_P to 1987 dollars
      do iPr=1,MNPROD !for each product
       do t=1,INTLSTEP !for each step
        Product_Import_P(iPr,1,t,CURIYR)=Product_Import_P(iPr,1,t,CURIYR)/mc_jpgdp(36)
       end do         !end for each step
      end do          !end for each product
...
!
!        Product_Import_P(iPr,1,1,iYr) = ( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.01
!        Product_Export_P(iPr,1,1,iYr) = ( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*100.0
!        Product_Import_P(iPr,1,2,iYr) = ( IMP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.02
!        Product_Export_P(iPr,1,2,iYr) = ( EXP_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*90.0
!
...
!         
!
!        Product_Import_P(13,1,1,iYr) = ( IMP_P(13,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.01    
!        Product_Import_P(13,1,2,iYr) = ( IMP_P(13,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.02  
!        Product_Import_P(19,1,1,iYr) = ( IMP_P(19,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.01   
!        Product_Import_P(19,1,2,iYr) = ( IMP_P(19,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*0.02
!         
!
!    end do !end do iYr=1, MNXYR
...
!      do t=1,EUSTEP
!        P_Europe_Gas(t,1989+CURIYR)=P_Europe_Gas(t,1989+CURIYR)/mc_jpgdp(36)
!      end do
!
      ...
      do iPr =1,MNPROD
        if(C_MC_P(iPr,1989+CURIYR) .NE. 0 ) then
           ...
           do t=1,MCSTEP
            ...
            P_Start = ( C_MC_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1-BP(t))
            P_End   = ( C_MC_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) )*(1-BP(t+1))


            if (t .EQ.1) then
                Q_Start = 0.0
            else
                Q_Start = C_MC_Q(iPr,1989+CURIYR)*(P_Start/( C_MC_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**D_E(1989+CURIYR)
            end if

            Q_End   = C_MC_Q(iPr,1989+CURIYR)*(P_End/( C_MC_P(iPr,1989+CURIYR) + 0.96*mc_jpgdp(36)*(BRENT_PRICE(CURIYR) - START_PRICE(CURIYR)) ))**D_E(1989+CURIYR)
...
!10001       format (2x, 14F10.2)
 ...
 !end if
      ...
      do iPr =1,MNPROD
           do t=1,MCSTEP
             P_C_MC_Demand(t,1989+CURIYR,iPr)= P_C_MC_Demand(t,1989+CURIYR,iPr)/mc_jpgdp(36)
        end do
      end do
...
89          continue
     ...
     !if (CURCALYR .EQ. 2015) then
            ...
            do 91, c=1,MNCRUD
                ...
                !write (DBGUNIT,92) (P_Foreign_Crude(c,1,8,ii)*mc_jpgdp(36)+P_Crude(1,ii) , ii = 2008, 2071)     !P_Foreign_Crude(c,Max_Crude_Source,t,1989+CURIYR)
                !write (DBGUNIT,92) (P_Total_Crude(8,ii)*mc_jpgdp(36) , ii = 2008, 2071)
                write (DBGUNIT,92) (P_NON_US_DEMAND(c,1,1,ii)*mc_jpgdp(36) , ii = 2008, 2071)
92          format (2x, 64F10.2)
91          continue

write(DBGUNIT, '(//,A)') "Brent Prices"
write (DBGUNIT,99) (BRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)
99          format (2x, 43F10.2)

write(DBGUNIT, '(//,A)') "XBrent Prices"
write (DBGUNIT,100) (XBRENT_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)
100          format (2x, 73F10.2)

write(DBGUNIT, '(//,A)') "Start Prices"
write (DBGUNIT,99) (START_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)

write(DBGUNIT, '(//,A)') "XStart Prices"
write (DBGUNIT,100) (XSTART_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)

write(DBGUNIT, '(//,A)') "WTI Prices1"
write (DBGUNIT,101) (WTI_PRICE(ii)*mc_jpgdp(36) , ii = 19, 61)
101          format (2x, 43F10.2)

write(DBGUNIT, '(//,A)') "XWTI Prices1"
write (DBGUNIT,102) (XWTI_PRICE(ii)*mc_jpgdp(36) , ii = 19, 91)
