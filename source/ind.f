...
!    21   Light Chemicals 									  325 excluding bulk chemicals (so only 3254, 3259)
!    22   Other non-metallic minerals                         327 excluding Cement/Lime and Glass
!    23   Other primray metals		  						  331 excluding aluminum and steel
!    24   Miscellaneous Finished Goods 						  312    Beverage and Tobacco Products
!                                                             313    Textile Mills
!                                                             314    Textile Product Mills
!                                                             315    Apparel
!                                                             316    Leather and Allied Products
!                                                             323    Printing and Related Support
!                                                             324 nec  Other Petroleum and Coal Products  (e.g. shingles, asphalt for roads )
!                                                             337    Furniture and Related Products
!                                                             339    Miscellaneous manufacturing (e.g. toys, medical supplies & coffins)
!                                                                    --which is why the informal name of industry is "cradle to grave"
!
!  ORDER OF ENERGY SOURCES: (NOT FOR BUILDINGS)
!
!  THE MAIN QUANTITY ARRAY:  QTYMAIN(mainfuels,5)
...
!  17.              - ASPHALT & ROAD OIL
!  18.              - LUBES & WAXES
!  19.              - PETROCHEMICAL (NAPHTHA) FEEDSTOCK
!  20.              - KEROSENE
...
! Variable sharing module
      ...
      implicit none
      include 'parametr'
	  include 'CCATSDAT'  ! for CCATS cementCCS retrofits
      include 'ncntrl'
      include 'emmparm'
      include 'macout'
      include 'qblk'
      ...
      include 'ab32'
      include 'indepm'
	  include 'emoblk'   ! to include the carbon fee emtax(curiyr)
...
!
	  ...
	  REAL HGLFEEDCALIBFACT ! FOR HGL FEEDSTOCK CALIBRATION TO BE APPLIED TO 1 BEYOND STEOLASTYR (PGR)
	  ...
	  REAL HGLFEEDTARGET(5) ! HGL feedstock target based on history vs. model results
	  REAL PCFFEEDTARGET(5, MNUMYR) ! PCF feedstock target based on history vs. model results
	  REAL H2_chem_demand_reg(6,4)	! base year H2 feedstock consumption for 6 chemical subsectors by census region: industrial gases (really supply),
							! other inorganics, petrochemicals,other organics, resins, and ag chemicals
	  REAL H2_chem_supply_reg(6,4)	! same as H2_chem_demand_reg, but for H2 supply
	  REAL H2_chem_demand_div(MNUMCR,7)	! base year H2 feedstock consumption for 6 chemical subsectors by census division (TBtu); index 7 is total
	  REAL H2_chem_supply_div(MNUMCR,7)	! base year H2 feedstock consumption for 6 chemical subsectors by census division (TBtu); index 7 is total	  
	  REAL H2_ref_supply_div(MNUMCR)		! base year H2 production from refinery SMRs, based on Petroleum Supply Annual data
...
! Process/Assembly Variables
      ...
      REAL    BASE_SHIP             ! base year shipments read from enprod.txt; dollar value of output for end-use industries, and tonnes of output for process flow industries
      REAL    PRODCUR(4,maxstep)    ! production capacity by vintage and process/assembly step
      ...
      INTEGER IFLOC(17,maxstep+1)     ! stores fuel identifier numbers per process step
      ...
      REAL    EINTER(3,17,maxstep)  ! intercept term for unit energy consumption values
      REAL    ENPINTLAG(3,17,maxstep) ! lag year values to vintaged unit energy consumption by fuel,process
...
! glass
	  ...
	  REAL container_fuel_reduction(MNUMYR)	! % fuel reduction for container glass furnaces resulting from melting recycled cullet instead of raw materials
...
!!!! cement CCS variables !!!!!!!!!!!
   ...
   real ElectricUseCCS        ! COC purchased electricity use (MMBtu/tonne CO2)

   real BaseCapCCS(1:inumreg)    ! base capital cost (2018 dollars/tonne CO2)
   real CapAdderCCS(1:inumreg,MNUMYR)   ! capital cost adder to base capital cost (2018 dollars/tonne CO2)
   
   real PPCCS(1:inumreg)         ! payback period for determining what percentage of retrofits should be
                                 ! installed in a given year

   real OplusMfixCCS(1:inumreg)  ! fixed O&M cost (2018 dollars/tonne CO2)
   real OplusMvarCCS(1:inumreg)  ! non-fuel variable O&M cost (2018 dollars/tonne CO2)
   
   real slopeCCS(1:inumreg)      ! slope of fitted exponential regression
   real yinterceptCCS(1:inumreg) ! y-intercept of fitted exponential regression
   real normCCS(1:inumreg)       ! normalization constant
   real halfofintCCS(1:inumreg)  ! half of the integral of the fitted exponential distribution 
   real lowestCCS(1:inumreg)     ! lowest kiln COC (2018 dollars/tonne CO2)
   real highestCCS(1:inumreg)    ! highest kiln COC (2018 dollars/tonne CO2)
   real meanofdistCCS(1:inumreg) ! mean of fitted exponential distribution
   
   real techpotenCCS(1:inumreg)  ! technical potential for CCS when economical

   real LastConsYrCCS            ! last 45Q eligible construction year for CCS equipment (2032 according to IRA)
   real CreditDurCCS             ! 45Q credit duration CCS (nominally 12 years according to IRA)
   real CCSstartyear             ! read in year when realistically we think CCS rertrofits on cement plants can begin 
                                 ! (either for sequestration in geological sites or for EOR)
...
! technology choice industries common variables for consumption
	  REAL NgCons(maxstep,mnumyr)                   ! NG Consumption for for Tech industries (TBtu)
      REAL HFOCons(maxstep,mnumyr)                  ! HFO Consumption for Tech industries (TBtu)
      REAL ElecCons(maxstep,mnumyr)                 ! Electricity Consumption for Tech industries (TBtu)
      ...
      REAL CoalSCons(maxstep,mnumyr)                ! Coal (Sub-Bituminous) Consumption for Tech industries (TBtu)
...
! I&S Coke variables
      ...
      REAL IS_CProcess(4) ! Coke Process Output factors; breeze,coke oven gas,tars,light oils
...
!  cogen market penetration assumptions
      ...
      real CapCostYearly(2015:endyr,nsys)   !  Capital cost/kw assumptions
      ...
      real CHeatRateYearly(2015:endyr,nsys) !  heatrate assumptions by year, system
...
! This only works if nload=nsys. If ever nsys or nload changes, this statement will have to change to a different mapping.
      ...
      integer*2 CapCostMultEnd   ! ENDING (?) YEAR 	year in which CapCostMult goes into effect
!====================================
...
! BY CENSUS REGION
      ...
      REAL QTYMAIN(mainfuels,5)    ! Consumption of main fuel j in region k  
...
! H2            20
	  ...
	  REAL AGCO2(NUM_CON_FUELS,MNUMYR), MINECO2(NUM_CON_FUELS,MNUMYR), CONSTCO2(NUM_CON_FUELS,MNUMYR), FOODCO2(NUM_CON_FUELS,MNUMYR)
	  REAL PAPERCO2(NUM_CON_FUELS,MNUMYR), CHEMCO2(NUM_CON_FUELS,MNUMYR), GLASSCO2(NUM_CON_FUELS,MNUMYR), CEMENTCO2(NUM_CON_FUELS,MNUMYR)
	  REAL STEELCO2(NUM_CON_FUELS,MNUMYR), ALUMCO2(NUM_CON_FUELS,MNUMYR), FABMETALCO2(NUM_CON_FUELS,MNUMYR), MACHINECO2(NUM_CON_FUELS,MNUMYR)
	  REAL COMPUTECO2(NUM_CON_FUELS,MNUMYR), TRANEQUIPCO2(NUM_CON_FUELS,MNUMYR), ELECEQUIPCO2(NUM_CON_FUELS,MNUMYR), WOODPRODCO2(NUM_CON_FUELS,MNUMYR)
	  REAL PLASTICCO2(NUM_CON_FUELS,MNUMYR), LTCHEMCO2(NUM_CON_FUELS,MNUMYR), OTHRNMMCO2(NUM_CON_FUELS,MNUMYR), OTHRPRIMCO2(NUM_CON_FUELS,MNUMYR), MISCFINCO2(NUM_CON_FUELS,MNUMYR)
	  REAL CO2EM(NUM_CON_FUELS,MNUMYR), REFCO2(NUM_CON_FUELS,MNUMYR), H2CO2(NUM_CON_FUELS,MNUMYR)
      ...
      COMMON/CO2VARS/AGCO2, MINECO2, CONSTCO2, FOODCO2, PAPERCO2, CHEMCO2, GLASSCO2, CEMENTCO2, &
				...
				WOODPRODCO2, PLASTICCO2, LTCHEMCO2, OTHRNMMCO2, OTHRPRIMCO2, MISCFINCO2, & 
				CO2EM, REFCO2, H2CO2
!
!====================================
! Carbon-Accounting Industry Assumption
      ...
      real QtyByInd(NUM_CON_FUELS,numind)  ! sum of qtymain, qtyrenw into 19 reporting fuel categories by industry
      real clinkFac/0.12545/      ! factor to convert tons of clinker production to metric tonnes of C
      real CARBSHRCA(numind),CARBSHR_AB(numind,5)  ! Share of each industry's fossil consumption for AB32.
!============================
	  real Glass_Furnace_Process_Emissions(4)	! glass process emissions by furnace; 1=flat, 2=container, 3=blown, 4=fiber/specialty
!==============================
! process emissions factors
	real CO2_Cement_Process_Emission_Factor/0.507/ 	! cement process emissions based on clinker production, in tonnes CO2/tonne clinker produced
													! from pg. 177 of https://www.ipcc-nggip.iges.or.jp/public/gp/bgp/3_1_Cement_Production.pdf
	real CO2_Lime_Process_Emission_Factor/0.751/	! lime process emissions based on quicklime production, in tonnes CO2/tonne quicklime produced
													! from pg. 31 of https://www.eula.eu/wp-content/uploads/2019/02/A-Competitive-and-Efficient-Lime-Industry-Technical-report-by-Ecofys_0.pdf
	real CO2_Glass_Process_Emission_Factor/0.155/	! glass process emissions based on (non-cullet) furnace input, in tonnes CO2/tonnes virgin glass	
													! based on https://www.epa.gov/system/files/documents/2024-02/us-ghg-inventory-2024-chapter-4-industrial-processes-and-product-use.pdf
													! and 2018 glass shipments in ironstlx, as well as an assumed 30% recycle rate for container glass in 2018
!==============================
! indbench
...
! Define Data Type BUFYEAR to hold all input/output varying by industry and region for a single year
      ...
      REAL    RPTCAP(5)         ! CHP capacity by fuel MW
      REAL    QTYMAIN(mainfuels)  ! these are for the copybuf commmands
      REAL    QTYINTR(7)
      REAL    QTYRENW(9)
      REAL    BYPBSCM(mainfuels)
      REAL    BYPBSCI(7)
      REAL    BYPBSCR(9)
	  REAL    BYPADIBYR(9)
      ...
      REAL    ENPINT(3,17,maxstep)
      REAL    ENPINTLAG(3,17,maxstep)
      REAL    ENPQTY(4,18,maxstep)
      ...
      INTEGER IFLOC(17,maxstep+1)
      REAL    BCSC(3,17,maxstep)     
      INTEGER IFBYP(maxstep)
      INTEGER IFLOCBY(10,maxstep)
      REAL    BYPCSC(3,5,maxstep)
      REAL    EINTER(3,17,maxstep)
      INTEGER NumRptGrps                  ! Number of Reporting SubGroups for Process Assembly Energy reporting (used in Chemicals for Subindustry rpt), max:11, min:0
      INTEGER NumRptGrpSteps(12)          ! Number of steps in each reporting SubGroup for P/A
      INTEGER RptGrpSteps(12,maxstep)     ! Identification of step numbers included in each reporting subgroup in P/A
      Character*24 RptGrpNames(12)        ! Reporting group names
...
!*********************************************************************************AE
	  ...
	  REAL fertilizer_H2(mnumyr,5)	! historical H2 feedstock demand for fertilizer production
...
common/ind_easy/INDETTCBUS, INDC4TCBUS, INDPRTCBUS, INDPPTCBUS, INDDFTCBUS, INDRFTCBUS, INDMGTCBUS, &
                  ...
                  INDNGNUKUS, INDNGINX, INDZSAJQUS, INDCLYCBUS, INDCLKCBUS, INDCCNIBUS, INDEXICPUS, INDHVICBUS, INDHVCCBUS, INDWWICBUS, INDOWICBUS, INDPCEPCONB, INDPMTCPUS
	...
	REAL INDPMTCPUS(MNUMYR)		! Marketeable petcoke (includes industrial and power sector, but not refining), already converted to TBtu (from million b/d)
...
!******
      ...
      LSTITER=FCRL
	DO ID=1,11	! Remove once HMM permanently on/HMM values in restart file
		IF (PH2IN(ID,curiyr).le.0) PH2IN(ID,curiyr)=2*PNGIN(ID,curiyr)
	ENDDO
...
! zero reporting variables prior to base year
        ...
        LTCHEMCON(:,:,curiyr)=0.
        OTHRNMMCON(:,:,curiyr)=0.
        OTHRPRIMCON(:,:,curiyr)=0.
		MISCFINCON(:,:,curiyr)=0.

! GHG_PROCESSIN(industry, year) contains national-level, energy-independent process emissions by industry and year
	    GHG_PROCESSIN(1:24,curiyr) = 0.0 ! SET ALL INDUSTRIES' PROCESS EMISSIONS AS ZERO BY DEFAULT
...
!  If a variable is in physical units it is indicated in the comment
      ...
      ! Uses price in 87$ and then inflates to current macro $ (yr=2012 for AEO2023)
      ResidAdd(:)=0
      DO CoReg=0,NDREG
        ID=division(CoReg)
        ResidAdd(ID)=ResidAdd(ID)+QBMASCL(2,CoReg,curiyr)*PBMASCL(2,CoReg,curiyr)*MC_JPGDP(23)/1000
      ENDDO
      ...
      DO 5 ID=1,11
        ...
        ! subcomponents of organic
		chemship(8,ir,curcalyr)=chemship(8,ir,curcalyr) + MC_REVIND(ID,49,curiyr) ! petrochemicals
		chemship(9,ir,curcalyr)=chemship(9,ir,curcalyr) + MC_REVIND(ID,50,curiyr) ! other organics
		
! Add the food subindustry shipments to obtain the total.  Then, sum food shipments for the regions.

        foodship(1,ir,curcalyr)=foodship(1,ir,curcalyr) + MC_REVIND(ID,2,curiyr) ! Grain and Oil Seed Milling
...
!    20   BOM-plastics
!    21   BOM-light chemicals
!    22   Other non-metallic minerals
!    23   Other primary metals
!    24   Miscellaneous Finished Goods (residual)
!    the below is the python dictionary that assigns macro industries to idm industries and the split
!    elif strx == 'mac2indnum': # mc_revinds with dictionary values are split this way{idm_industry1:share1, idm_industry2:share2 &c}
!        return {1:311, 2:71, 3:72, 4:73, 5:74, 6:24, 7:24, 8:19, 9:24, 
!                10:8, 11:3220001, 12:3220002, 13:3220003, 14:24, 15:91, 16:92, 17:92, 18:93, 19:94, 
!                20:21, 21:3250001, 22:3250002, 23:3250003, 24:3250004, 25:324110, 26:99, 27:20, 28:10, 29:3270001, 
...
!       ship_dict = {26:{24:0.91, 12:0.09}, 31:{11:0.28, 24:0.72}}
        ...
        OUTIND(24,ID,curiyr)=MC_REVIND(ID, 6,curiyr) + &         ! tobacco
                      ...
                      MC_REVIND(ID,26,curiyr) * 0.91 + &  ! other pet, coal  - this portion of mcrevind 26 NOT coke plants, which go to steel
                      MC_REVIND(ID,41,curiyr)              ! misc mfg
!  need to split up mc_revind.  below is from Python split_ship subroutine in "step 4" program
...
! Record Lime base year production for indexing
        ...
        ENDIF
! Record Cement base year production for indexing
        IF (curcalyr.eq.ibyr) THEN
             CEMENTIBYR=MC_REVIND(ID,30,ICURIYR)
        ENDIF
! Record Aluminum base year production for indexing
        IF (curcalyr.eq.ibyr) THEN
             ALUMIBYR=MC_REVIND(ID,34,ICURIYR)
        ENDIF
...
!******
      DO 10 ID=1,11                       ! POT_MACRO convert MC_EMPNA to changed index.
        ...
        EMPIND(10,ID)=MC_EMPNA(ID,12,curiyr)*0.193  ! Nonmetallic minerals (was Glass), NAICS 327  updated 9/23/22   !
        EMPIND(11,ID)=MC_EMPNA(ID,12,curiyr)*0.253  ! Nonmetallic minerals (was Cement and lime), NAICS 327  updated 9/23//22 KPE

        EMPIND(12,ID)=MC_EMPNA(ID,13,curiyr)*0.373 + MC_EMPNA(ID,10,curiyr)*0.038 !Primary metals (was Iron and Steel), NAICS 331 plus a portion of naics 324 for coke plants
        EMPIND(13,ID)=MC_EMPNA(ID,13,curiyr)*0.248    !Primary metals (was Aluminum), NAICS 331
        ...
        EMPIND(21,ID)=MC_EMPNA(ID,9,curiyr)      ! light chemicals --- all chemicals not in bulk chemicals

        EMPIND(22,ID)= MC_EMPNA(ID,12,curiyr)*0.554    !Nonmetallic minerals (was Cement and lime), NAICS 327  updated 9/23//22 KPE! other nonmetallic minerals
		                                               ! see L:\mid\kpe\data\MECS_2018\final_MECS\AEO2023_files\employment calculations for empna.xlsx

        EMPIND(23,ID)= MC_EMPNA(ID,13,curiyr)*0.379     ! other primary metals.  see spreadsheet listed immiediately above
        EMPIND(24,ID)=MC_EMPNA(ID,2,curiyr) + & !BOM Miscellaneous finished goods; beverages and tobacco, NAICS 312
                     ...
                     MC_EMPNA(ID,10,curiyr)*0.382 + & !BOM Other, petroleum and coal, NAICS 324
...
!  ASSIGN NEMS PRICES TO IND VARIABLES  (from epmMPLBK--alternate to MPBLK with different variable names)
    DO ID=1,11
        ...
        DPRCX(2,ID,ibif)=PH2IN(ID,curiyr)        ! H2 feedstock has no CO2 tax
        ...
        DPRCX(13,ID,ibif)=aLGINPF(ID,curiyr)	! the adjusted HGL feed price
        ...
        DPRCX(23,ID,ibif)=PH2IN(ID,curiyr)      ! H2 fuel has no CO2 tax
...
!    multiplied by variable ab_allow_p (California only) are equal to 87$/kg MMBTU
        IF(AB32SW.EQ.1.AND.CURCALYR.GE.ibyr2) then
			iyy=max(curiyr,6)
!         emis(1)=em_elec(9,11,icy)/qelas(11,iy)       ! elec note: qelas in quads here
			emis(1)=sum(em_elec(9,8:9,(curiyr+baseyr-1)))/sum(qelas(8:9,max(curiyr,6)))
			...
			emis(6)=erlin(iyy)   * C_CO2_FACTOR          ! resid (low sulfur)
			emis(7)=edsin(iyy)   * C_CO2_FACTOR          ! distillate
			emis(8)=elgin(iyy)   * C_CO2_FACTOR          ! propane
			...
			emis(13)=epfin(iyy)  * C_CO2_FACTOR          ! petroleum (naphtha) feedstock
			emis(14)=eksin(iyy)  * C_CO2_FACTOR          ! kerosene
			emis(15)=eotin(iyy)  * C_CO2_FACTOR          ! other ind
			emis(16)=enqngpf(iyy)  * C_CO2_FACTOR        ! nat gas feedstock
			emis(17)=enqlgpf(iyy)  * C_CO2_FACTOR        ! HGL feedstocks
			emis(18)=0.0           * C_CO2_FACTOR        ! renewables, all sequestered
			emis(19)=0.0                                 ! H2 feedstock placeholder
			emis(20)=0.0                                 ! H2 fuel
          
			DPRCX(1,9,ibif)=aELIN(9,curiyr)+0.*emis(1)*ab_allow_p(curiyr)   ! electricity. Marked-up for carbon in EMM.
			DPRCX(2,9,ibif)=PH2IN(9,curiyr)                                 ! H2 feedstock has no CO2 tax
			...
			DPRCX(6,9,ibif)=aLPIN(9,curiyr)+emis(2)*ab_allow_p(curiyr)      ! NG lease and plant fuel
			...
			DPRCX(12,9,ibif)=aLGIN(9,curiyr)+0.*emis(8)*ab_allow_p(curiyr)     ! propane H&P. Marked-up for carbon in LFMM.
			...
			DPRCX(19,9,ibif)=aPFIN(9,curiyr)+emis(13)*ab_allow_p(curiyr)    ! Petrochemical (naphtha) feedstock
			DPRCX(20,9,ibif)=aKSIN(9,curiyr)+0.*emis(14)*ab_allow_p(curiyr) ! Kerosene. Marked-up for carbon in LFMM.
			DPRCX(21,9,ibif)=0.0  !aRSIN(9,curiyr)+emis(13)*ab_allow_p(curiyr)    ! other feedstock; doesn't exist in IDM
			DPRCX(22,9,ibif)=aOTIN(9,curiyr)+emis(15)*ab_allow_p(curiyr)    ! Other Petroleum
			DPRCX(23,9,ibif)=PH2IN(9,curiyr)                                ! H2 fuel has no CO2 tax
        ENDIF
...
!  These are from the unadjusted price common block, MPBLK
		...
		DPRCX(2,ID,ibif)=PH2IN(ID,curiyr)
        ...
        DPRCX(13,ID,ibif)=PLGINPF(ID,curiyr)  ! use the HGL feed price from pmore
        ...
        DPRCX(23,ID,ibif)=PH2IN(ID,curiyr)
...
!         emis(1)=em_elec(9,11,icy)/qelas(11,iy)       ! elec note: qelas in quads here
          ...
          emis(18)=0.0           * C_CO2_FACTOR        ! renewables, all sequestered
          emis(19)=0.0                                 ! H2 feedstock placeholder; will need to account for production methods    NSK H2
          emis(20)=0.0                                 ! H2 placeholder; will need to account for production methods    NSK H2          
...
!          DPRCX(2,9,ibif)=PELIN(9,curiyr)-emis(1)*ab_allow_p(curiyr)         ! not used. Remove marked-up for carbon in EMM.
          IF(CURCALYR.GE.ibyr) DPRCX(3,9,ibif)=PGFIN(9,curiyr)+emis(2)*ab_allow_p(curiyr)      ! NG core (PA and BLD) POT_ibyr215 2015 to ibyr most next 17 rows
          IF(CURCALYR.GE.ibyr) DPRCX(4,9,ibif)=PGIIN(9,curiyr)+emis(2)*ab_allow_p(curiyr)      ! NG non-core (BSC)    
          IF(CURCALYR.GE.ibyr) DPRCX(5,9,ibif)=PGFIN(9,curiyr)+emis(16)*ab_allow_p(curiyr)     ! NG feedstock
          IF(CURCALYR.GE.ibyr) DPRCX(6,9,ibif)=PLPIN(9,curiyr)+emis(2)*ab_allow_p(curiyr)      ! NG L&P
          ...
          IF(CURCALYR.GE.ibyr) DPRCX(10,9,ibif)=PRLIN(9,curiyr)+emis(6)*ab_allow_p(curiyr)     ! residual fuel, low sulfur
          IF(CURCALYR.LT.ibyr) DPRCX(11,9,ibif)=PDSIN(9,curiyr)-emis(7)*ab_allow_p(curiyr)        ! distillate. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LT.ibyr) DPRCX(12,9,ibif)=PLGIN(9,curiyr)-emis(8)*ab_allow_p(curiyr)        ! LPG H&P. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LT.ibyr) DPRCX(13,9,ibif)=PLGINPF(9,curiyr)-emis(17)*ab_allow_p(curiyr)     ! the adjusted lpg feed price. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LT.ibyr) DPRCX(14,9,ibif)=PMGIN(9,curiyr)-emis(9)*ab_allow_p(curiyr)        ! mogas. Remove marked-up for carbon in LFMM.
          ...
          IF(CURCALYR.GE.ibyr) DPRCX(18,9,ibif)=PRSIN(9,curiyr)+emis(6)*ab_allow_p(curiyr)     ! Residual Fuel (average)
          DPRCX(19,9,ibif)=PPFIN(9,curiyr)+0.*emis(13)*ab_allow_p(curiyr)    ! Petrochemical Feedstocks
          IF(CURCALYR.LT.ibyr) DPRCX(20,9,ibif)=PKSIN(9,curiyr)-emis(14)*ab_allow_p(curiyr)       ! Kerosene. Remove marked-up for carbon in LFMM.
...
!		   Electrolyzer= HMM electrolyzers
! first make sure the prices are not zero!  ****
		...
		IF(pelinh2e(id,curiyr).le.0) &
            pelinh2e(id,curiyr)=pelinh2e(id,curiyr-1)
			
        DPRCX(23,ID,1:2)=PH2IN(ID,curiyr)
		DPRCX(24,ID,1:2)=PELINp(ID,curiyr)  ! for electricity, no difference between adj and unadjusted prices
        DPRCX(25,ID,1:2)=PELINs(ID,curiyr)
        DPRCX(26,ID,1:2)=PELINm(ID,curiyr)
		DPRCX(27,ID,1:2)=PELINH2e(ID,curiyr)
    ENDDO
...
!! Do lag prices
      ...
      DO ID=1,11
        ...
        DPRCXLAG(2,ID,ibif)=PH2IN(ID,iy)     ! H2 feedstock has no carbon tax
        ...
        dprcxlag(13,ID,ibif)=aLGINPF(ID,iy)  ! the adjusted HGL feed price
        ...
        DPRCXLAG(23,ID,ibif)=PH2IN(ID,iy)     ! H2 fuel has no carbon tax
...
!    multiplied by the California allowance cost, ab_allow_p, are equal to 87$/kg MMBTU
        IF(AB32SW.EQ.1.AND.CURCALYR.GE.ibyr2) then      ! POT_ibyr2
          iyy=max(curiyr-1,6)
!         emis(1)=em_elec(9,11,icy)/qelas(11,iy)       ! elec note: qelas in quads here
          emis(1)=sum(em_elec(9,8:9,iy+baseyr-1))/sum(qelas(8:9,max(iy,6)))
          emis(2)=engin(iyy)   * C_CO2_FACTOR          ! ngas
          emis(3)=eclin(iyy)   * C_CO2_FACTOR          ! steam coal
          emis(4)=emcin(iyy)   * C_CO2_FACTOR          ! met coal
          emis(5)=eclin(iyy)   * C_CO2_FACTOR          ! net coke import (steel only)
          ...
          DPRCXLAG(2,9,ibif)=PH2IN(9,iy)     						! H2 feedstock has no carbon tax
          ...
          DPRCXLAG(23,9,ibif)=PH2IN(9,iy)     					! H2 fuel has no carbon tax
...
!  These are from the unadjusted price common block, MPBLK
        ...
        dprcxlag(2,ID,ibif)=PH2IN(ID,iy)
        ...
        dprcxlag(13,ID,ibif)=PLGINPF(ID,iy)  ! use the HGL feed price from pmore
        ...
        dprcxlag(23,ID,ibif)=PH2IN(ID,iy)
...
!         emis(1)=em_elec(9,11,icy)/qelas(11,iy)       ! elec note: qelas in quads here
          ...
          emis(15)=eotin(iyy)  * C_CO2_FACTOR          ! other ind
          emis(16)=enqngpf(iyy)  * C_CO2_FACTOR        ! nat gas feed stocks
          emis(17)=enqlgpf(iyy)  * C_CO2_FACTOR        ! HGL feedstock
          emis(18)=0.            * C_CO2_FACTOR        ! renewables, all sequestered
          emis(19)=0.0                                 ! H2 feedstock placeholder; will need to account for production methods    NSK H2
          emis(20)=0.0                                 ! H2 placeholder; will need to account for production methods    NSK H2          

          IF(CURCALYR.GT.ibyr) DPRCXLAG(3,9,ibif)=PGFIN(9,iy)+emis(2)*ab_allow_p(iy)      ! NG core (PA and BLD) POT_ibyr215 2015 to ibyr most next 17 rows
          IF(CURCALYR.GT.ibyr) DPRCXLAG(4,9,ibif)=PGIIN(9,iy)+emis(2)*ab_allow_p(iy)      ! NG non-core (BSC)
          IF(CURCALYR.GT.ibyr) DPRCXLAG(5,9,ibif)=PGFIN(9,iy)+emis(16)*ab_allow_p(iy)     ! NG feedstock
          IF(CURCALYR.GT.ibyr) DPRCXLAG(6,9,ibif)=PLPIN(9,iy)+emis(2)*ab_allow_p(iy)      ! NG L&P
          ...
          IF(CURCALYR.GT.ibyr) DPRCXLAG(10,9,ibif)=PRLIN(9,iy)+emis(6)*ab_allow_p(iy)     ! residual fuel, low sulfur
          IF(CURCALYR.LE.ibyr) DPRCXLAG(11,9,ibif)=PDSIN(9,iy)-emis(7)*ab_allow_p(iy)        ! distillate. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LE.ibyr) DPRCXLAG(12,9,ibif)=PLGIN(9,iy)-emis(8)*ab_allow_p(iy)        ! LPG H&P. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LE.ibyr) DPRCXLAG(13,9,ibif)=PLGINPF(9,iy)-emis(17)*ab_allow_p(iy)     ! the adjusted lpg feed price. Remove marked-up for carbon in LFMM.
          IF(CURCALYR.LE.ibyr) DPRCXLAG(14,9,ibif)=PMGIN(9,iy)-emis(9)*ab_allow_p(iy)        ! mogas. Remove marked-up for carbon in LFMM.
          ...
          IF(CURCALYR.GT.ibyr) DPRCXLAG(18,9,ibif)=PRSIN(9,iy)+emis(6)*ab_allow_p(iy)     ! Residual Fuel (average)
          DPRCXLAG(19,9,ibif)=PPFIN(9,iy)+0.*emis(13)*ab_allow_p(iy)    ! Petrochemical Feedstocks
          IF(CURCALYR.LE.ibyr) DPRCXLAG(20,9,ibif)=PKSIN(9,iy)-emis(14)*ab_allow_p(iy)       ! Kerosene. Remove marked-up for carbon in LFMM.
...
!  groups: Primary=food,paper,chemicals,steel,aluminum;
!          Shift  =metal based durables;
!          Miscellaneous =all the rest except H2 electrolyzers
!		   H2E = H2 electrolyzer load
! first make sure the prices are not zero!  ****
        if(pelinp(id,curiyr).le.0) &
            pelinp(id,curiyr)=pelinp(id,curiyr-1)
        if(pelins(id,curiyr).le.0) &
        ...
        iy=curiyr-1
        dprcxlag(23,ID,1:2)=PH2IN(ID,iy)
		dprcxlag(24,ID,1:2)=PELINp(ID,iy)  ! for electricity, no difference between adj and unadjusted prices
        dprcxlag(25,ID,1:2)=PELINs(ID,iy)
        dprcxlag(26,ID,1:2)=PELINm(ID,iy)
	    dprcxlag(27,ID,1:2)=PELINH2e(ID,iy)	
      ENDDO
...
!******
      IF(CURCALYR.LE.SEDSLastYr) THEN
        I = curiyr
        DO ID=1,11
          DCONX(1,ID)=QSELIN(ID,I)
          DCONX(2,ID)=QH2INPF(ID,I)               ! No H2 feedstock in SEDS
		  ...
		  DCONX(23,ID)=QH2INHP(ID,I)
          DCONX(24,ID)=qELINp(ID,I)
          DCONX(25,ID)=qELINs(ID,I)
          DCONX(26,ID)=qELINm(ID,I)
		  DCONX(27,ID)=qELINH2e(ID,I)
        ENDDO
      ELSE
        DO ID=1,11
         DCONX(1,ID)=QELIN(ID,curiyr-1)
         DCONX(2,ID)=QH2INPF(ID,curiyr-1)              ! Placeholder for H2 feedstock NSK H2
		 ...
		 DCONX(23,ID)=QH2INHP(ID,curiyr-1)
         DCONX(24,ID)=qELINp(ID,curiyr-1)
         DCONX(25,ID)=qELINs(ID,curiyr-1)
         DCONX(26,ID)=qELINm(ID,curiyr-1)
		 DCONX(27,ID)=qELINH2e(ID,curiyr-1)
       ENDDO
      ENDIF
    DO ibif=1,2
		DO IF=1,27
...
!******
    ibif=1
        ...
        PRCX(23,5,ibif)=PH2IN(11,curiyr)
		PRCX(24,5,ibif)=pELINp(11,curiyr)
        PRCX(25,5,ibif)=pELINs(11,curiyr)
        PRCX(26,5,ibif)=pELINm(11,curiyr)
		PRCX(27,5,ibif)=pELINH2e(11,curiyr)
! Unadjusted Prices:  for "Bifurcated" carbon allowance scheme where some covered; some not.
!  These are from the unadjusted price common block, MPBLK
    ibif=2
        ...
        PRCX(13,5,ibif)=PLGINPF(11,curiyr)  ! the HGL feed price
...
!******
    ibif=1
        ...
        prcxlag(2,5,ibif)=PH2IN(11,curiyr-1)			! H2 feedstock has no CO2 tax
		...
		prcxlag(23,5,ibif)=PH2IN(11,curiyr-1)			! H2 fuel has no CO2 tax
        prcxlag(24,5,ibif)=PELINp(11,curiyr-1)
        prcxlag(25,5,ibif)=PELINs(11,curiyr-1)
        prcxlag(26,5,ibif)=PELINm(11,curiyr-1)
		prcxlag(27,5,ibif)=PELINH2e(11,curiyr-1)
! Unadjusted Prices:  for "Bifurcated" carbon allowance scheme where some covered; some not.
!  These are from the unadjusted price common block, MPBLK
    ibif=2
        ...
        prcxlag(13,5,ibif)=PLGINPF(11,curiyr-1)  ! the HGL feed price
...
!******
      ...
      INTEGER IP,IPOLL,IREG,indx  ! kpe indx is industry
      INTEGER ISPROCFLOW(numind)
...
!******     
      INTEGER FEEDYEARS, MAX_YEARS, QCR_YEARS, ASM_YEARS

      ! will have a varying number of entries, depending on the start (ibyr) and end (SEDSLASTYR+FEEDYEARS)of the data period
...
!******
      IF(IYR.EQ.ibyr2.AND.curitr.EQ.1) THEN
        CALL RCNTL  !   Read indrun.txt and itlbshr.txt (boiler elasticities and latest MECS bsc fuel totals)
        if (inddir.ne.8 .and. inddir.ne.12) CALL MecsLess860b ! calculate base year boiler fuel by subtracting 860b CHP fuel from Mecs indirect fuels
      ENDIF
...
! update lag variables to initial data for current year.
   ...
   IF(IYR.GT.ibyr2.AND.curitr.EQ.1) THEN  ! fstiter instead of curitr/
        DO IREG=1,4
          indreg=IREG
          do indnum=1,indmax
             inddir=indnum
...
!******
	  ...
	  FNAME='CARBSHRCA         '   ! Specify file to read in Califorina's carbon shares data (from 2018)
      NEW=.FALSE.
      IUNITCARBSHRCA=FILE_MGR('O',FNAME,NEW)
		
      Call Read_carbshrca(IUNITCARBSHRCA, carbshrcadata, NUMIND)
      IUNITCARBSHRCA=FILE_MGR('C',FNAME,NEW)
      ...
      DO 45 INDNUM=1,INDMAX
        ...
        IF ((CURCALYR .eq. ibyr) .AND. (CURITR .eq. 1)) THEN
            ...
            Call Read_coal(QCR_YEARS, IUNITCOAL, qcrdata, MNUMYR, NUMIND, IBYR, ICURIYR, qcrcokeplnt, qcrindchp, qcrindnchp)
            IUNITCOAL=FILE_MGR('C',FNAME,NEW)


	        FNAME='IND_ELECTRIC         '   ! Specify file to read in electricity data/projections (from 2018 to SEDS year plus 2)
            NEW=.FALSE.
            IUNITELEC=FILE_MGR('O',FNAME,NEW)
            ...
            DO IREG=1,4
                indreg=IREG
                IF ((inddir.eq.8).or.((inddir.ge.10).and.(inddir.le.13))) then     ! Use Quarterly Coal Report data for steam coal instead of MECS
                    IF (MECS_Data(inddir, 5, 4).ne.0.0) THEN    ! Keep value at 0 if MECS is at 0, even if QCR is nonzero
                        qcrdata(icuriyr,inddir,indreg) = qcrdata(icuriyr,inddir,5) * MECS_Data(inddir, indreg, 4)/MECS_Data(inddir, 5, 4) ! QCR total divided by region based on MECS
                        MECS_Data(inddir, indreg,4) = qcrdata(icuriyr,inddir,indreg)                                                 ! QCR value replaces MECS values
                        IF (indreg.eq.4) THEN
                            MECS_Data(inddir, 5,4) = SUM(MECS_Data(inddir, 1:4,4))
                        ENDIF
                    ENDIF
                ENDIF
            ENDDO
	  ...
	  if (inddir == 8 .or. (inddir >=10 .and. inddir <= 13) ) ISPROCFLOW(inddir) = 1
...
! read building energy for 1)lighting, 2)hvac, 3)facility support, 4) onsite transportation,
!							5) Conventional electricity generation, and 6) Other nonprocess

! fuels indices are 1: elec, 2:ngas, 3:steam, 4:dist, 5:propane
            enbint=0.
            read(ibeu,*) inpind,dumc,inpreg,enbint(1,1), &                  ! lighting   # KPE updated 9/23 
                                            enbint(2,1:3), &               ! hvac         OK
                                            enbint(3,1:2), & 				! facility support
                                           (enbint(4,1),enbint(4,4:5)), &  	! transport
                                            enbint(5,2),    			&	! conventional electricity generation
                                            enbint(6,1:2)     				! other nonprocess 
...
! set electricity prices for the industry as baseload or medium load, or other
           if (inddir.eq.7.or.inddir.eq.8.or.inddir.eq.9.or.  & ! primary
            ...
            elseif(inddir.ge.14.and.inddir.le.18) then ! 5 metal based durables industries
              ...
              prcx(1,indreg,1:2)=prcx(26,indreg,1:2)
              prcxyr(1,indreg,curiyr,1:2)=prcxyr(26,indreg,curiyr,1:2)
            endif
            IF(IWDBG.EQ.1.AND.IOPEN.EQ.1.AND.LSTITER.EQ.1) &
   ...
   use i_
      ...
      INTEGER ifuel
       ...
       DO IR=1,4
          ...
          PRCX(26,IR,1:2)=ELEC*PRCX(26,IR,1:2)
		  PRCX(27,IR,1:2)=ELEC*PRCX(27,IR,1:2)
...
! Do lag prices
       DO IR=1,4
          ...
          prcxlag(26,IR,1:2)=ELEC*prcxlag(26,IR,1:2)
		  prcxlag(27,IR,1:2)=ELEC*prcxlag(27,IR,1:2)
...
!******
	...
	INTEGER INDBMOVR
    INTEGER INDBMOVR2         ! Benchmark Switch
...
!******
    ...
    real, parameter:: Ethane_heat = 0.0000491794  ! trillion Btu/tonne
    real, parameter:: Naphtha_heat = 0.0000431386 ! trillion Btu/tonne
      ...
      real Yields_ethane(11)/0.8091, 0.0591, 0.0704, 0.0194, 0.0178, 0.0081, 0.0081, 0.0, 0.0008, 0.0, 0.0073/           ! Yields from ethane cracking
      real Yields_naphtha(11)/0.3867, 0.0097, 0.1694, 0.1547, 0.0476, 0.0507, 0.0437, 0.0224, 0.0166, 0.0251, 0.0735/    ! Yields from naphtha cracking
    ...
    ! FEEDSTOCK PERCENTAGES BY REGION PROVIDED BY PETER
      REAL FEEDNGPERCENT(4)/0.11,0.0,0.89,0.0/      ! Based on MECS 2018, with everything but methanol (NAICS 325199) moved to HMM	  
      REAL FEEDHGLPERCENT(4) /0.01,0.03,0.96,0.0/     ! NSK updated 10/16/2019, based on MECS 2014 (MECS 2018 has data withheld)
       !! REAL COALCOKEIMPORTPERCENT(11)/0.0,0.18,0.55,0.01,0.0,0.01,0.25,0.0,0.0,0.0,1.0/    ! Approximate 2014–18 average share of net coal coke exports from 1/2020 Quarterly Coal Report (complicated
      ...
      REAL COALCOKEIMPORTPERCENT(11)/0.0, 0.28, 0.41, 0.01, -0.02, 0.0, 0.31, 0.0, 0.01, 0.0,1.0/! Approximate 2017–21 average share of net coal coke exports based on 1/2023 Quarterly Coal Report. Complicated
...
! FEEDHGLtotal, feedngtotal, and feedethtotal will have a varying number of entries, depending on the start (ibyr) and end (SEDSLASTYR+FEEDYEARS)of the data period
      ...
      REAL,ALLOCATABLE::FEEDHGLtotal(:)             ! FEEDHGLtotal is allocatable, because the number of entries can change every year
...
! H2 FEEDSTOCK         	TQMAIN(2,ICR)    BMAIN(2,1:5)    QH2INPF                    NSK H2
...
! PROPANE H&P     		TQMAIN(12,ICR)   BMAIN(12,1:5)    QLGIN
! HGL FEEDSTOCK      	TQMAIN(13,ICR)   BMAIN(13,1:5)    INQLGPF
...
! NAPHTHA FEEDSTOCK  	TQMAIN(19,ICR)   BMAIN(19,1:5)    QPFIN
! KEROSENE        		TQMAIN(20,ICR)   BMAIN(20,1:5)    QKSIN = constant after last STEO year
! OTHER PETRO FEEDSTOCK TQMAIN(21,ICR)   BMAIN(21,1:5)    QOTIN
! OTHER PETRO      		TQMAIN(22,ICR)   BMAIN(22,1:5)    QOTIN
! H2 H&P                TQMAIN(23,ICR)   BMAIN(23,1:5)    QH2INHP
...
IF (CURCALYR .eq. ibyr2 .AND. CURITR .eq. 1) THEN
    ...
    ALLOCATE(feedhgltotal(MAX_YEARS+10))
    ALLOCATE(feedethtotal(MAX_YEARS+10))
    ALLOCATE(feedpropanetotal(MAX_YEARS+10))
...
! New code to read in ethane and propane Census Region fractions NSK 10/3/2019
    ALLOCATE(EthaneDivFrac(9, MAX_FRAC_YEARS+10))
    ALLOCATE(PropaneDivFrac(9, MAX_FRAC_YEARS+10))

    FNAME='FEEDSTOCK         '      ! Specify file to read in HGL and natural gas feedstock data/projections
    NEW=.FALSE.
	...
	FNAME='IND_H2         '      ! Specify file to read in H2 feedstock data
    NEW=.FALSE.
    IUNITH2=FILE_MGR('O',FNAME,NEW)
...
IF(curcalyr.eq.ibyr) THEN
		chem_macro(1,1:9)=MC_REVIND(1:9,51,curiyr)	! industrial gases
		chem_macro(2,1:9)=MC_REVIND(1:9,52,curiyr)	! other inorganics
		chem_macro(3,1:9)=MC_REVIND(1:9,49,curiyr)	! petrochemicals
		chem_macro(4,1:9)=MC_REVIND(1:9,50,curiyr)	! other organics
		chem_macro(5,1:9)=MC_REVIND(1:9,18,curiyr)	! resins
		chem_macro(6,1:9)=MC_REVIND(1:9,19,curiyr)	! ag chems
	DO ISUB=1,6	! For 6 chemical sub-subsectors
		H2_chem_demand_div(1:2,ISUB)=chem_macro(ISUB,1:2)/SUM(chem_macro(ISUB,1:2))*H2_chem_demand_reg(ISUB,1)
		H2_chem_demand_div(3:4,ISUB)=chem_macro(ISUB,3:4)/SUM(chem_macro(ISUB,3:4))*H2_chem_demand_reg(ISUB,2)
		H2_chem_demand_div(5:7,ISUB)=chem_macro(ISUB,5:7)/SUM(chem_macro(ISUB,5:7))*H2_chem_demand_reg(ISUB,3)
		H2_chem_demand_div(8:9,ISUB)=chem_macro(ISUB,8:9)/SUM(chem_macro(ISUB,8:9))*H2_chem_demand_reg(ISUB,4)
		
		H2_chem_supply_div(1:2,ISUB)=chem_macro(ISUB,1:2)/SUM(chem_macro(ISUB,1:2))*H2_chem_supply_reg(ISUB,1)
		H2_chem_supply_div(3:4,ISUB)=chem_macro(ISUB,3:4)/SUM(chem_macro(ISUB,3:4))*H2_chem_supply_reg(ISUB,2)
		H2_chem_supply_div(5:7,ISUB)=chem_macro(ISUB,5:7)/SUM(chem_macro(ISUB,5:7))*H2_chem_supply_reg(ISUB,3)
		H2_chem_supply_div(8:9,ISUB)=chem_macro(ISUB,8:9)/SUM(chem_macro(ISUB,8:9))*H2_chem_supply_reg(ISUB,4)
		IF (ISUB .eq. 1) THEN	! assume all merchant gases in census region 3 are produced in division 7, and all merchant gases in census region 4 are produced in census division 9
				H2_chem_demand_div(5:6,ISUB)=0.0	! (this is 0 for demand anyways, unless we eventually model an H2 demand in merchant gases)
				H2_chem_demand_div(7,ISUB)=H2_chem_demand_reg(ISUB,3)
				H2_chem_demand_div(8,ISUB)=0.0
				H2_chem_demand_div(9,ISUB)=H2_chem_demand_reg(ISUB,4)
				
				H2_chem_supply_div(5:6,ISUB)=0.0
				H2_chem_supply_div(7,ISUB)=H2_chem_supply_reg(ISUB,3)
				H2_chem_supply_div(8,ISUB)=0.0
...
! CREATE FADE FACTORS. FADE IS USED TO CREATE A COMPOSITE BENCHMARK FACTOR.
	   ...
	   FADE = fade_factor(FADEYRS,INDBMOVR2)  ! KPE function--# of years to fade, indbmovr; 
	                                          ! based on sin function, so last two years very small fade for continuity
    ...
    REF(5) = 0
    REF(6) = 0
    REF(7) = QCLRF(11,CURIYR) + QCLETH(CURIYR,11)
    REF(8) = 0
    REF(9) = 0
    REF(10) = QRLRF(11,CURIYR)
...
IF (CURIYR .LE. MSEDYR)  THEN
	IF (FCRL .EQ. 1) THEN
		...
		IF (CURIYR.ge.ICURIYR) WRITE(IUNITBENCH,*) "NATURAL GAS: ","QSNGIN = ", QSNGIN(11,CURIYR), &
            ...
            "CGOGSQ = ",CGOGSQ(11,CURIYR,ING3), &
               "FEEDNGTOTAL = ", FEEDNGTOTAL(CURIYR-ICURIYR+1)
		WRITE(IUNITBENCH,*) "COAL ", "QSCLIN = ", QSCLIN(11,CURIYR), "QCLRF = ", &
             QCLRF(11,CURIYR), "QCLETH = ",QCLETH(CURIYR,11), &
             "CGOGSQ = ", CGOGSQ(11,CURIYR,ICL1), "OGSUPGAS(1,11,IY) * CFNGC(IY) =", OGSUPGAS(1,11,IY) * CFNGC(IY)
       ...
       IF (CURIYR.ge.ICURIYR) WRITE(IUNITBENCH,*) "LPG ", "QSLGIN = ", QSLGIN(11,CURIYR), "QSKSIN = KEROSENE = ",&
          QSKSIN(11,CURIYR), "QLGRF = ", QLGRF(11,CURIYR), &
              "FEEDHGLTOTAL = ", FEEDHGLTOTAL(CURIYR-ICURIYR+1)
...
! The hardcoded CFELQ/10**9 above is to convert CCATS electricity pipeline use from MWh to trillion Btu 
    ...
    !  QNGETH HAS DIFFERENT ARRAY DIMENSIONS
		SEDS9(ICD,3) = (QSNGIN(ICD,CURIYR) - QNGRF(ICD,CURIYR) - &
			QNGETH(CURIYR,ICD) - SEDS9(ICD,5) - &
			CGOGSQ(ICD,CURIYR,ING3) - QNGHMHP(ICD,CURIYR) - QNGHMPF(ICD,CURIYR)) * FIRMSHR(ICR)
    ...
    ! STEAM COAL
      ...
      ! NET COKE IMPORTS
		SEDS9(ICD,9) = QSCIIN(11,CURIYR) * COALCOKEIMPORTPERCENT(ICD)
      ...
      ! PET COKE (DOES include refinery cat coke)
...
! NOW CREATE BF_SEDS.  IF BMAIN=0, BF_SEDS SHOULD JUST BE 1.
    DO ICR = 1,4
...
!**** SEDS BENCHMARK FOR MAIN FUELS  ***************************
    DO IFUEL=1,mainfuels-1
        ...
        ELSE
			BF_SEDS(IFUEL,ICR) = 1.0
        END IF

    ENDDO
	
! redundant with BF_SEDS=(ifuel,:) below, except now the output file gets the correct values for H2 and lease & plant fuel regional BF_SEDS
	BF_SEDS(2,ICR) = 1.0  ! hydrogen feedstock      NSK H2
    BF_SEDS(6,ICR) = 1.0
    BF_SEDS(23,ICR) = 1.0 ! hydrogen heat & power     NSK H2
...
!**** SEDS BENCHMARKS FOR THE RENEWABLES **********************
      DO IFUEL=1,8
		...
		ELSE
			RBF_SEDS(IFUEL,ICR) = 1.0
		END IF
      END DO
      RBF_SEDS(8,ICR) = 1.0  ! KPE set msw benchmark factor at 1.
...
!*******************************************************************************
    ...
    BF_SEDS(2,:) = 1.0  ! hydrogen feedstock      NSK H2
    BF_SEDS(6,:) = 1.0	! lease and plant fuel
    BF_SEDS(23,:) = 1.0 ! hydrogen heat and power      NSK H2
...
!###
        ...
        STEO(3) = MAX(0.0,(STEOQ(CURCALYR,13) - QNGRF(11,CURIYR) - &
            QNGETH(CURIYR,11) - FEEDNGTOTAL(CURIYR-ICURIYR+1) - &
			SUM(CGOGSQ(1:9,CURIYR,ING3)) - QNGHMHP(11,CURIYR) - QNGHMPF(11,CURIYR)) * FIRMSHR(5))

        ! NG H&P NONFIRM
        STEO(4) = STEO(3) * ((1. - FIRMSHR(5)) / FIRMSHR(5))

        ! NG FEEDSTOCK ... THIS IS READ INTO PROGRAM AND WILL CREATE A BENCHFACTOR OF 1
        STEO(5) = FEEDNGTOTAL(CURIYR-ICURIYR+1)
...
!        STEO(6) = STEOQ(CURCALYR,14)
        ...
        STEO(7) = MAX( 0.0, STEOQ(CURCALYR,15) - QCLRF(11,CURIYR) - &
            QCLETH(CURIYR,11) - SUM(CGOGSQ(1:9,CURIYR,ICL1)) - SUM(OGSUPGAS(1,1:9,CURIYR)) * CFNGC(CURIYR) )
        ...
        STEO(12) = MAX(0.0,STEOQ(CURCALYR,3) - &
         QLGRF(11,CURIYR) - FEEDHGLTOTAL(CURIYR-ICURIYR+1))

        ! HGL FEEDSTOCK
         STEO(13) = FEEDHGLTOTAL(CURIYR-ICURIYR+1)

        ! MOGAS
        STEO(14) = STEOQ(CURCALYR,4)

        ! STILL GAS
        STEO(15) = 0.0  ! SET TO ZERO BECAUSE INDUSTRIAL DOES NOT USE STILL GAS

        ! PET COKE (DOES NOT include refinery catcoke; also assumes no refinery consumption of marketable coke/petcoke)
        STEO(16) = STEOQ(CURCALYR,9) !- QPCRF(11,CURIYR) - QCCRF(11,CURIYR))
        ...
        ! HYDROGEN, but it doesn't exist in STEO (yet)
        STEO(23) = 0.0
...
! denominator could be negative
        ...
        END DO
		BF_STEO(2) = 1.0            ! H2 feedstock (not in STEO)    NSK H2
        BF_STEO(23)= 1.0            ! H2 (not in STEO)    NSK H2
...
!
    ...
    IF(curcalyr.le.END_YR_FEEDSTOCK)THEN
        HGL_recycle_rate(curiyr)=1.0
    ELSE
        HGL_recycle_rate(curiyr)=(1.0-0.0132*PETIN(11,curiyr)-0.0218)/HGL_recycle_rate_base
    ENDIF

! Model flexible cracking capacity; only exists in division 7/region 3
    IF (curcalyr.eq.sedslastyr+FEEDYEARS) THEN
        ...
        Naphtha_flex_consumption(3,curiyr)=MAX(0.0, feednaphtotal(CURIYR-ICURIYR+1)-Naphtha_nonflex_consumption)  ! Naphtha consumption from flexible cracking capacity, in tBtu
        ...
        Ethylene_slope=(5.0*dot_product(Ethylene_x,Ethylene_y)-SUM(Ethylene_x(1:5))*SUM(Ethylene_y(1:5)))/ &
                        (5.0*SUM(Ethylene_x(1:5)**2)-SUM(Ethylene_x(1:5))**2)								! equation for linear regression slope (tonnes ethylene/2012$ shipments)
						
        Ethylene_intercept=(SUM(Ethylene_y(1:5))-Ethylene_slope*SUM(Ethylene_x(1:5)))/5.0 					! equation for linear regression intercept (tonnes ethylene)
    ENDIF
    ...
    IF (curcalyr.gt.sedslastyr+FEEDYEARS) THEN
        ...
        Cost_ethylene_from_ethane(3,curiyr)=Ethane_price_mass(3,curiyr)/Yields_ethane(1)     ! Cost of ethylene cracked from ethane, in 1987$/tonne ethylene produced
        Cost_ethylene_from_naphtha(3,curiyr)=Naphtha_price_mass(3,curiyr)/Yields_naphtha(1)  ! Cost of ethylene cracked from naphtha, in 1987$/tonne ethylene produced
        ...
        DO CHEM_INDEX=2,11                  ! Comparison is ultimately being done on a per-tonne-ethylene basis, so ethylene price (CHEM_INDEX=1) doesn't matter
            ...
            IF (CHEM_INDEX .eq. 2) THEN         ! Hydrogen
                Chem_product_price(CHEM_INDEX, curiyr)=PH2IN(7,curiyr)*H2_heat
            ...
            Cost_ethylene_from_ethane(3,curiyr)=Cost_ethylene_from_ethane(3,curiyr)-Chem_product_price(CHEM_INDEX, curiyr)*Yields_ethane(CHEM_INDEX)/Yields_ethane(1)
            Cost_ethylene_from_naphtha(3,curiyr)=Cost_ethylene_from_naphtha(3,curiyr)-Chem_product_price(CHEM_INDEX, curiyr)*Yields_naphtha(CHEM_INDEX)/Yields_naphtha(1)
        ...
        If (Cost_ethylene_from_ethane(3,curiyr) .gt. Cost_ethylene_from_naphtha(3,curiyr)) then
	        Naphtha_flex_consumption(3,curiyr)=MIN(Quick_flex_capacity(3,curiyr)*Naphtha_heat/Yields_naphtha(1), &
             Naphtha_flex_consumption(3,curiyr-1)+0.5*Quick_flex_capacity(3,curiyr)*Naphtha_heat/Yields_naphtha(1))
        EndIf
        ...
        Do ICD=1,9           
            ...
            IF (ICD .eq. 7) THEN    ! Assuming flex capacity and regular capacity growth is only ever in census division 7
                QETINPF(ICD,curiyr)=Base_ethane_demand(curiyr)-(Naphtha_flex_consumption(3,curiyr)*Yields_naphtha(1)/Naphtha_heat)*Ethane_heat/Yields_ethane(1) &
    ...
    DO IFUEL=1,mainfuels-1
       ...
       BF(CURIYR,IFUEL,ICR) = (1 - FADE) * BF_SEDS(IFUEL,ICR) + &
        ...
        ! Custom benchmark factor targets for HGL feedstock, petchem feedstock, and natural gas feedstock
                IF (CURCALYR .GT. (SEDSLASTYR + FEEDYEARS)) THEN  
                    ...
                    IF (IFUEL .EQ. 13) THEN ! Index 13 = HGL feedstock
                        ...
                        Naphtha_to_HGL(3,curiyr)=(BMAIN(19,3)*BF(CURIYR,19,3)-(Naphtha_flex_consumption(3,curiyr) + Naphtha_nonflex_consumption))*(Yields_naphtha(1)/Naphtha_heat)*(Ethane_heat/Yields_ethane(1))
                        Naphtha_to_HGL(5,curiyr)=Naphtha_to_HGL(3,curiyr)
                        
                        HGLFEEDTARGET(5) = BMAIN(13,5) * HGLFEEDCALIBFACT                                                       ! Baseline adjustment based on last read in year
                        HGLFEEDTARGET(1:4) = MAX(HGL_recycle_rate(curiyr)*HGLFEEDTARGET(5) * FEEDHGLPERCENT(1:4) + Naphtha_to_HGL(1:4,curiyr), 0.0)      ! Add excess (or deficit) end-use model naphtha demand; should only be in region 3
                        HGLFEEDTARGET(5) = SUM(HGLFEEDTARGET(1:4))                                                              ! Re-sum regional values
                    END IF
                ELSEIF (CURIYR.ge.ICURIYR) THEN
                    NGFEEDTARGET(5) = FEEDNGTOTAL(CURIYR-ICURIYR+1)
                    NGFEEDTARGET(1:4) = NGFEEDTARGET(5) * FEEDNGPERCENT(1:4)
	                HGLFEEDTARGET(5) = FEEDHGLTOTAL(CURIYR-ICURIYR+1)
                    HGLFEEDTARGET(1:4) = HGLFEEDTARGET(5) * FEEDHGLPERCENT(1:4)
                    ! PCFFEEDTARGET only is used in years after SEDSLASTYR + FEEDYEARS
				ELSE
				    NGFEEDTARGET(1:5) = 0
                    HGLFEEDTARGET(1:5) = 0
                END IF	
        ...
        write (IFEEDOUT,'(A,5F10.3)') 'HGL     ',HGLFEEDTARGET(1:5)
    ...
    IF (CURCALYR .GT. (STEOLASTYR - INDBMOVR2)) THEN
        ...
        ELSE IF (IFUEL .EQ. 19) THEN
            IF (curcalyr.eq.sedslastyr+FEEDYEARS) THEN ! Use naphtha read-in value for sedslastyr+FEEDYEARS
                ...
                write (IFEEDOUT,*) 'curcalyr region PCFFEEDTARGET = ',curcalyr,ICR,feednaphtotal(CURIYR-ICURIYR+1)*NaphthaDivFrac(ICR)
                    BF(CURIYR,19,ICR) = feednaphtotal(CURIYR-ICURIYR+1)*NaphthaDivFrac(ICR) / BMAIN(19,ICR)
                ELSE
...
DO ICD=1,9
    ...
    ! RESIDUAL FUEL TOTAL AND RESIDUAL FUEL LOW
      ...
      QRLIN(ICD,IY) = QRSIN(ICD,IY)                             ! QRHRF=QRLRF=0 after STEO, but if industrial consumption of high-sulfur resid was ever non-0, these equations probably wouldn't work correctly
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	IF ((curcalyr.eq.STEOLastYr).or.(Curcalyr.eq.(STEOLastYr-1))) THEN ! LFMM doesn't benchmark QCCRF region-level numbers to STEO
		QPCIN(ICD,IY) = BMAIN_ADJUSTED(16,ICR) * DIVSHR(QSPCIN(1,MSEDYR),ICD,ICR) &
          + QPCRF(ICD,IY) + QCCRF(ICD,IY)*QCCRF(11,IY)/SUM(QCCRF(1:9,IY))
	ENDIF
    ...
    ! HYDROGEN H&P
      QH2INHP(ICD,IY) = 0.0          ! NSK H2
      
    ! HYDROGEN TOTAL
      QH2IN(ICD,IY) = QH2INPF(ICD,IY) + QH2INHP(ICD,IY)
	  
	! BASE YEAR BYPRODUCT H2
	  ...
	  ! zero out negative byproduct supply until we figure out the accounting with LFMM
...
! END RENEWABLE FUELS
         ...
         IF (CURCALYR .lt. SEDSLASTYR+1) Then             ! Fixed regional (and, slightly, total) read-ins froms SEDS numbers
          Do ID=1,11                                      ! (BMAIN_ADJUSTED isn't properly matching regional totals); NSK 1/15/2020        
              QELIN(ID,IY) = QSELIN(ID,IY)                ! Electricity---will be calibrated later; giving value for now
              ...
              QCLIN(ID,IY) = QSCLIN(ID,IY)                ! Steam coal---will be calibrated later; giving value for now
              QMCIN(ID,IY) = QSMCIN(ID,IY)                ! Met coal
              QCIIN(ID,IY) = QSCIIN(11,IY)*COALCOKEIMPORTPERCENT(ID)                ! Net coal coke imports based on approximate Census Division breakdown (average of 2017–21)
...
!  CREATE NATIONAL LEVEL Q ESTIMATES
    ...
    QH2INPF(11,IY) = sum(QH2INPF(1:9,IY))       ! NSK H2
...
!    INQNGHP(11,IY) = sum(INQNGHP(1:9,IY))  does not exist yet
	...
	BYPRDH2IN(11,IY)=sum(BYPRDH2IN(1:9,IY))	! NSK H2
...
! put another set of WRITE statements here ....
    IF (FCRL .EQ. 1) THEN
      ...
      IF (CURIYR .LE. MSEDYR)  THEN
        ...
        WRITE(IUNITBENCH,*) "QH2INPF = ", QH2INPF(11,CURIYR), "REF = ", REF(2)              ! NSK H2
        WRITE(IUNITBENCH,*) "QH2INHP = ", QH2INHP(11,CURIYR), "REF = ", REF(22)
        WRITE(IUNITBENCH,*) "QH2IN = ", QH2IN(11,CURIYR)
...
! ethane demand (national) - is technically part of the "HGL" in the MER
! assume that all ethane is used for petrochemical feedstock

! HGL feedstock ///////////////////////////////////////////////////////////// NSK 9/27/2019
    ...
    ELSEIF (CURIYR .gt. MSEDYR+FEEDYEARS) Then                          ! Put in projected numbers after MSEDYR+FEEDYEARS (2021 for AEO 2020); NSK 4/1/2020 
        ...
        qppinpf(11,curiyr) = (inqlgpf(11,curiyr)-qetinpf(11,curiyr)-qprolenerf(11,curiyr)-heattemp2(curiyr))*qppinpf(11,MSEDYR+FEEDYEARS)/ &                 ! Natural gasoline after the last read-in year (MSEDYR+FEEDYEARS, or 2027 for AEO 2026) is set to be
            ...
            IF (qppinpf(11,curiyr).lt.0.0) THEN
                ...
                qppinpf(11,curiyr)=0.0
            ENDIF
            
            IF (qisinpf(11,curiyr).lt.0.0) THEN
                qbuinpf(11,curiyr)=qbuinpf(11,curiyr)+qisinpf(11,curiyr)
                qisinpf(11,curiyr)=0.0
            ENDIF
            
            IF (qbuinpf(11,curiyr).lt.0.0) THEN
                qprinpf(11,curiyr)=qprinpf(11,curiyr)+qbuinpf(11,curiyr)
                qbuinpf(11,curiyr)=0.0                
            ENDIF
...
! BUT THE ALGORITHM FOR PRINTING OUT INDT.CSV IS CRYPTIC.
		...
		DO ICR=1,5
			...
			BENCHFAC(8,ICR) = BF(CURIYR,13,ICR)	! LPG NEEDS WORKW
            ...
            BENCHFAC(18,ICR) = BF(CURIYR,2,ICR)     ! H2 FEEDSTOCK
            BENCHFAC(19,ICR) = BF(CURIYR,22,ICR)    ! H2           
		END DO
...
! initialization.  prevents unsightly accumulation on each nems iteration
     QE2NGIN(:,iy,:)=0.
     QE2H2IN(:,iy,:)=0.     ! H2 variable needs to be added as global variable  NSK H2
     QE2CLIN(:,iy,:)=0.
     QE2MCIN(:,iy,:)=0.
     QE2MGIN(:,iy,:)=0.
     QE2DSIN(:,iy,:)=0.
     ...
     QE2H2IN(11,iy,1) = SUM(QH2INHP(1:9,iy))    ! H2 variable needs to be added as global variable  NSK H2
...
! Sum the non-covered share of energy for each fossil fuel over the numind industries
     do indus=1,numind
       ...
       QE2H2IN(11,iy,2) = QE2H2IN(11,iy,2) + (1.-carbshr(indus)) * QtyByInd( 19,indus)  ! NSK H2
...
! the policy's coverage assumptions.
     ...
     QE2H2IN(11,iy,1) = max(0.,QE2H2IN(11,iy,1) - QE2H2IN(11,iy,2))!   NSK H2
...
! CREATE NATIONAL BENCHMARK FACTORS FOR FEEDSTOCKS FOR LAST BENCHMARKED YEAR.
	IF (CURCALYR .LE. SEDSLASTYR + FEEDYEARS) THEN
		...
		HGLFEEDCALIBFACT = INQLGPF(11,IY) / BMAIN(13,5)
	END IF
! END CODE:
!******
991 FORMAT(3X,'WEXOG')
...
!-------------------STEO read-in from steovars.csv--------------
...
! Other petroleum (12)
     ...
     STEOQ(STEOLastYr,12)=INDABTCBUS(MSEDYR+3) + INDUOTCBUS(MSEDYR+3) + INDMSTCBUS(MSEDYR+3) + INDSNTCBUS(MSEDYR+3) + &
      ...
      DO IYEAR=STEOLastYr-6, STEOLastYr
          ...
          STEOQ(IYEAR,9) = INDPMTCPUS(IYEAR-1990+1)-INDPCEPCONB(IYEAR-1990+1)   ! Petroleum coke (marketed coke) is equal to total STEO petcoke (no catcoke) minus
...
!   READ IN ONE DATA RECORD
      READ(IUNIT4,1,ERR=100,END=105,IOSTAT=IOS) INDNAME,INDREG,ITAG,ISTR
      irecd=irecd+1
...
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
!
      SUBROUTINE ICHECK(IFLAG,NUMVAR,ICHARAR,ICHARX,INDEXX)
   use i_
      IMPLICIT NONE
      INTEGER IFLAG,NUMVAR,ILOOP,INDEXX
...
!
   ...
   use i_
      ...
      CHARACTER*200 ISTR
      IERR=0
      READ(ISTR,*,ERR=100,IOSTAT=IOSi) INDDIR,IDVAL,BASE_SHIP,dum, &
           STEMCUR

      ! trying to see if reducing the shipmentn value by a factor of 100 (was increased by factor of 100)
	  ! in the enprod file works.
	  if ((INDDIR.eq.8).or.(INDDIR.eq.11).or.(INDDIR.eq.12)) PRODX_Base(inddir,INDREG)=BASE_SHIP*100 !base year shipments (tonnes) for pulp & paper and iron & steel calculations
...
!****  
      ...
      IF(IDVAL.EQ.1) THEN
        PHDRAT=BASE_SHIP/PRODVX        !RATIO OF PHY TO DOLLAR VALUE
        PRODX=PHDRAT*PRODVX
...
! to read on year/industry at a time)
    ...
    !  if(indnum.le.numind-3) then  ! keeping this for now because only 21 industries are read in
      FNAME='EXSTCAP'
      IUNIT9=FILE_MGR('O',FNAME,NEW)
      READ(IUNIT9,'()') ! skip first line
10    continue ! return here until end of file is reached
      read(iunit9,*,end=99) iyear,adum,ind,ir,adum,cgfuel, &
        tmpcap,tmpgen,tmpelf,tmpthr,tmpfac,tmpihr,tmpgrd
!3 format(i4,',"',a15,'",',i3,',',i3,',"',a6,'",',i5,5(',',f14.0),',',f7.5,',',f9.0,f6.4)
        iyear=iyear-baseyr+1
		if (ind <= 21) then
            cogcap(ir,iyear,ind,cgfuel)=tmpcap/1000.
            coggen(ir,iyear,ind,cgfuel)=tmpgen/1000000.
            cogelf(ir,iyear,ind,cgfuel)=tmpelf/1000000.
            cogthr(ir,iyear,ind,cgfuel)=tmpthr/1000000.
            coggrd(ir,iyear,ind,cgfuel)=tmpgrd
		ELSE
		     continue
		endif
...
! OPEN FILE CONTAINING PLANNED CAPACITY VALUES
   ...
   nadds=0
20 continue
      read(iunit11,*,end=98) iyear,adum,ind,ir,adum,cgfuel, &
        tmpcap,tmpgen,tmpelf,tmpthr,tmpfac,tmpihr,tmpgrd
      nadds=nadds+1
      iadd=nadds
	  if (ind <= 21) then
...
!   adding effects of planned additions to last years stock.
...
!========================================================================================
  subroutine addupcogs
     use i_
  implicit none
  integer incap  !switch to use ibyr weighted average or ibyr+1 weighted average
...
! total fuel
   if ((inddir.ne.8).or.(inddir.ne.12)) then    ! these are already done in ppis_bsc for industries and year
	do ir=1,9
		do i=1,numind ! -3    ! need to fix eventually as we disaggregate old industry 21 into 21-24 KPE
...
! total industries
  do ifuel=1,numflchp+1
    do ir=1,11
      coggen(ir,curiyr,numind+1,ifuel)=sum(coggen(ir,curiyr,1:numind,ifuel))  ! -3 omitted for all
...
! assign ftab global reporting variables (national).  collapse fuels from 11 to 7 using imatch
  do i=1,numind  ! KPE:: revised.  Setting everything to 0 for all industries--dictionary updated
...
! average the grid share across regions and fuels for each industry
  do i=1,numind  ! keeping this in because we don't have proper dimensioning of this variable yet.
...
! write summary of 860 and nonCHP boiler fuel
    ...
    do inddir=1,numind+1 ! -3 omitted
      do indreg=1,5
        write(6,'(2i3,10f9.2)') inddir,indreg, &
...
! Copy the data to the industry/region buffer
      ...
      COPYBUF(INDREG,INDDIR).BIOSTEAMLAG   = BIOSTEAMLAG
	  ...
	  COPYBUF(INDREG,INDDIR).BYPADIBYR  = BYPADIBYR 
...
!
 ...
 ! Copy the data from the industry/region buffer to "local" variables
      ...
      BIOSTEAMLAG      = COPYBUF(INDREG,INDDIR).BIOSTEAMLAG
	  ...
	  BYPADIBYR        = COPYBUF(INDREG,INDDIR).BYPADIBYR 
...
!****
          if ((inddir.eq.8).and.(curcalyr.ge.ibyr))  then
...
!****
      if(version.eq.'tech') then
        if(inddir.eq.11.and.curcalyr.ge.ibyr) then
          call cement_industry
        endif
        if(inddir.eq.13.and.curcalyr.ge.ibyr) then
          call aluminum_industry
        endif
        if(inddir.eq.10.and.curcalyr.ge.ibyr) then
          call glass_industry
        endif
		 if(inddir.eq.12.and.curcalyr.ge.ibyr) then
...
!****
      ...
      IF(INDREG.EQ.4.AND.CURCALYR.EQ.ibyr) THEN
		...
		! carbshrcadata(i) corresponds to covered emissions in California's ARB reporting mapped to IDM industry i
        CARBSHRCA(1)= carbshrcadata(1)*(12./44.)*1.0E-6/INDCARBREG4(AGCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(2)= carbshrcadata(2)*(12./44.)*1.0E-6/INDCARBREG4(AGCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(3)= carbshrcadata(3)*(12./44.)*1.0E-6/INDCARBREG4(MINECON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(4)= carbshrcadata(4)*(12./44.)*1.0E-6/INDCARBREG4(MINECON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(5)= carbshrcadata(5)*(12./44.)*1.0E-6/INDCARBREG4(MINECON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(6)= carbshrcadata(6)*(12./44.)*1.0E-6/INDCARBREG4(CONSTCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(7)= carbshrcadata(7)*(12./44.)*1.0E-6/INDCARBREG4(FOODCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(8)= carbshrcadata(8)*(12./44.)*1.0E-6/INDCARBREG4(PAPERCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(9)= carbshrcadata(9)*(12./44.)*1.0E-6/INDCARBREG4(CHEMCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(10)= carbshrcadata(10)*(12./44.)*1.0E-6/INDCARBREG4(GLASSCON(1,1,curiyr+1),curiyr+1)
		! Need to add CO2 clink and CO2 lime because California ARB reporting contains process emissions
        CARBSHRCA(11)= carbshrcadata(11)*(12./44.)*1.0E-6/((INDCARBREG4(CEMENTCON(1,1,curiyr+1),curiyr+1))+ (co2_clink(curiyr+1,4) + co2_lime(curiyr+1,4))/1.0E6)   !  co2_clink is in C units.
        CARBSHRCA(12)= carbshrcadata(12)*(12./44.)*1.0E-6/INDCARBREG4(STEELCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(13)= carbshrcadata(13)*(12./44.)*1.0E-6/INDCARBREG4(ALUMCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(14)= carbshrcadata(14)*(12./44.)*1.0E-6/INDCARBREG4(FABMETALCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(15)= carbshrcadata(15)*(12./44.)*1.0E-6/INDCARBREG4(MACHINECON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(16)= carbshrcadata(16)*(12./44.)*1.0E-6/INDCARBREG4(COMPUTECON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(17)= carbshrcadata(17)*(12./44.)*1.0E-6/INDCARBREG4(TRANEQUIPCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(18)= carbshrcadata(18)*(12./44.)*1.0E-6/INDCARBREG4(ELECEQUIPON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(19)= carbshrcadata(19)*(12./44.)*1.0E-6/INDCARBREG4(WOODPRODCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(20)= carbshrcadata(20)*(12./44.)*1.0E-6/INDCARBREG4(PLASTICCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(21)= carbshrcadata(21)*(12./44.)*1.0E-6/INDCARBREG4(LTCHEMCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(22)= carbshrcadata(22)*(12./44.)*1.0E-6/INDCARBREG4(OTHRNMMCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(23)= carbshrcadata(23)*(12./44.)*1.0E-6/INDCARBREG4(OTHRPRIMCON(1,1,curiyr+1),curiyr+1)
        CARBSHRCA(24)= carbshrcadata(24)*(12./44.)*1.0E-6/INDCARBREG4(MISCFINCON(1,1,curiyr+1),curiyr+1)
      ...
      IF(AB32SW.EQ.1.AND.INDREG.EQ.4.AND.CURCALYR.GE.ibyr) then
      SELECT CASE (INDDIR)
        Case (1:2)
         ab_covd_em_AG = 0.
        ...
        Case(21)
         ab_covd_em_LTCHEM = 0.
		 ab_covd_em_LTCHEM = ab_indcarbCA(LTCHEMCON(1,1,CURIYR),CURIYR,CARBSHR_AB(INDDIR,INDREG))
        Case(22)
         ab_covd_em_OTHRNMM = 0.
		 ab_covd_em_OTHRNMM = ab_indcarbCA(OTHRNMMCON(1,1,CURIYR),CURIYR,CARBSHR_AB(INDDIR,INDREG))
        Case(23)
         ab_covd_em_OTHRPRIM = 0.
		 ab_covd_em_OTHRPRIM = ab_indcarbCA(OTHRPRIMCON(1,1,CURIYR),CURIYR,CARBSHR_AB(INDDIR,INDREG))
        Case(24)
         ab_covd_em_MISCFIN = 0.
		 ab_covd_em_MISCFIN = ab_indcarbCA(MISCFINCON(1,1,CURIYR),CURIYR,CARBSHR_AB(INDDIR,INDREG))
        end select
...
!***** CA carbon emissions of covered industies, units Million metic tCeq
        ...
        ab_covd_em_ind(CURIYR)=ab_covd_em_AG+ab_covd_em_MINE+ab_covd_em_CONST+      &
                               ...
                               ab_covd_em_WOODPRD+ab_covd_em_PLASTIC+ 				 &
							   ab_covd_em_LTCHEM+ab_covd_em_OTHRNMM+                 &
							   ab_covd_em_OTHRPRIM+ab_covd_em_MISCFIN
        ...
        IF (isnan(ab_covd_em_ind(CURIYR))) THEN
           WRITE(6,2037) CURIRUN, CURIYR+1989, CURITR, ab_covd_em_ind(CURIYR),          &
                               ...
                               ab_covd_em_COMP, ab_covd_em_TRANEQ, ab_covd_em_ELECEQ,   &
                               ab_covd_em_WOODPRD, ab_covd_em_PLASTIC,                  &   
							   ab_covd_em_LTCHEM, ab_covd_em_OTHRNMM, ab_covd_em_OTHRPRIM, ab_covd_em_MISCFIN
 2037      FORMAT(1X,"ab_covd_em_ind_is_NAN",3(":",I4),22(":",F12.6))
		END IF

 !  ***** CA process industrial process emissions; zeroed out 2/24 
...
! emission factors only defined from 95 on, so use 95 for prior years
      ...
      emis(17)=enqlgpf(iyy)  * C_CO2_FACTOR        ! HGL feedstock
      emis(18)=0.          * C_CO2_FACTOR          ! renewables, all sequestered
      emis(19)=0.                                  ! H2 feedstock set as 0 for now, but will have to be calculated      NSK H2
      emis(20)=0.                                  ! H2 set as 0 for now, but will have to be calculated                NSK H2
! multiply fuel use in trill btu times emission factor.
! and divide by 1000
      Indcarbreg4=0.
      do i =2,NUM_CON_FUELS
        indcarbreg4=indcarbreg4+x_con(i,4)*emis(i)/1000.
      enddo
!      i=1
!!! indirect CO2 from electricity generation is excluded:
!!!      emis(1)=sum(em_elec(9,8:9,icy))/sum(qelas(8:9,iy))       ! elec note: qelas in quads here (Western region)
!!!      indcarbreg4=indcarbreg4+x_con(i,4)*emis(i)/1000.
      return
      END
...
!  adjusting the units (dollars or tonnes) based in industry option IDVAL.
!  For PHYSICAL UNITS (tonnes) (IDVAL=1), convert units using
...
! Note: TPCPrat is the result of a function that returns the ratio of fuel prices relative to the base year	
    ...
    IF (curcalyr.eq.ibyr) REI_Adj(1:3,ifuel,istep) = 1.0           ! Initialize REI in base year
...
! biomass UEC will go up slightly ***
      DO IS=1,MPASTP
        do ifuel=1,ifbyp(is)
...
!  sensitive tpc effect, if any.  Here, it is more byproduct with higher prices
!****
...
!****
      ...
      DO 70 IFuel=1,IFBYP(IS)
        ...
        BYPQTY(4,IFuel,IS)=sum(BYPQTY(1:3,IFuel,IS))
		
   788 	format('prodcur quantities for fuel, step, and industry',',',3(F12.2,','),',',4(I6,','))
   789 	format('bypint quantities for fuel, step, and industry',',',3(F12.2,','),',',4(I6,','))
   790 	format('Byproduct quantities for fuel, step, and industry',',',4(F12.2,','),',',4(I6,','))
...
!****
	...
	! *** the bsc, except in minor cases where none is produced in the pa ***
			  IFY=IFX-40
			  BYPBSCR(IFY)=BYPBSCR(IFY)+BYPQTY(4,IFuel,IS)    !  4 in the first subscript is the sum of 1:3. not an error
	!			endif
	!          ENDIF
			ENDIF
...
!  997 FORMAT('Renewable byproduct fuels check:  IFX, IFY, ifuel, is, bypqty, BYPADIBYR, wood byp', &
!              4I8, 1X, 3F12.4)
...
!****
    DO 61 IS=1,MPASTP
		DO 61 IFUEL=1,IFMAX(IS)
			...
			IF ((inddir.eq.9).and.(is.eq.6).and.(ifuel.eq.1)) THEN	! H2 feedstock for inorganics is based on "other inorganics" shipments; index 1 is H2 feedstock, because no electricity feedstock
				ENPQTY(1:2,IFUEL,IS)=0.0	! because we don't have a PRODCUR specific to other inorganics (we have a macro series, but would need to add steps to itech for PRODCUR), and because the feedstock UECs are flat, don't do vintaging for H2 feedstock
				ENPQTY(3,IFUEL,IS)=chemship(7,indreg,curcalyr)*ENPINT(3,IFUEL,IS)	! other inorganics (total inorganics minus industrial gases)
				ENPQTY(4,IFUEL,IS)=sum(ENPQTY(1:3,IFUEL,IS))
			ENDIF
			IF((inddir.eq.9).and.(is.eq.24).and.(ifuel.eq.1))THEN	! H2 feedstock for agricultural chemicals
				IF(curcalyr.le.(STEOLastYr-2))THEN
					ENPQTY(1:2,IFUEL,IS)=0.0	! feedstock UECs are flat, don't do vintaging for H2 feedstock
					ENPQTY(3,IFUEL,IS)=fertilizer_H2(curiyr,indreg)	! ag chemicals
					ENPQTY(4,IFUEL,IS)=sum(ENPQTY(1:3,IFUEL,IS))
				ENDIF
			ENDIF
	61 CONTINUE

    if(versx.eq.'tech') then
...
! IDVAL for ind 11 is 1 so units are MMBTU and get converted later
    ...
    IF(inddir.eq.11.and.curcalyr.ge.IBYR) then
       ...
       ! Steps, third index, are 1=finish grind, 2=kilns, 3=raw grind, 4=lime, 5=imports (not done here)
         ENPQTY(:,:,1:4)=0.0
          ! fuel 1 Elect
		  ! Note that here is where we add the cement kiln CCS electricity
		  ! Here add the CCS fuel consumption (electricityconsumedCCS) to the kiln step (step #2)
          ! note that electricityconsumedCCS is computed in the cementCCS subroutine in trills but
		  ! since ENPQTY is divided by 1000000.0 later we need to put it in MMBtu

            if (curcalyr.gt.STEOLASTYR) then ! this conditional ensures that the extra electricity and natural gas
			                                   ! used for cement CCS gets incorporated into the cement fuel use without
											   ! getting "reabsorbed" into the benchmark factor (hence the "gt.STEOLASTYR")
               ...
               ENPQTY(4,1,2)=IND_FLOW_SHARE(inddir,2,indreg)*(ElecCons(2,curiyr) + & 
			      sum(electricityconsumedCCS(1:4,curiyr)) * 1000000.0)
          ...
          ! fuel 2 natural gas
		  ! Note that here is where we add the cement kiln CCS natural gas
		  ! Here add the CCS fuel consumption (naturalgasconsumedCCS) to the kiln step (step #2)
          ! note that naturalgasconsumedCCS is computed in the cementCCS subroutine in trills but
		  ! since ENPQTY is divided by 1000000.0 later we need to put it in MMBtu

               ENPQTY(4,2,2)=IND_FLOW_SHARE(inddir,2,indreg)*(NgCons(3,curiyr)*1000.0 + &
			      sum(naturalgasconsumedCCS(1:4,curiyr)) * 1000000.0)
               ENPQTY(4,2,3)=IND_FLOW_SHARE(inddir,3,indreg)*NGCons(4,curiyr)

               ENPQTY(4,2,4)=IND_FLOW_SHARE(inddir,2,indreg)*NGCons(5,curiyr) 

!		       write (5525,*)'check for consumption'
!			   write (5525,*) 'curiyr curcalyr indreg = ', curiyr, curcalyr, indreg
!			   write (5525,*) 'ElecCons & electricityconsumedCCS ', ElecCons(2,curiyr), sum(electricityconsumedCCS(1:4,curiyr))
!			   write (5525,*) 'IND_FLOW_SHARE(inddir,2,indreg) = ', IND_FLOW_SHARE(inddir,2,indreg)
!              write (5525,*) 'ENPQTY(4,1,2) = ', ENPQTY(4,1,2)
!			   write (5525,*) 'ENPQTY(4,2,2) = ', ENPQTY(4,2,2)
!			   write (5525,*) 'NgCons & naturalgasconsumedCCS ', NgCons(2,curiyr), sum(naturalgasconsumedCCS(1:4,curiyr))
!			   write (5525,*) 'electricityconsumedCCS(indreg,curiyr) ', sum(electricityconsumedCCS(1:4,curiyr))
!			   write (5525,*) 'electricityconsumedCCS(indreg,curiyr-1) ', sum(electricityconsumedCCS(1:4,curiyr-1))
!			   write (5525,*) 'naturalgasconsumedCCS(indreg,curiyr) ', sum(naturalgasconsumedCCS(1:4,curiyr))
!			   write (5525,*) 'naturalgasconsumedCCS(indreg,curiyr-1) ', sum(naturalgasconsumedCCS(1:4,curiyr-1))
!			   write (5525,*) 'time for consumption'
            else
               ENPQTY(4,1,1)=IND_FLOW_SHARE(inddir,1,indreg)*ElecCons(1,curiyr)
               ENPQTY(4,1,2)=IND_FLOW_SHARE(inddir,2,indreg)*ElecCons(2,curiyr)
               ENPQTY(4,1,3)=IND_FLOW_SHARE(inddir,3,indreg)*ElecCons(4,curiyr)
...
!            write (125,*) 'ENPQTY'
!			write (125,*) ENPQTY(4,1,1), ENPQTY(4,1,2), ENPQTY(4,1,3), ENPQTY(4,1,4)
!			write (125,*) ENPQTY(4,2,2), ENPQTY(4,2,3), ENPQTY(4,2,4)
!			write (125,*) 'check for consumption'
!            
            write (5525,*) 'ENPQTY'
			write (5525,*) ENPQTY(4,1,1), ENPQTY(4,1,2), ENPQTY(4,1,3), ENPQTY(4,1,4)
			write (5525,*) ENPQTY(4,2,2), ENPQTY(4,2,3), ENPQTY(4,2,4)
			write (5525,*) 'check for consumption'
          ...
          ! fuel 4 Coking Coal
            ENPQTY(4,4,2)=IND_FLOW_SHARE(inddir,2,indreg)*CoalMCons(3,curiyr) *1000.0
          ...
          ! fuel 6 Distl
	        ...
	        ENPQTY(4,6,4)=IND_FLOW_SHARE(inddir,2,indreg)*(HFOCons(5,curiyr))			! lime 
...
!              7: Coke
    ...
    IF(inddir.eq.12.and.curcalyr.ge.ibyr) then
	    ENPQTY(:,:,1:8)=0.0
		do is=1,5
            ENPQTY(4,1,is)=ElecCons(is,curiyr)*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0		! electricity
            ENPQTY(4,2,is)=NgCons(is,curiyr)*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0			! natural gas
            IF (is.le.3) THEN	! steps 1—3
                ENPQTY(4,3,is)=0.0																! resid
                ENPQTY(4,4,is)=0.0 																! other petroleum
			ELSEIF(is.eq.4) THEN ! step 4 has different fuel indices (boooo)
                ENPQTY(4,3,is)=CoalSCons(is,curiyr)*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0  ! steam coal
                ENPQTY(4,4,is)=0.0																! resid
				ENPQTY(4,5,is)=0.0 																! other petroleum
            ENDIF
		enddo
		 
		is=6	! combine steps 6 and 7 into step 6 since DRI is split into 2 steps in process calcs
        ENPQTY(4,1,is)=(ElecCons(6,curiyr)+ElecCons(7,curiyr))*IND_FLOW_SHARE(inddir,6,indreg)*1000000.0    	! electricity
		ENPQTY(4,2,is)=(H2FeedCons(6,curiyr)+H2FeedCons(7,curiyr))*IND_FLOW_SHARE(inddir,6,indreg) *1000000.0	! H2 feedstock	NSK H2		 
		...
		ENPQTY(4,1,is-1)=ElecCons(is,curiyr)*IND_FLOW_SHARE(inddir,is-1,indreg)*1000000.0      ! electricity
        ENPQTY(4,2,is-1)=NgCons(is,curiyr)*IND_FLOW_SHARE(inddir,is-1,indreg)*1000000.0        ! natural gas
        ENPQTY(4,3,is-1)=CoalSCons(is,curiyr)*IND_FLOW_SHARE(inddir,is-1,indreg) *1000000.0    ! steam coal
        ENPQTY(4,4,is-1)=CoalMCons(is,curiyr)*IND_FLOW_SHARE(inddir,is-1,indreg)*1000000.0     ! met coal
		ENPQTY(4,5,is-1)=0.0																	! resid
		ENPQTY(4,6,is-1)=0.0																	! other petroleum
		
    ENDIF !end iron and steel

! Pulp and Paper Model
		 ...
		 steam_share(7) = 1.0 ! paper_steam_grs(1,curiyr)/sum(paper_steam_grs(1:26,curiyr)) ! kpe wood
		   ...
		   ! steps 24-26 are the only steps with nonelectric conventional fuels
            ...
            ENPQTY(4,8,is)=(pp_hfoshr(4)*((1. - kraft_shr)*SUM(HFOCons(24:26,curiyr)) + &
			          steam_share(is)*(PP_Steam_Boiler(3)+PP_Steam_Cogen(3))))*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0 ! other petroleum
            ENPQTY(4,5,is)=(pp_hfoshr(2)*((1. - kraft_shr)*SUM(HFOCons(24:26,curiyr)) + &
			          steam_share(is)*(PP_Steam_Boiler(3)+PP_Steam_Cogen(3))))*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0 ! petcoke
            ENPQTY(4,4,is)=(pp_hfoshr(3)*((1. - kraft_shr)*SUM(HFOCons(24:26,curiyr)) + &
			          steam_share(is)*(PP_Steam_Boiler(3)+PP_Steam_Cogen(3))))*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0 ! distillate
	        ENPQTY(4,6,is)=((1. - kraft_shr)*SUM(CoalSCons(24:26,curiyr)) + &
			          steam_share(is)*(PP_Steam_Boiler(4)+PP_STEAM_Cogen(4)))*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0 ! steam coal
			...
			ENPQTY(4,8,is)=(pp_hfoshr(4)*(kraft_shr*SUM(HFOCons(24:26,curiyr)) + &
			          steam_share(is)*(PP_Steam_Boiler(3)+PP_Steam_Cogen(3))))*IND_FLOW_SHARE(inddir,is,indreg)*1000000.0 ! other petroluem
			...
			! NOTE:  ENPRQTY should be in trills-- the conversx is not done on the renewable variable
      endif
    endif ! ' of   if (versx .eq. 'tech')
    ...
    DO 80 IS=1,MPASTP
		DO 80 IV=1,4
			...
			DO 80 IFUEL=1,IFMAX(IS)
				ENPQTY(IV,18,IS)=ENPQTY(IV,18,IS)+ENPQTY(IV,IFUEL,IS)
 80 CONTINUE
...
!paper industry -- primary fuel, steam, and biomass
	    ...
	    DO IS=1,5
				ENPMQTY(1)=ENPMQTY(1)+ENPQTY(4,1,IS)       ! electricity
				ENPMQTY(3)=ENPMQTY(3)+ENPQTY(4,2,IS)       ! natural gas
            IF (is.le.3) THEN ! steps 1—3
				ENPMQTY(10)=ENPMQTY(10)+ENPQTY(4,3,IS)  ! resid (0)
				ENPMQTY(22)=ENPMQTY(22)+ENPQTY(4,4,IS)  ! other petroleum (0)
			ELSEIF (is.eq.4) THEN 	! step 4 (BOF) has different fuel indices (boooo)			 
                ENPMQTY(7)=ENPMQTY(7)+ENPQTY(4,3,IS)    ! steam coal
                ENPMQTY(10)=ENPMQTY(10)+ENPQTY(4,4,IS)  ! resid (0)
                ENPMQTY(22)=ENPMQTY(22)+ENPQTY(4,5,IS)  ! other petroleum (0)	 
			ENDIF
		ENDDO
		
		IS=6	! step 6 (DRI)
			ENPMQTY(1)=ENPMQTY(1)+ENPQTY(4,1,IS)		! electricity
			ENPMQTY(2)=ENPMQTY(2)+ENPQTY(4,2,IS)		! H2 feedstock		NSK H2			
			ENPMQTY(3)=ENPMQTY(3)+ENPQTY(4,3,IS)		! natural gas
            ENPMQTY(7)=ENPMQTY(7)+ENPQTY(4,4,IS)		! steam coal
		    ENPMQTY(8)=ENPMQTY(8)+ENPQTY(4,5,IS)		! met coal
            ENPMQTY(10)=ENPMQTY(10)+ENPQTY(4,6,IS)		! resid (0)
            ENPMQTY(22)=ENPMQTY(22)+ENPQTY(4,7,IS)		! other petroleum (0)
		
		IS=7	! step 7 (coking)
			ENPMQTY(1)=ENPMQTY(1)+ENPQTY(4,1,IS)		! electricity
			ENPMQTY(3)=ENPMQTY(3)+ENPQTY(4,2,IS)		! natural gas
            ENPMQTY(7)=ENPMQTY(7)+ENPQTY(4,3,IS)		! steam coal
		    ENPMQTY(8)=ENPMQTY(8)+ENPQTY(4,4,IS)		! met coal
            ENPMQTY(10)=ENPMQTY(10)+ENPQTY(4,5,IS)		! resid (0)
            ENPMQTY(22)=ENPMQTY(22)+ENPQTY(4,6,IS)		! other petroleum (0)
	endif
...
!    Initialized from exstcap.txt, average of 2017-2021/US(11)/"all fl"(11)
      data coggrdnew/ & 
      0.2038, & ! agcrop
      0.8188, & ! agother
      0.0000, & ! coal min
      ...
      0.0181, & ! plastics
      0.1516, & ! light chemicals
      0.0000, & ! other non-metallic minerals
      0.0000, & ! other primary metals
      0.0000/   ! miscellaneous finished 
...
! KPE:
! THIS SUBROUTINE HAS BEEN CHANGED TO USE BSCIBYR
! BOILER AMOUNTS AND MAKE THE PRICE A LITTLE more
! STRAIGHTFORWARD. THERE ARE 3 RENORMALIZATIONS AND 
! ONLY PETROLEUM USES THE BSCIBYR TO DISAGGREGATE TOTAL PETROLEUM
...
! grid/own-use shares at the industry level
    do I=L,M ! census divisions in current indreg
        do ifuel=1,numflchp
          ...
          fuelcogsteam(ifuel)=fuelcogsteam(ifuel)+cogthr(I,YR,INDDIR,IFUEL)/chp_eff ! CHP beff 80%
          ...
          fuel(3)=cogelf(I,YR,INDDIR,IFUEL)+cogthr(I,YR,INDDIR,IFUEL)/chp_eff
...
!  need to be generated by non-cogeneration boilers
      ...
      cogsteam=sum(fuelcogsteam(1:numflchp))*chp_eff  ! CHP beff 80%
...
! getting rid of boiler fuel for bulk CHEMICALS
! putting it in food instead
! this works because the industry for food (7) is 
...
! kpe revision (issue 1369, October 2024):  
! Trying a new approach: take read-in value of boilers for 2018, Subtract fuelsys(7)
! to account for renewable CHP (part of the legacy code). Convert to BIOSTEAM
! by using efficiency. For subsequent years, grow biosteam by PRODuction
! growth. For fuel consumption, convert back dividing by efficiency. 
! This calculation is at the end.
!
...
!  is derived initially from exogenous Form 860b cogeneration estimates.
	...
	!  if (noncogsteam.eq.0) biosteam=0.
	   if (inddir.ge.7) biosteam = biosteam ! kpe see what happens when this is commented out
      noncogfossteam = max(0.,noncogsteam-biosteam)     
      if(prtdbgi.eq.3)write(6,'(a,i4,i3,a,i1,7f10.2)') 'steam ',iyr,inddir,'r', &
       indreg,stemcur,cogsteam, biosteam, biosteamlag, prodvx, prodvxlag, noncogfossteam
...
!  ADD building energy by fuel
 ...
 2334    FORMAT(1X,"ENBQTY_5_4_IS_NAN",5(":",I4),2(":",F21.6))
      ...
      QTYMAIN(12,INDREG)=QTYMAIN(12,INDREG)+ENBQTY(5,5)  ! propane
...
!  groups: Primary=food,paper,chemicals,steel,aluminum,refining
!          Shift  =metal based durables;
!          Miscellaneous =all the rest.
!		   H2 electrolyzers =electricity from H2 electrolysis in HMM
    IF (inddir.eq.1) xelin(1,indreg)=REFCON(1,indreg,curiyr)
	IF (inddir.eq.7.or.inddir.eq.8.or.inddir.eq.9.or. &
          inddir.eq.12.or.inddir.eq.13) THEN
        xelin(1,indreg)=xelin(1,indreg) + qtymain(1,indreg)
    ELSEIF (inddir.ge.14.and.inddir.le.18) THEN
        xelin(2,indreg)=xelin(2,indreg) + qtymain(1,indreg)
    ELSE
...
!  SUM UP TOTALS OVER FUELS.
      ...
      DO 100 IR=1,5
        QTYMAIN(mainfuels,IR)=0.0
        DO 100 IFUEL=1,mainfuels-1
          QTYMAIN(mainfuels,IR)=QTYMAIN(mainfuels,IR)+QTYMAIN(IFUEL,IR)
  100 CONTINUE
      ...
      DO 150 IR=1,5
        DO IFUEL=1,mainfuels
          ...
          IF (IFUEL.EQ.18 .AND. IR.EQ.5) THEN                              ! Overwrites value for IFUEL = 18 (Lubes & Waxes) at national level
            LUBEIDX = 100.* ( MC_REVIND(11, 1,curiyr) &
                            + MC_REVIND(11,15,curiyr) &
                            + MC_REVIND(11,33,curiyr) &
                            + MC_REVIND(11,40,curiyr) ) &
                          / ( MC_REVIND(11, 1,13) &
                            + MC_REVIND(11,15,13) &
                            + MC_REVIND(11,33,13) &
                            + MC_REVIND(11,40,13) )             ! Production index for lube exposed industries, where 2002=100
...
!  WRITE OVERALL CONSUMPTION TO DEBUG FILE.
      ...
      IF(IOPEN.EQ.1.AND.LSTITER.EQ.1.and.IWDBG.eq.1) &
       WRITE(IUNIT1,900) IYR,QTYMAIN(mainfuels,INDREG), &
...
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   ...
   use i_
      ...
      IY = IYR - 1989
!  Initialize if this is the first time through (INDNUM will be 1) indicies on the con variables are fuels, census region, and year
      IF (INDNUM.EQ.1) THEN
         qtybyind(:,:)=0.

         MANHP(1:NUM_CON_FUELS,icr,IY)       =0.0
         NONHP(1:NUM_CON_FUELS,icr,IY)       =0.0
         REFCON(1:NUM_CON_FUELS,icr,IY)      =0.0
		 H2CON(1:NUM_CON_FUELS,icr,IY)		 =0.0
         FOODCON(1:NUM_CON_FUELS,icr,IY)     =0.0
         PAPERCON(1:NUM_CON_FUELS,icr,IY)    =0.0
         CHEMCON(1:NUM_CON_FUELS,icr,IY)     =0.0
         GLASSCON(1:NUM_CON_FUELS,icr,IY)    =0.0
         STEELCON(1:NUM_CON_FUELS,icr,IY)    =0.0
         CEMENTCON(1:NUM_CON_FUELS,icr,IY)   =0.0
         ALUMCON(1:NUM_CON_FUELS,icr,IY)     =0.0
         AGCON(1:NUM_CON_FUELS,icr,IY)       =0.0
         MINECON(1:NUM_CON_FUELS,icr,IY)     =0.0
         CONSTCON(1:NUM_CON_FUELS,icr,IY)    =0.0
         FABMETALCON(1:NUM_CON_FUELS,icr,IY) =0.0
         MACHINECON(1:NUM_CON_FUELS,icr,IY)  =0.0
         COMPUTECON(1:NUM_CON_FUELS,icr,IY)  =0.0
         TRANEQUIPCON(1:NUM_CON_FUELS,icr,IY)=0.0
         ELECEQUIPON(1:NUM_CON_FUELS,icr,IY) =0.0
         WOODPRODCON(1:NUM_CON_FUELS,icr,IY) =0.0
         PLASTICCON(1:NUM_CON_FUELS,icr,IY)  =0.0 
         LTCHEMCON(1:NUM_CON_FUELS,icr,IY)	 =0.0
         OTHRNMMCON(1:NUM_CON_FUELS,icr,IY)  =0.0
         OTHRPRIMCON(1:NUM_CON_FUELS,icr,IY) =0.0
	     MISCFINCON(1:NUM_CON_FUELS,icr,IY)  =0.0
...
!   REFCON used for Table 35 in Ftab.  reassigned in main.f after PMM runs
		...
		SUM(QNGeth(IY,DIVS(icr,1):DIVS(icr,2))) ! nat gas h&p
...
!	Add refinery consumption to MANUFACTURING Heat and Power
        DO IF=1,NUM_CON_FUELS
			MANHP(IF,icr,IY)=REFCON(IF,icr,IY)
        ENDDO

!	Add HMM consumption to H2CON, but do NOT add H2CON to MANHP, as that would result in double-counting
		H2CON(1,icr,iy)= SUM(QELHM(DIVS(icr,1):DIVS(icr,2),IY))
		H2CON(2,icr,iy)= SUM(QNGHMHP(DIVS(icr,1):DIVS(icr,2),IY))
		H2CON(16,icr,iy)= SUM(QNGHMPF(DIVS(icr,1):DIVS(icr,2),IY))
...
!  Fill each of the other Industry consumption tables.
      SELECT CASE (INDDIR)
        ...
        Case (21)
          call indfillcon(LTCHEMCON(1,icr,IY)   ,QTYMAIN(1,icr),QTYRENW(1,icr))
        Case (22)
          call indfillcon(OTHRNMMCON(1,icr,IY)   ,QTYMAIN(1,icr),QTYRENW(1,icr))
        Case (23)
          call indfillcon(OTHRPRIMCON(1,icr,IY)   ,QTYMAIN(1,icr),QTYRENW(1,icr))
        Case (24)
          call indfillcon(MISCFINCON(1,icr,IY)   ,QTYMAIN(1,icr),QTYRENW(1,icr))
      end select
   ...
   use i_
      ...
      real con(NUM_CON_FUELS)
      real qtymain1(mainfuels)
      real qtyrenw1(9)
...
! petrol coke  11   16
! asphalt      12   17
! petrol feed  13   18,19,21 ! lubes,petrochem feedstock, oth pet feedstoc
! kero         14   20
! oth ind      15   22
! ng feed      16   5
! HGL feed     17   13
...
!      integer icon(23)/1, 19, 2, 2,16, 2, 3, 4, 5, 6, 7, 8, &
!                      17, 9,10,11,12,13,13,14,13,15,20/
      ! KPE comment---instead of using data for icon, which is hard to verify,
	  ! use select instead.  Longer, but you don't go blind reading the data
      integer icon(mainfuels-1)

      do i=1,mainfuels-1
        if (i.eq.1) then 
        !if ((i.ge.1).and.(i.le.2)) then NSK H2
		       icon(i) = 1
	    elseif ((i.eq.3).or.(i.eq.4).or.(i.eq.6)) then 
		       icon(i) = 2
	    elseif (i.eq.7) then 
		       icon(i) = 3
	    elseif (i.eq.8) then 
	    ...
	    elseif (i.eq.17) then 
		       icon(i) = 12
        elseif ((i.eq.18).or.(i.eq.19).or.(i.eq.21)) then 
		       icon(i) = 13
	    elseif (i.eq.20) then 
		       icon(i) = 14
	    elseif (i.eq.22) then 
	    ...
	    elseif (i.eq.13) then 
		       icon(i) = 17
        elseif (i.eq.2) then        ! NSK H2
               icon(i) = 19
        elseif (i.eq.23) then       ! NSK H2
               icon(i) = 20   
        else
		       !icon(i) = 18
      	endif	
...
!		 if ((curitr == 1).and.(prtdbgi.gt.1)) &  ! .and.((inddir == 8)).or.(inddir == 12)) &
...
!		        inddir, indreg, curcalyr, i, icon, con(icon(i)), qtymain1(i) 
      enddo
      do j=1,8
        con(18)=con(18)+qtyrenw1(j) ! renewables
      enddo
      return
...
! Retally the individual reporting arrays to produce Table 44 results (to include latest LFMM version of REFCON)
      MANHP(:,:,IY)= REFCON(:,:,IY) +     FOODCON(:,:,IY) +   PAPERCON(:,:,IY) +        CHEMCON(:,:,IY) + &
                ...
                ELECEQUIPON(:,:,iy) + WOODPRODCON(:,:,iy) + PLASTICCON(:,:,iy)                          + &
				LTCHEMCON(:,:,iy)	+ OTHRNMMCON(:,:,iy)  + OTHRPRIMCON(:,:,iy) + MISCFINCON(:,:,iy) 
...
! steps
      character*100 line,dumc*35, duma*20, dumd*20
...
! read step definitions 
            read(line,*) inpind,inpreg,dumc,nsteps,ngrps  ! OK
            if(inpind.eq.inddir.and.inpreg.eq.indreg) then
              ...
              NumRptGrps=ngrps
              do is=1,mpastp
31              read(iunit,'(a)') line
                if(line(1:1).eq.'*') goto 31
                read(line,*) inpind,inpreg,inpstp,duma,dumc, &
                  ntmax(inpstp), &
                  (IPASTP(inpstp,IDOWN),IDOWN=1,ntmax(inpstp))
                  if(prtdbgi.gt.1) &
                   write(6,*) 'prodflow read-in: industry/region/step, 2strings*', &
                               inpind,'*',inpreg,'*',inpstp,'*',duma,'*',dumc, &
                  ntmax(inpstp), &
                  (IPASTP(inpstp,IDOWN),IDOWN=1,ntmax(inpstp))
...
! System characteristics--8 (nsys) system sizes considered
         ...
         CALL GETRNGR('capcostyearly   ',CapCostYearly   ,endyr-2015+1,nsys,1)
...
1005  format(10x,a,I4,a,I4)
      ...
      write(O,1000) ' '
!      write(O,1005) 'For industry ', inddir, ' and region', indreg 
      write(O,1000) ' '
   ...
   use i_
      ...
      if(indreg.eq.4.and.inddir.eq.numind) then
        ...
        do ifuel=1,numflchp
          do indx=1,numind
            do ic=1,9
              sumit(ifuel)=sumit(ifuel) + coggen(ic,yr,indx,ifuel)
            enddo
          enddo
        enddo
...
! check whehter we need to redimension these variables
      REAL ENPMQTYT(mainfuels,0:8)     ! ENERGY CONSUMPTION BY FUEL
      ...
      REAL ENPQTYT(18,maxstep+1,5)! ENERGY CONSUMPTION FOR PROCESS & ASSEMBLY
      !REAL ENPQTYT(16,maxstep+1,5)! ENERGY CONSUMPTION FOR PROCESS & ASSEMBLY   NSK H2      
      REAL ENPQTYsub(18,12,maxstep+1,5) !energy summed by fuel for up to 12 step subset groups (for chemical subindustries and end-use totals)
      !REAL ENPQTYsub(16,12,maxstep+1,5) !energy summed by fuel for up to 12 step subset groups (for chemical subindustries and end-use totals)      NSK H2
...
!**********************************************************
      ...
      character*40 bldname(12)/'Lighting--Electricity',  &  ! 1 --- KPE rincreased length from 25 to 40
         'HVAC--Electricity     ','HVAC--Natural Gas     ','          HVAC--Steam', & ! 3+ 1 = 4
         'Support--Electricity  ','Support--Natural Gas  ', &  ! 4 +2 = 6
         'Transport--Electricity','Transport--Distillate','Transport--LPG', & ! 6+ 3= 9
		 'Conventional Elec Gen--Natural Gas', 'Other Nonprocess Use--Electricity', &
		 'Other Nonprocess Use--Natural Gas'/
 ...
 ! for wk1 output routine.  holds unit number
	 ...
	 ! KPE revision 6/16/23 --- took out fuels 5, 13, and 19 for all industries except bulk chem
      integer printem(14,numind)/ &
       11,  1,  3,  4, 10, 11, 12, 14, 22, 41, 42, 48,  0,  0,  &    ! ind # 1
       11,  1,  3,  4,  7, 10, 11, 12, 14, 22, 41, 42,  0,  0,  &    ! ind # 2
        7,  1,  3,  7, 10, 11, 14, 22,  0,  0,  0,  0,  0,  0,  &    ! ind # 3
        9,  1,  3,  4,  7, 10, 11, 14, 22, 42,  0,  0,  0,  0,  &    ! ind # 4
        8,  1,  3,  4,  7, 10, 11, 14, 22,  0,  0,  0,  0,  0,  &    ! ind # 5
        7,  1,  3, 11, 12, 14, 17, 22,  0,  0,  0,  0,  0,  0,  &    ! ind # 6
       11,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42, 48,  0,  0,  &    ! ind # 7
       13,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41, 42, 43, 48,  &    ! ind # 8
       13,  1,  2,  3,  4,  5,  7, 10, 11, 12, 13, 19, 22, 48,  &    ! ind # 9
        8,  1,  3,  4,  7, 10, 11, 12, 22,  0,  0,  0,  0,  0,  &    ! ind # 10
       11,  1,  3,  4,  7,  8, 10, 11, 12, 16, 22, 48,  0,  0,  &    ! ind # 11
       12,  1,  2,	3,  4,  7,  8,  9, 10, 11, 12, 22, 41,  0,  &    ! ind # 12
       10,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41,  0,  0,  0,  &    ! ind # 13
       10,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42,  0,  0,  0,  &    ! ind # 14
       10,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42,  0,  0,  0,  &    ! ind # 15
       10,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42,  0,  0,  0,  &    ! ind # 16
       10,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42,  0,  0,  0,  &    ! ind # 17
       11,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41, 42,  0,  0,  &    ! ind # 18
       11,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42, 48,  0,  0,  &    ! ind # 19
       10,  1,  3,  4,  7, 10, 11, 12, 22, 41, 42,  0,  0,  0,  &    ! ind # 20
       11,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41, 42,  0,  0,  &    ! ind # 21  we may have to modify these numbers
	   11,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41, 42,  0,  0,  &    ! ind # 22
	   11,  1,  3,  4,  7, 10, 11, 12, 16, 22, 41, 42,  0,  0,  &    ! ind # 23
	   12,  1,  3,  4,  7, 10, 11, 12, 16, 17,  22, 41, 42, 0/       ! ind # 24
...
!**********************************************************
	...
	REAL fuelIndexArray(mainfuels,3)			!Matches different fuel indices to the same fuel
	REAL benchValArray(mainfuels,24)			!Array of benchmark ratios
...
!**********************************************************
      ...
      iyr= curcalyr
!**********************************************************
!	Set up fuelIndexArray and benchValArray for 2018 overwrite
!**********************************************************

	!Column Order: QTYMAIN-CON-MECS
	do k=1, mainfuels
		do ka=1, 3
			fuelIndexArray(k,ka)=0.0
		enddo
	enddo
	!QTYMAINxINDDIR
	do k=1, mainfuels
		do ka=1, 24
			benchValArray(k,ka)=1.0
		enddo
	enddo
	!IFn
	do k=1, mainfuels
	...
	do k=1, mainfuels
		fuelIndexArray(k,1)=k
	enddo
	fuelIndexArray(1,2)=1
	fuelIndexArray(3,2)=2
	fuelIndexArray(4,2)=2
	fuelIndexArray(5,2)=16
	fuelIndexArray(6,2)=2
	fuelIndexArray(7,2)=3
	fuelIndexArray(8,2)=4
	fuelIndexArray(9,2)=5
	fuelIndexArray(10,2)=6
	fuelIndexArray(11,2)=7
	fuelIndexArray(12,2)=8
	fuelIndexArray(13,2)=17
	fuelIndexArray(14,2)=9
	fuelIndexArray(15,2)=10
	fuelIndexArray(16,2)=11
	fuelIndexArray(17,2)=12
	fuelIndexArray(18,2)=13
	fuelIndexArray(19,2)=13
	fuelIndexArray(20,2)=14
	fuelIndexArray(21,2)=13
	fuelIndexArray(22,2)=15

	fuelIndexArray(1,3)=1
	fuelIndexArray(3,3)=2
	fuelIndexArray(4,3)=2
	!fuelIndexArray(5,3)=16
	fuelIndexArray(6,3)=2
	fuelIndexArray(7,3)=4
	fuelIndexArray(8,3)=9
	!fuelIndexArray(9,3)=5
	fuelIndexArray(10,3)=3
	fuelIndexArray(11,3)=5
	fuelIndexArray(12,3)=6
	!fuelIndexArray(13,3)=17
	!fuelIndexArray(14,3)=9
	!fuelIndexArray(15,3)=10
	fuelIndexArray(16,3)=7
	!fuelIndexArray(17,3)=12
	!fuelIndexArray(18,3)=13
	!fuelIndexArray(19,3)=13
	!fuelIndexArray(20,3)=14
	!fuelIndexArray(21,3)=13
	fuelIndexArray(22,3)=10
!**********************************************************
...
!  ESTABLISH TOTAL ENERGY CONSUMPTION BY MAIN FUEL.
		  ...
		  DO IFn=1,mainfuels
			 IF (iyr.eq.ibyr) THEN
				...
				SELECT CASE (INDNUM)
					Case (8)
						IF (papercon(fuelIndexArray(IFn,2),INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,fuelIndexArray(IFn,3))/papercon(fuelIndexArray(IFn,2),INDREG,IY)
					Case (10)
						IF (glasscon(fuelIndexArray(IFn,2),INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,fuelIndexArray(IFn,3))/glasscon(fuelIndexArray(IFn,2),INDREG,IY)
					Case (11)
						IF (cementcon(fuelIndexArray(IFn,2),INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,fuelIndexArray(IFn,3))/cementcon(fuelIndexArray(IFn,2),INDREG,IY)
					Case (12)
						IF (steelcon(fuelIndexArray(IFn,2),INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,fuelIndexArray(IFn,3))/steelcon(fuelIndexArray(IFn,2),INDREG,IY)
					Case (13)
						IF (alumcon(fuelIndexArray(IFn,2),INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,fuelIndexArray(IFn,3))/alumcon(fuelIndexArray(IFn,2),INDREG,IY)
				end select
				if ((INDNUM.ge.8).and.(INDNUM.le.13).and.(INDNUM.ne.9)) then
					ENPMQTYT(IFn,INDREG) = ENPMQTYT(IFn,INDREG)*benchValArray(IFn,INDNUM)

					EnConsArray(IFn) = EnConsArray(IFn)+ENPMQTYT(IFn,INDREG)
					ENPMQTYT(IFn,5) = EnConsArray(IFn)
				endif
				endif
			 ENDIF
          ENDDO
		  ...
		  DO IFn=1,9
			 IF (iyr.eq.ibyr) THEN
				SELECT CASE (INDNUM)
					...
					Case (10)
						IF (glasscon(18,INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,8)/glasscon(18,INDREG,IY)
					Case (11)
						IF (cementcon(18,INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,8)/cementcon(18,INDREG,IY)
					Case (12)
						IF (steelcon(18,INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,8)/steelcon(18,INDREG,IY)
					Case (13)
						IF (alumcon(18,INDREG,IY).ne.0.0) benchValArray(IFn,INDNUM) = MECS_Data(indnum,INDREG,8)/alumcon(18,INDREG,IY)
				end select
				if ((INDNUM.ge.8).and.(INDNUM.le.13).and.(INDNUM.ne.9)) then
					ENPRQTYT(IFn,INDREG) = ENPRQTYT(IFn,INDREG)*benchValArray(IFn,INDNUM)
					RenConsArray(IFn) = RenConsArray(IFn)+ENPRQTYT(IFn,INDREG)
					ENPRQTYT(IFn,5) = RenConsArray(IFn)
				endif
			 ENDIF
          ENDDO
		  if (INDREG.eq.4) Then
...
!  ESTABLISH BUILDING COMPONENT CONSUMPTION BY FUEL.
          ...
          BLDENCONS( 7,5) = BLDENCONS( 7,5)+ENBQTY(4,1)  ! onsite tran, el
          BLDENCONS( 8,5) = BLDENCONS( 8,5)+ENBQTY(4,4)  ! onsite tran, df
          BLDENCONS( 9,5) = BLDENCONS( 9,5)+ENBQTY(4,5)  ! onsite tran, df
          BLDENCONS(10,5) = BLDENCONS(10,5)+ENBQTY(5,2)  ! conventional electricity generation, ng
          BLDENCONS(11,5) = BLDENCONS(11,5)+ENBQTY(6,1)  ! other nonprocess, elec
          BLDENCONS(12,5) = BLDENCONS(12,5)+ENBQTY(6,2)  ! other noprocess, ng

!  new bldgncons array

          BLDENCONS( 1,INDREG) = ENBQTY(1,1)  ! lighting
          BLDENCONS( 2,INDREG) = ENBQTY(2,1)  ! hvac el
          BLDENCONS( 3,INDREG) = ENBQTY(2,2)  ! hvac ng
          BLDENCONS( 4,INDREG) = ENBQTY(2,3)  ! hvac stm
          BLDENCONS( 5,INDREG) = ENBQTY(3,1)  ! fac support, el
          BLDENCONS( 6,INDREG) = ENBQTY(3,2)  ! fac support, ng
          BLDENCONS( 7,INDREG) = ENBQTY(4,1)  ! onsite tran, el
          BLDENCONS( 8,INDREG) = ENBQTY(4,4)  ! onsite tran, df
          BLDENCONS( 9,INDREG) = ENBQTY(4,5)  ! onsite tran, lpgf
          BLDENCONS(10,INDREG) = ENBQTY(5,2)  ! conventional electricity generation, ng
          BLDENCONS(11,INDREG) = ENBQTY(6,1)  ! other nonprocess, elec
          BLDENCONS(12,INDREG) = ENBQTY(6,2)  ! other noprocess, ng

! ESTABLISH PROCESS AND ASSEMBLY (p/a) COMPONENT CONSUMPTION BY FUEL.
! CONTROLLED THROUGH ITECH AND PRODFLOW
        gfmax=0  ! count for number of fuels used in each sub group of steps
        gfloc=0  ! fuel id numbers for each sub group of steps
        istot=MPASTP+1
        ifmax(istot)=0	
		DATA STfuelmap /1,2,3,7,8,10,22/ 	!Iron and Steel industry fuels for process and assembly total from array ENPQTY (elec,H2 feedstock,ngas,steam coal,met coal,resid,other petroleum)
				...
				if(iff.ne.36)then  ! do not add in coke consumption
					if ((inddir.eq.12).and.(curcalyr.gt.ibyr2)) then
						...
						if (IFn.le.7) then
							...
							SELECT CASE (IS)	! sum up fuels across steps; summation fuel indices are the same as for step 6
								CASE(1:3)
									IF (IFn.eq.1) THEN		! electricity
										ENPQTYT(1,IStot,5)  	= ENPQTYT(1,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(1,IStot,indreg)	= ENPQTYT(1,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.2) THEN	! natural gas
										ENPQTYT(3,IStot,5)  	= ENPQTYT(3,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(3,IStot,indreg)	= ENPQTYT(3,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.3) THEN	! resid
										ENPQTYT(6,IStot,5)  	= ENPQTYT(6,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(6,IStot,indreg)	= ENPQTYT(6,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.4) THEN	! other petroleum
										ENPQTYT(7,IStot,5)  	= ENPQTYT(7,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(7,IStot,indreg)	= ENPQTYT(7,IStot,indreg)+ENPQTY(4,IFn,IS)									
									ENDIF
								CASE(4)
									IF (IFn.eq.1) THEN		! electricity
										ENPQTYT(1,IStot,5)  	= ENPQTYT(1,IStot,5)+ENPQTY(4,IFn,IS)
									...
									ELSEIF (IFn.eq.2) THEN	! natural gas
										ENPQTYT(3,IStot,5)  	= ENPQTYT(3,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(3,IStot,indreg)	= ENPQTYT(3,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.3) THEN	! steam coal
										ENPQTYT(4,IStot,5)  	= ENPQTYT(4,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(4,IStot,indreg)	= ENPQTYT(4,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.4) THEN	! resid
										ENPQTYT(6,IStot,5)  	= ENPQTYT(6,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(6,IStot,indreg)	= ENPQTYT(6,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.5) THEN	! other petroleum
										ENPQTYT(7,IStot,5)  	= ENPQTYT(7,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(7,IStot,indreg)	= ENPQTYT(7,IStot,indreg)+ENPQTY(4,IFn,IS)									
									ENDIF
								CASE(5)
								...
								CASE(6)
									IF (IFn.eq.1) THEN		! electricity
										ENPQTYT(1,IStot,5)  	= ENPQTYT(1,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(1,IStot,indreg)	= ENPQTYT(1,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.2) THEN	! H2 feedstock
										ENPQTYT(2,IStot,5)  	= ENPQTYT(2,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(2,IStot,indreg)	= ENPQTYT(2,IStot,indreg)+ENPQTY(4,IFn,IS)	
									ELSEIF (IFn.eq.3) THEN	! natural gas
										ENPQTYT(3,IStot,5)  	= ENPQTYT(3,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(3,IStot,indreg)	= ENPQTYT(3,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.4) THEN	! steam coal
										ENPQTYT(4,IStot,5)  	= ENPQTYT(4,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(4,IStot,indreg)	= ENPQTYT(4,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.5) THEN	! met coal
										ENPQTYT(5,IStot,5)  	= ENPQTYT(5,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(5,IStot,indreg)	= ENPQTYT(5,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.6) THEN	! resid
										ENPQTYT(6,IStot,5)  	= ENPQTYT(6,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(6,IStot,indreg)	= ENPQTYT(6,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.7) THEN	! other petroleum
										ENPQTYT(7,IStot,5)  	= ENPQTYT(7,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(7,IStot,indreg)	= ENPQTYT(7,IStot,indreg)+ENPQTY(4,IFn,IS)
									ENDIF										
								CASE(7)
									...
									ELSEIF (IFn.eq.3) THEN	! steam coal
										ENPQTYT(4,IStot,5)  	= ENPQTYT(4,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(4,IStot,indreg)	= ENPQTYT(4,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.4) THEN	! met coal
										ENPQTYT(5,IStot,5)  	= ENPQTYT(5,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(5,IStot,indreg)	= ENPQTYT(5,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.5) THEN	! resid
										ENPQTYT(6,IStot,5)  	= ENPQTYT(6,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(6,IStot,indreg)	= ENPQTYT(6,IStot,indreg)+ENPQTY(4,IFn,IS)
									ELSEIF (IFn.eq.6) THEN	! other petroleum
										ENPQTYT(7,IStot,5)  	= ENPQTYT(7,IStot,5)+ENPQTY(4,IFn,IS)
										ENPQTYT(7,IStot,indreg)	= ENPQTYT(7,IStot,indreg)+ENPQTY(4,IFn,IS)
									ENDIF								
								END SELECT					
...
! assign fuel by group, fuel, step.  sum across regions; create group fuel total (igf2)
        ...
        DO IS=1,MPASTP+1
            DO IFn=1,IFMAX(IS)+1
				IF(ENPQTYT(IFn,IS,indreg).ne.0.0) THEN		! NSK: this keeps causing trouble with printout csvs
					IFUELpa(is,IFn) = 1
				ENDIF
            ENDDO
        ENDDO
...
!         refin(2,1:11)=QENRF(1:11,curiyr)
          ...
          refin(5,1:11)=QGIRF(1:11,curiyr)+qNGeth(curiyr,1:11)
...
!**********************************************************
         ...
         Line =  '  Value of Output (billion 2012$)'
         if(OUTPUTACT(indreg).gt.0.) then
            Line =  '  Value of Output (billion 2012$)'
            IROW = IROW + 1
            if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
            CALL cknum(I15,IROW,icol,dble(OUTPUTACT(indreg)),2,4)
            Line = '  Output, w/Subsector Energy Weighting (billion 2012$)'
         endif
         IROW = IROW + 1
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
...
!**********************************************************
	! Disabled 'Unit Energy Consumption (MMBTU/TONNE)' functionality
		...
		IF(IDVAL.NE.1) THEN
         ...
         ELSE
           LINE =  'Unit Energy Consumption (Thousand BTU/2012$)'
         ENDIF
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
...
!**********************************************************
         ...
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
          do if=1,4
           ...
           Line = genname1(if)
           if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
           CALL cknum(I15,IROW,icol,dble(RPTCAPT(i,indreg)),0,4)
         ...
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
          do if=1,4
           ...
           Line = genname2(if)
           if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
           CALL cknum(I15,IROW,icol,dble(RPTGENT(i,indreg)),0,4)
         ...
         IROW = IROW + 2
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
         CALL cknum(I15,IROW,icol,dble(ELOWNT(indreg)),0,4)

         Line = '    Grid Sales'
         IROW = IROW + 1
         if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
         CALL cknum(I15,IROW,icol,dble(ELSALET(indreg)),0,4)
         ...
         do if=1,4
           ...
           Line = genname3(if)
           if(once(vers))CALL ckstr(I15,IROW,1,LINE,1,0,1)
           CALL cknum(I15,IROW,icol,dble(GENFUELT(i,indreg)),0,4)
...
!	Calculate replacement TPC (BCSC) for each region and fuel for Agricultural sectors
    ...
    IF(curcalyr.eq.ibyr) GOTO 6666       ! Jump over calculations in the first year to avoid dividing by zero
...
!	Calculate replacement TPC (BCSC) for each region and fuel for Coal Mining sector
	...
	IF(iyr.eq.ibyr) GOTO 6688       ! Jump over calculations in the first year to avoid dividing by zero
...
!	Calculate replacement TPC (BCSC) for each region and fuel for Oil & Gas Mining sector
	...
	IF(iyr.eq.ibyr) GOTO 6689       ! Jump over calculations in the first year to avoid dividing by zero
...
!	Calculate replacement TPC (BCSC) for each region and fuel for Other Mining sectors
	...
	IF(iyr.eq.ibyr) GOTO 6699       ! Jump over calculations in the first year to avoid dividing by zero
    ...
    IF (IFUEL.eq.5) THEN    ! prevent steam coal use because SEDS doesn't have any nonmanufacturing steam coal
        BCSC(1:3,IFUEL,ISTEP)=0.0
    ENDIF
...
! Road investment variable, from Macro Module
      write (IFEEDOUT,*)
...
!SUSAN_UPDATE
	  ...
	  character*16 FUELNAME(NUM_CON_FUELS)
! Corresponding Indices in Con and QTYMAIN array
!
!             con   qtymain
! elec          1    1
...
! H2 fuel      20   23
	  ...
	  DO IFUEL=1,NUM_CON_FUELS
		...
		WRITE(IUNITBUG,811) CURCALYR, IY, IFUEL, FUELNAME(IFUEL), "LIGHT CHEMICALS     ", LTCHEMCON(IFUEL,5,IY), LTCHEMCO2(IFUEL,IY), CO2EM(IFUEL,IY)  ! KPE NEW INDUSGTRIES 9/23/22 
		WRITE(IUNITBUG,811) CURCALYR, IY, IFUEL, FUELNAME(IFUEL), "OTHER NON METALLIC  ", OTHRNMMCON(IFUEL,5,IY), OTHRNMMCO2(IFUEL,IY), CO2EM(IFUEL,IY)  
		WRITE(IUNITBUG,811) CURCALYR, IY, IFUEL, FUELNAME(IFUEL), "OTHER PRIMARY METALS", OTHRPRIMCON(IFUEL,5,IY), OTHRPRIMCO2(IFUEL,IY), CO2EM(IFUEL,IY)  
		WRITE(IUNITBUG,811) CURCALYR, IY, IFUEL, FUELNAME(IFUEL), "MISC FINISHED GOODS ", MISCFINCON(IFUEL,5,IY), MISCFINCO2(IFUEL,IY), CO2EM(IFUEL,IY)  
    ...
    DO IND_idx=1,INDMAX
        ...
        ! electricity benchmarking; for chemicals, HMM consumption is so small we can ignore any discrepancy between ASM (with H2 SMRs) and our chemicals (without H2 SMRs)
			IF ((QTYMAIN(1,IREG).ne.0.0).and.(IND_idx.ge.7)) THEN   
                ...
                ELSEIF (curcalyr.gt.END_YR_ASM) THEN
                    hist_bench(curiyr,IND_idx,IREG,1)=hist_bench(curiyr-1,IND_idx,IREG,1)
                ELSE
                    hist_bench(curiyr,IND_idx,IREG,1)=1.0
				ENDIF
            ...
            IF ((QTYMAIN(7,IREG).ne.0.0).and.(IND_idx.ge.7)) THEN   ! steam coal benchmarking
                IF ((curcalyr.ge.ibyr).and.(curcalyr.le.END_YR_QCR)) THEN   ! Steam coal benchmarking to QCR begins in the base year
                    hist_bench(curiyr,IND_idx,IREG,7)=qcrdata(curiyr,IND_idx,5)/QTYMAIN(7,5)
                ELSEIF (curcalyr.gt.END_YR_QCR) THEN
                    hist_bench(curiyr,IND_idx,IREG,7)=hist_bench(curiyr-1,IND_idx,IREG,7)
                ELSE
                    hist_bench(curiyr,IND_idx,IREG,7)=1.0
                ENDIF
    ...
    if (curcalyr.gt.2027) then
      ...
      SUBROUTINE CO2CALC
		...
		include 'ghgrep'
          ...
          CO2EM(1,IYY)=em_indy(20,11,curcalyr)/qelin(11,iyy)
          ...
          CO2EM(6,IYY)=erlin(iyy) * CarbF             ! resid (low sulfur)
          ...
          CO2EM(17,IYY)=enqlgpf(iyy) * CarbF          ! HGL feedstock
          CO2EM(18,IYY)=0.0                           ! renewables, all sequestered
          CO2EM(19,IYY)=0.0                           ! H2 feedstock   NSK H2
          CO2EM(20,IYY)=0.0                           ! H2 fuel        NSK H2
          ! KPE:  next line is the last line written in indbug.txt in this run.
          write(IUNITBUG,'(" QEL:  ",I6,I4,5F12.3)') curcalyr,iyy, em_elec(9,11,curcalyr),qelas(11,iyy), em_indy(20,11,curcalyr), qelin(11,iyy),co2em(1,iyy)
        ...
        DO IFUEL=1,NUM_CON_FUELS
            ...
            LTCHEMCO2(IFUEL,iyy) = LTCHEMCON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0
            OTHRNMMCO2(IFUEL,iyy) = OTHRNMMCON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0
            OTHRPRIMCO2(IFUEL,iyy) = OTHRPRIMCON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0
            MISCFINCO2(IFUEL,iyy) = MISCFINCON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0
            REFCO2(IFUEL,iyy) = REFCON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0
			H2CO2(IFUEL,iyy) = H2CON(IFUEL,5,iyy) * CO2EM(IFUEL,iyy) / 1000.0			 
        ENDDO
...
! finish grinding, kiln, burner, raw grinding
       ...
       EXTERNAL RTOVALUE
        ...
        EXQ=RTOVALUE('EXQ     ',0) 
        if (EXQ.eq.1) then
          call cementCCS  ! cement retrofit CCS subroutine for CCATS
        endif
      !lime
        call TECH_STEP(5)
       return
...
! CEMENT Kiln Capacity
       ...
       do i=0,30
          new_cap_survk(i+1)=1.0/(1.0+exp(-is_calib(inddir)-(i*(-is_calib(inddir))/(is_lifetime(inddir)/2.0))))
       end do
       if (curiyr.ge.ICURIYR) then
...
! baseline capacity of Kilns
      ...
      alpha_decay(2)=IS_ALPHA_DECAY(inddir,2)
       ...
       if ((curcalyr.eq.ibyr).and.(curitr.eq.1)) then ! POT_ibyr2 2007 to ibyrset inital values for alpha_kiln
       ...
       if (curiyr.ge.ICURIYR2-1) then
        ...
        numiter=0
1667    if (curcalyr.eq.ibyr) numiter=numiter+1  ! POT_ibyr2 2007 to ibyr
		...
		do i=1,NUMROT
          if (int(obsoleteyr(inddir,stepnum,i)) .LE. curcalyr) then
			scale_comp(i) = 0.0
			  ...
			  scale_comp(i)=cm_logit_coeff_k(1)*totfixcst(i) + &           					! Fixed costs term
						   ...
						   (cm_logit_coeff_k(4) * &                                                    ! process emissions
						   emtax(curiyr) * (MC_JPGDP(20) / MC_JPGDP(-2)) * (12.011/44.01) * 1000.0 * & ! must convert emtax to
						   IS_EMISS(inddir,stepnum,i))+ &                                              ! 2009$/tonne CO2
						   cm_logit_coeff_k(5)*ht_serv(i)+&                                 ! Heat service term
						   alpha_kiln(i)*exp(-IS_ALPHA_DECAY(inddir,1)*(curcalyr-ibyr)*ln2/IS_ALPHA_DECAY(inddir,2))
      ...
      do i=1,NUMROT
           if (int(obsoleteyr(inddir,stepnum,i)) .LE. curcalyr) then
			logit_comp(i)= 0.0
				...
				logit_comp(i)=exp(cm_logit_coeff_k(1)*totfixcst(i) + &           				! Fixed costs term
						   ...
						   IS_EMISS(inddir,stepnum,i))+ &                                              ! 2009$ /tonne CO2
						   cm_logit_coeff_k(5)*ht_serv(i)+&                                     ! Heat service term
                           alpha_kiln(i)*exp(-IS_ALPHA_DECAY(inddir,1)*(curcalyr-ibyr)*ln2/IS_ALPHA_DECAY(inddir,2))+scale_fctr)  ! scale_fctr should be inside exponent
           endif	
            !sum logit_comp
         ...
         !!	Adjust alternative specific constants (ASC) to ensure initial equipment shares match historical values
            if ((curcalyr.eq.ibyr).and.(curitr.eq.1)) then
              ...
              do i=1,NUMROT
                ...
                if ((is_add_tech_share(inddir,stepnum,i).ne.0.0).and.(shares(i,curiyr).ne.0.0)) then
                    adjust_factork(i) = log(is_add_tech_share(inddir,stepnum,i)/shares(i,curiyr))
         ...
         do i=1,NUMTYP
           ...
           if (curiyr==ICURIYR) then
             ...
             basecapb(curiyr-1,i) = basecapIBYR(i)
              ...
              basecapb(curiyr,i)=basecapb(curiyr-1,i)-(basecapIBYR(i)/is_baselifecr(inddir))
...
1668    if (curcalyr.eq.IBYR) numiter=numiter+1
		...
		do i=1,NUMTYP
         if (int(obsoleteyr(inddir,stepnum,i)) .LE. curcalyr) then
		   scale_comp(i)=0.0
			  ...
			  scale_comp(i)=cm_logit_coeff_b(1)*totfixcst(i) + &                       					! fixed costs term
							...
							is_alpha(inddir,stepnum,i)*exp(-IS_ALPHA_DECAY(inddir,1)*(curcalyr-ibyr)*ln2/IS_ALPHA_DECAY(inddir,2))                                
			  ! Test initial calculations to ensure values won't go out of bounds
			   if (scale_comp(i).lt.scale_min) scale_min=scale_comp(i)
		   endif
		enddo
        ...
        do i=1,NUMTYP
          if (int(obsoleteyr(inddir,stepnum,i)) .LE. curiyr+1989) then
			logit_comp(i)=0.0
			   ...
			   logit_comp(i)=exp(cm_logit_coeff_b(1)*totfixcst(i) + &                				! fixed costs term
...
!****************
        if ((curcalyr.eq.ibyr).and.(curitr.eq.1)) then
             ...
             do i=1,NUMTYP
				if ((is_add_tech_share(inddir,stepnum,i).ne.0.0).and.(shares(i,curiyr).ne.0.0)) then
					adjust_factorb(i) = log(is_add_tech_share(inddir,stepnum,i)/shares(i,curiyr))  ! Calculate deviation from historical values; should be using base year added shares, not existing shares (NSK 7/2022)
...
!****************        
         ...
         write(IUNITBUG,*) 'tot_burner_fuel(5)=',curcalyr,tot_burner_fuel(5)
        ...
        do i=1,NUMTYPG
          if (int(obsoleteyr(inddir,stepnum,i)) .LE. curcalyr) then
			scale_comp(i)=0.0
			   ...
			   scale_comp(i)=( cm_logit_coeff_r(1)*totfixcst(i)+ &           				! Fixed cost term
					 ...
					 (exp(-IS_ALPHA_DECAY(inddir,1)*(curcalyr-IBYR)*ln2/IS_ALPHA_DECAY(inddir,2)))))
			   if (scale_comp(i).lt.scale_min) scale_min=scale_comp(i)
		  endif
        enddo
        scale_adj = 30.0
...
1669    if (curcalyr.eq.ibyr) numiter=numiter+1
        ...
        do i=1,NUMTYPG
           if (int(obsoleteyr(inddir,stepnum,i)) .LE. curiyr+1989) then
            ...
            logit_comp(i)=exp(cm_logit_coeff_r(1)*totfixcst(i) + &          				! Fixed costs term
                       cm_logit_coeff_r(2)*IS_fuel_use(inddir,stepnum,1,i)*&                ! Electricity costs term
                       (PELIN(MNUMCR,curiyr)*MC_JPGDP(20))+ &
                       ...
                       cm_logit_coeff_r(4)*IS_EMISS(inddir,stepnum,i)+ &                    ! CO2 emissions (T/kT output) term
                       (is_alpha(inddir,stepnum,i)* &
                       (exp(-IS_ALPHA_DECAY(inddir,1)*(curcalyr-ibyr)*ln2/IS_ALPHA_DECAY(inddir,2))))+scale_fctr)
			endif
            !sum logit
...
!!	Adjust alternative specific constants (ASC) to ensure initial equipment shares match historical values
            if ((curcalyr.eq.ibyr).and.(curitr.eq.1)) then
              do i=1,NUMTYPG
                if ((is_add_tech_share(inddir,stepnum,i) .ne. 0.0) .and. (shares(i,curiyr) .ne. 0.0)) then
                   adjust_factorg(i) = log(is_add_tech_share(inddir,stepnum,i)/shares(i,curiyr))  ! Calculate deviation from historical values
                else
...
! cement CCS to be tied in with CCATS (note that CCATS CO2 price variables are temporarily filled in IDM)
       subroutine cementCCS
       use i_

       IMPLICIT NONE

       integer idummy ! dummy index for regions
	   integer idumyr ! dummy index for curiyr

!!!!!!!!!!!!!!!!!!!!!!! CCATS placeholder !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! take away these declarations once IDM has been integrated with CCATS
! these variables are defined in CCATS     
!testSACCS       real CST_CMT_INV(inumreg,MNUMYR) ! capital cost CCS $/tonne (1987 dollars)
!testSACCS       real CST_CMT_OM(inumreg,MNUMYR)  ! O&M cost CCS $/tonne (1987 dollars) - includes fuel costs
!testSACCS       real SUP_CMT_45Q(inumreg,MNUMYR) ! CO2 volume captured from cement plants that are 45Q eligible (tonnes)
!testSACCS       real SUP_CMT_NTC(inumreg,MNUMYR) ! CO2 volumes from cement plants no tax credit (tonnes)
!testSACCS       real CO2_PRC_REG_45Q(inumreg,MNUMYR) ! CO2 price from CCATS (1987$/tonne) after optimization, 45Q eligible, by Census Region
!testSACCS       real CO2_PRC_REG_NTC(inumreg,MNUMYR) ! CO2 price from CCATS (1987$/tonne) after optimization, no tax credit, by Census Region


! set up baseline (2018) fuel costs: fuel costs in future years will be differentials relative to
! these 2018 baseline costs as we adjust the mean of each regional Cost of Capture from its 2018 baseline
! units are computed in 2018 $/tonne CO2
       real baselinenaturalgascost(inumreg)
	   real baselineelectricitycost(inumreg)

! variables to store current year's fuel costs used to shift mean of COC cost curve from 2018 baseline
! they are regional but also dimensioned by year because these fuel costs are used in the foresight ALGORITHM
! to help determine how much new CCS capacity to add each projection year
! units are computed in 2018 $/CO2 
       real currentnaturalgascost(inumreg,MNUMYR)
	   real currentelectricitycost(inumreg,MNUMYR)
	   real totalfuelcost(inumreg,MNUMYR) ! just a total of previous two fuel costs

! for testing and write out purposes store current year's regional natural gas and electricity prices
       real natgasprice(inumreg+1)
	   real elecprice(inumreg+1)

! store baseline (2018) total Costs which include variable and fixed cost
! note that the variable costs include fuel costs based on 2018 natural gas and electricity 
! fuel prices 
       real totalcostCCS(inumreg)

! maximum possible CO2 which can be theoretically captured with CCS retro equipment
       real maxpossibleCCS(inumreg) ! note this is not indexed by year since only old capacity can add CCS equipment
       real testtotalCCS(inumreg)   ! dummy variable to test breach of maximum CCS possible
	                                ! testtotalCCS is the accumulated installed CCS capacity in M tonnes CO2

! maximum economically viable CCS retrofit capacity
       real maxeconomicalCCS(inumreg,MNUMYR)

! actual CCS retrofit capacity installed based on maxeconomicalCCS and penetration rate 
       real economicalCCS(inumreg,MNUMYR)

! fuel shift for COC distriibution change from 2018 mean
! dimensioned also by year to accomodate foresight in CCS retrofit build decisions
       real fuelshiftCOC(inumreg,MNUMYR)

! total shift in COC based on shift due to fuel prices relative to 2018 and capital cost adder
       real totalshiftCOC(inumreg,MNUMYR)

! shifted y-intercept
! dimensioned by region AND year because we have foresight for new CCS retrofits
       real yinterceptshiftCCS(inumreg,MNUMYR)

! COC cureve paramters for jsut the curiyr which are used for actual CO2 captured AFTER
! CCS equipment has been installed.
       real fuelshiftCO2(inumreg)
	   real yinterceptshiftCO2(inumreg)
       real totalshiftCO2(inumreg)

! CCS scale factor to account for the fact that as CCS retrofit equipment is installed
! there is less available cement caapcity left to install CCS. This factor lowers the normalization
! constant each model year to reflect that the total cumulative supply of possible CCS retrofit capacity
! must diminish as more CCS retrofits are installed. 	
       real CCSretrofitscalar(inumreg,MNUMYR)
   
       real accumulate45Q(inumreg) ! accumulate CO2 from all previously installed CCS retrofits (within tax credit period)
	   real accumulateNTC(inumreg) ! accumulate CO2 from all previously installed CCS retrofits (outside tax credit period)
	   real testaccumulate        ! temporary variable for testing economics of previously installed CCS equipment's
	                              ! ability to capture CO2 in any given future year after installation
	   integer curcalyrminusone ! counter to compare with curiyr print out results on last iteration
   
       integer LastConsYrCCSint      ! integer version of ironstl.xlsx value of LastConsYrCCS
       integer CreditDurCCSint       ! integer version of ironstl.xlsx value of CreditDurCCSint
       integer CCSstartyearint       ! integer version of ironstl.xlsx value of CCSstartyear
	   
	   integer endyrCCS                 ! end year for loop which accumulated CO2 from current model year and past CCS retrofits

       real realinterestrate                  ! the real interest rate used to compute the dynamic Capital Recovery Factor (CRF)
	                                          ! based on the NEMS nominal Treasury 10-year rate MC_RMTCM10Y(curiyr)

       real caprecfactccs                            ! dynamic Capital Recovery Factor

       real totalcapturedCO2(inumreg)   ! total CO2 captured each year (for debuggin purposes only)

       real absolutetotalcost(inumreg,MNUMYR) ! for evaluation purposes: TOTAL cost (base capital + cap adder + ALL O&M)
                                              ! this mean TOTAL cost is ultimately what should be compared with the 
...
! "intergerize" variables that should be integers but are read in as real variable types from ironstl.xlsx
       ...
       CCSstartyearint = int(CCSstartyear + 0.000001)


!check if prices and 45Q parameters are stored

       if (curcalyr.eq.2033) then
          write (5525,*) 'price check '
          do idummy = 36,61	   
             write (5525,*) 1989+idummy,pngin(MNUMCR,idummy),PELIN(MNUMCR,idummy)
		  end do
          write (5525,*) 'CreditDurCCSint = ', CreditDurCCSint
          write (5525,*) 'LastConsYrCCSint = ', LastConsYrCCSint
		  write (5525,*) 'CCSstartyearint = ', CCSstartyearint
          write (5525,*) 'inumreg = ', INUMREG
		  write (5525,*) 'MNUMYR = ', MNUMYR
		  write (5525,*) 'CO2_ELEC = ', CO2_ELEC(1:4,curiyr)
  		  write (5525,*) 'CO2_ELEC = ', CO2_ELEC(1:11,curiyr)
	   end if
	
       if (curcalyr.gt.2025) then   ! check CO2 pipeline electricity use
          write (5525,*) 'curcalyr =  ', curcalyr
		  write (5525,*) 'CO2_ELEC = ', CO2_ELEC(1:4,curiyr)
	   end if

	
       write (5525,*) 'check that we are in cementCCS subroutine '
       write (5525,*) 'note that fuel prices (2018 real $) are to be used in CCS subroutine '
	   write (5525,*) 'but since fuel prices like pngin and prcx and pelin are in 1987 $'
	   write (5525,*) 'we need to inflate these to 2018 $ for the purposes of this subroutine'
       write (5525,*) 'CHECK YEAR: curcalyr curiyr = ', curcalyr, curiyr

!!!!!!!!!!!!!!!!!!!!!!! CCATS price placeholder !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! make up fake CO2 price from CCATS as a placeholder; this block will be removed after CCATS integration
! note that for now the fake CO2 price (in 1987$ hence the MC_JPGDP(29) factor 
! includes both the tax credit and the transportaion costs 
!testSACCS       do idumyr = 29,61 ! from 2018 to 2050
!testSACCS          do idummy = 1,inumreg
!testSACCS	         if (idumyr.lt.33) then ! set CO2 price to be zero before 2022
!testSACCS	            CO2_PRC_REG_45Q(idummy,idumyr) = 0.0 / MC_JPGDP(29)
!testSACCS	        	CO2_PRC_REG_NTC(idummy,idumyr) = 0.0 / MC_JPGDP(29)
!testSACCS		     else ! after year 2044 45Q tax credit expires for all since last year for starting construction to be eligible for
!testSACCS			      ! the 12-year 45Q tax credit is 2032
!testSACCS		        CO2_PRC_REG_45Q(idummy,idumyr) = 70.0 / MC_JPGDP(29) ! maybe CO2 price just for for EOR after 2044?
!testSACCS                CO2_PRC_REG_NTC(idummy,idumyr) = 35.0 / MC_JPGDP(29) ! no tax credit CO2 price (maybe just EOR value?)
!testSACCS             end if
!testSACCS		  end do
!testSACCS	   end do
!!!!!!!!!!!!!!!!!!!!!!! CCATS price placeholder !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! check the fuel use parameters are read in correctly (note these are same for all regions
! and note that the fuel rates for CCS equipment are assumed NOT to improve over time, i.e.,
! they are held constant through the projection   
...
! nor does it include the variable fuel cost shift
       do idummy = 1,inumreg
          totalcostCCS(idummy) = baselinenaturalgascost(idummy) + baselineelectricitycost(idummy) + &
            BaseCapCCS(idummy) + OplusMfixCCS(idummy) + OplusMvarCCS(idummy)
       end do
       write (5525,*) 'baseline total mean COC in 2018 = ',totalcostCCS(1:inumreg)

! For CCATS, IDM needs to fill the COC capital (in 1987 $): this is just BaseCapCCS(region) + CapAdder(region), the FIRST
! of which is read in (for each of the four Census Regions) from the cement tab in ironstl.xlsx, and the
! second of which is computed dynamically using as a basse the assumed NETL interest rate of 4.63% (which leads to a 
! CRF of 0.0623).
! CCATS also regquires the O&M cost by Census region, which is provided in two parts: the "fixed" O&M costs are read in from
! ironstl.xlsx cement tab as OplusMfixCCS(region) and OplusMvarCCS(region), 
! but the variable fuel costs for natural gas and electricity must be
! computed each model year (see immediately below). Note since prcx is already in 1987$, we need to inflate to 2018 $
! for purposes of this subroutine using JPGDP(29). 
! Now get fuel costs by Census Region for curiyr 
       do idummy = 1,inumreg
          currentnaturalgascost(idummy,curiyr) = prcx(3,idummy,2) * MC_JPGDP(29) * NatGasUseCCS
          currentelectricitycost(idummy,curiyr) = prcx(1,idummy,2) * MC_JPGDP(29) * ElectricUseCCS	
          totalfuelcost(idummy,curiyr) = currentelectricitycost(idummy,curiyr) + currentnaturalgascost(idummy,curiyr)
       end do

! Compute the dynamic CapAdderCCS by employing the current interest rate 
! First compute the real interest rate = nominal interest rate - projected rate of inflation 
! Note that the 0.0623 hardcode below is the baseline Capital Recovery Factor which is derived from the 
! 2018 NETL cost assumptions for cement kiln CCS retrofits of a 4.63% interest rate and 30-year payback period
       realinterestrate = (MC_RMTCM10Y(curiyr) - ((MC_CPI(11,curiyr) / MC_CPI(11,curiyr-1)) - 1.0) * 100.0) / 100.0
	   write (5525,*) 'MC_RMTCM10Y(curiyr) = ', MC_RMTCM10Y(curiyr)
	   write (5525,*) 'realinterestrate = ', realinterestrate

       do idummy = 1,inumreg
	      write (5525,*) 'BaseCapCCS(idummy) = ', BaseCapCCS(idummy) 
          caprecfactccs = realinterestrate * ((1.0 + realinterestrate)**(PPCCS(idummy))) / &
	         (((1.0 + realinterestrate)**(PPCCS(idummy))) - 1.0)
	      CapAdderCCS(idummy,curiyr) = BaseCapCCS(idummy) * ((caprecfactccs / 0.0623) - 1.0)
		  write (5525,*) 'CapAdderCCS(idummy,curiyr) = ', CapAdderCCS(idummy,curiyr)
          write (5525,*) 'curcalyr & idummy & realinterestrate & CRF = ', curcalyr, idummy, realinterestrate, caprecfactccs
       end do


! now fill CCATS variables asking for COC for both capital and O&M
! for now every cost component is fixed except for the fuel costs
! also note that all variables which comprise the CCATS required information (CST_CMT_INV = capital COC and
! CST_CMT_OM = variable COC) are in 2018$ and must be deflated to 1987$ since CCATS does evertything in 1987$

       do idummy = 1,inumreg
	      CST_CMT_INV(idummy,curiyr) = (BaseCapCCS(idummy) + CapAdderCCS(idummy,curiyr)) / MC_JPGDP(29) ! convert to 1987$
		  CST_CMT_OM(idummy,curiyr) = (OplusMfixCCS(idummy) + OplusMvarCCS(idummy) + &
		     totalfuelcost(idummy,curiyr)) / MC_JPGDP(29) ! convert to 1987$
       ...
       if (curcalyr.eq.ibyr) then
	      ...
	      write (5525,*) 'MC_JPGDP(33) = ', MC_JPGDP(33)
	      write (5525,*) 'MC_JPGDP(61) = ', MC_JPGDP(61)
       end if

       write (5525,*) 'mean capital cost (1987$) ', CST_CMT_INV(1:4,curiyr)
       write (5525,*) 'mean variable cost (1987$) ', CST_CMT_OM(1:4,curiyr)
       write (5525,*) 'mean capital cost (2018$) ', CST_CMT_INV(1:4,curiyr) * MC_JPGDP(29)
...
! only print out last iteration's results
	      if (curcalyr.eq.curcalyrminusone+1) then
	         curcalyrminusone = curcalyr
			 write (126,*)
			 write (126,*) '-------------------------------------------------------------'
		     write (126,*) 'year = ',curcalyr-1

!	         do idummy = 1,inumreg + 1 ! do the "+ 1" here to include both Census Regional and national price check printout 
!	           write (126,*) 'electricity: region (2018$) price ', idummy, elecprice(idummy) * MC_JPGDP(29) ! 2018 $
!	           write (126,*) 'natural gas: region (2018$) price ', idummy, natgasprice(idummy) * MC_JPGDP(29) ! 2018 $
!             end do

			 write (126,*)
			 write (126,*) 'CO2_PRC_REG_45Q(inumreg,MNUMYR)'
			 write (126,*) 'assumed CO2 price after optimization, 45Q eligible CO2 (1987$/tonne CO2)'
			 write (126,*) CO2_PRC_REG_45Q(1:4,curiyr-1)
             write (126,*)
			 write (126,*) 'CO2_PRC_REG_NTC(inumreg,MNUMYR)'
			 write (126,*) 'assumed CO2 price after optimization, 45Q INeligible CO2 (1987$/tonne CO2)'
			 write (126,*) CO2_PRC_REG_NTC(1:4,curiyr-1)
             write (126,*)
             write (126,*) 'CST_CMT_INV(inumreg,MNUMYR)'
			 write (126,*) 'mean capital cost (1987$/tonne CO2) '
		     write (126,*) CST_CMT_INV(1:4,curiyr-1) 
             write (126,*)
             write (126,*) 'CST_CMT_OM(inumreg,MNUMYR)'
			 write (126,*) 'O&M cost CCS $/tonne (1987$/tonne CO2); includes fuel costs '
		     write (126,*) CST_CMT_OM(1:4,curiyr-1)
             write (126,*)
			 write (126,*) 'fixed variable costs'
			 write (126,*) OplusMfixCCS(1:4)/MC_JPGDP(29)
			 write (126,*) 
			 write (126,*) 'non-fuel variable costs'
			 write (126,*) OplusMvarCCS(1:4)/MC_JPGDP(29)
			 write (126,*)
			 write (126,*) 'fuel variable costs'
			 write (126,*) totalfuelcost(1:4,curiyr-1)/MC_JPGDP(29)
			 write (126,*)
             write (126,*) 'SUP_CMT_45Q(inumreg,mnumyr)'
			 write (126,*) 'CO2 volume captured from cement plants that are 45Q eligible (tonnes)'
		     write (126,*) SUP_CMT_45Q(1:4,curiyr-1)
             write (126,*)
             write (126,*) 'SUP_CMT_NTC(inumreg,mnumyr)'
			 write (126,*) 'CO2 volume captured from cement plants assuming no tax credit (tonnes)'
		     write (126,*) SUP_CMT_NTC(1:4,curiyr-1)
			 write (126,*) 
			 write (126,*) 'Bonus: Absolute total cost to install (1987$/tonne CO2)'
			 write (126,*) absolutetotalcost(1:4,curiyr-1)
			 write (126,*)
			 write (126,*) 'Bonus: total CO2 captured'
			 write (126,*) totalcapturedCO2(1:4)
			 write (126,*)
             write (126,*) 'Bonus: added CCS capacity in this year (tonnes)'
			 write (126,*) economicalCCS(1:4, curiyr-1) * 1000.0
			 write (126,*) 
			 write (126,*) 'Bonus: accumulated CCS capacity (tonnes)'
			 write (126,*) testtotalCCS(1:4) * 1000.0
			 write (126,*) 
			 write (126,*) 'Bonus: electricity consumed for CCS (trills)'
			 write (126,*) electricityconsumedCCS(1:4,curiyr-1)
			 write (126,*) 'Bonus: natural gas consumed for CCS (trills)'
			 write (126,*) naturalgasconsumedCCS(1:4,curiyr-1)
			 write (126,*) '-------------------------------------------------------------'
			 write (126,*)
			 do idumyr = curiyr-1, 61
			    write (5525,*) idumyr, idumyr+1989, CO2_PRC_REG_45Q(1:4,idumyr), CO2_PRC_REG_NTC(1:4,idumyr)
			 end do
			 write (5525,*) 'check foresight'
          end if
	   end if
! print out CCATS Results !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

! check fuel costs here to make sure our CD-weighted natural gas and electricity fuel prices are roughly
! equal to the CR fuel prices, and to check that for consistency of fuel prices across the years through 
! the credit duration 
       if (curcalyr.ge.CCSstartyearint .and. curcalyr.le.LastConsYrCCSint) then
          write (5525,*) 'check fuel costs  ++++++++++++++++++++'
		  do idummy = 1,inumreg
		     write (5525,*) 'idummy = ', idummy
             write (5525,*) 'use pcrx '
		     write (5525,*) 'curiyr = ',curiyr
			 write (5525,*) 'curcalyr = ', curcalyr
		     write (5525,*) 'natural gas cost = ', prcx(3,idummy,2) * MC_JPGDP(29) * NatGasUseCCS
		     write (5525,*) 'electricity use = ', prcx(1,idummy,2) * MC_JPGDP(29) * ElectricUseCCS	
 		     do idumyr = curiyr, curiyr + CreditDurCCSint
                write (5525,*) 'idumyr = ', idumyr			
			    if (idummy.eq.1) then
                   currentelectricitycost(idummy,idumyr) = ElectricUseCCS * MC_JPGDP(29) * &
				     (pelin(1,idumyr)*qelin(1,idumyr)+pelin(2,idumyr)*qelin(2,idumyr))/sum(qelin(1:2,idumyr))
	               currentnaturalgascost(idummy,idumyr) = NatGasUseCCS * MC_JPGDP(29) * &
				     (pngin(1,idumyr)*qngin(1,idumyr)+pngin(2,idumyr)*qngin(2,idumyr))/sum(qngin(1:2,idumyr))
                else if (idummy.eq.2) then
                   currentelectricitycost(idummy,idumyr) = ElectricUseCCS * MC_JPGDP(29) * &
				     (pelin(3,idumyr)*qelin(3,idumyr)+pelin(4,idumyr)*qelin(4,idumyr))/sum(qelin(3:4,idumyr))
	                  currentnaturalgascost(idummy,idumyr) = NatGasUseCCS * MC_JPGDP(29) * &
				     (pngin(3,idumyr)*qngin(3,idumyr)+pngin(4,idumyr)*qngin(4,idumyr))/sum(qngin(3:4,idumyr))
                else if	(idummy.eq.3) then
                   currentelectricitycost(idummy,idumyr) = ElectricUseCCS * MC_JPGDP(29) * &
				     (pelin(5,idumyr)*qelin(5,idumyr)+pelin(6,idumyr)*qelin(6,idumyr)+pelin(7,idumyr)*qelin(7,idumyr)) / &
				     sum(qelin(5:7,idumyr))
	               currentnaturalgascost(idummy,idumyr) = NatGasUseCCS * MC_JPGDP(29) * &
                     (pngin(5,idumyr)*qngin(5,idumyr)+pngin(6,idumyr)*qngin(6,idumyr)+pngin(7,idumyr)*qngin(7,idumyr)) / &
					 sum(qngin(5:7,idumyr))
				else ! only Census Region 4 is left
				   currentelectricitycost(idummy,idumyr) = ElectricUseCCS * MC_JPGDP(29) * &
				     (pelin(8,idumyr)*qelin(8,idumyr)+pelin(9,idumyr)*qelin(9,idumyr))/sum(qelin(8:9,idumyr))
	               currentnaturalgascost(idummy,idumyr) = NatGasUseCCS * MC_JPGDP(29) * &
				     (pngin(8,idumyr)*qngin(8,idumyr)+pngin(9,idumyr)*qngin(9,idumyr))/sum(qngin(8:9,idumyr))
                end if
		        write (5525,*) 'natural gas cost = ', currentnaturalgascost(idummy,idumyr)
		        write (5525,*) 'electricity use cost = ', currentelectricitycost(idummy,idumyr)	
             end do 
          end do
          write (5525,*) 'check fuel costs  ++++++++++++++++++++'
       end if
...
! Here we compute shift of COC distribution mean due to dynamic fuel price changes every year
! for the purposes of the foresight calculation which determines added new CCS retrofit capacity
! in any given year for which the added retrofit capacity will receive the 45Q tax credit 
! (currently by law that applies to any CCS retrofits added up to the end of 2032 which is
! read in as the parameter LastConsYrCCS from ironstl.xlsx).
! BUT we need to compute first the fuel cost in the current model year (curiyr = curcalyr) and all foresight
! years (the foresight length in years is determined by the read-in paramter PPCCS from ironstl.xlsx)
       if (curcalyr.ge.CCSstartyearint .and. curcalyr.le.LastConsYrCCSint) then ! don't bother computing fuel costs for the purposes
	                                                                       ! of adding CCS capacity unless 45Q tax credit in play
          continue
	      do idummy = 1,inumreg         ! get foresight fuel costs for all four Census Regions
		     do idumyr = curiyr,curiyr + PPCCS(idummy) - 1 ! need foresight fuel costs for all years within the
			                                               ! read-in payback period PPCCS; payback period includes
                                                           ! curiyr hence the "- 1" in the do loop

!               for each region we must compute the future natural gas and electricity cost using
!               forecasted natural gas and electricity prices. But since Census Regional prices are
!               only available for curiyr (e.g., prcx(1,region,2) for electricity and prcx(3,region,2) 
!               for natural gas), we need to employ the CD prices to compute the 
!               quantity-weighted average for the CR prices...except for princes in the current year curiyr 
                if (idumyr.eq.curiyr) then ! current model year employs the prcx prices since they are available
				                           ! note that this conditional ensures that whatever current model year is
										   ! as long as it is >= CCSstartyear CCS fuel costs are computed
                   currentnaturalgascost(idummy,idumyr) = prcx(3,idummy,2) * MC_JPGDP(29) * NatGasUseCCS
	               currentelectricitycost(idummy,idumyr) = prcx(1,idummy,2) * MC_JPGDP(29) * ElectricUseCCS				
                else ! for idumyr > curiyr AND curiyr <= LastConsYrCCS 
				     ! we must use CD weighted foresight prices for forecasted fuel use
                   continue
			       if (idummy.eq.1) then
                      currentelectricitycost(idummy,idumyr) = ElectricUseCCS * MC_JPGDP(29) * &
				        (pelin(1,idumyr)*qelin(1,idumyr)+pelin(2,idumyr)*qelin(2,idumyr))/sum(qelin(1:2,idumyr))
	                  ...
	                  currentnaturalgascost(idummy,idumyr) = NatGasUseCCS * MC_JPGDP(29) * &
				        (pngin(8,idumyr)*qngin(8,idumyr)+pngin(9,idumyr)*qngin(9,idumyr))/sum(qngin(8:9,idumyr))
			       end if ! end conditional for regional foresight prices calculation
				end if ! end conditional for curiyr = or not = to idumyr
             end do ! loop over current and foresight years
          end do ! loop over regions
       end if ! only for years in which the 45Q is obtained will be do the foresight		

! now using the future (foresight) CCS fuel costs computed above, compute the COC supply curve shift
! along with the y-intercept shift to be used in computing the amount of CCS retrofit additions 

       if (curcalyr.ge.CCSstartyearint .and. curcalyr.le.LastConsYrCCSint) then ! don't bother to compute shifts unless CCS added
	                                                                            ! retrofits capacity is allowed to
          continue                                                              ! be built using 45Q tax credit
	      do idummy = 1,inumreg         ! get COC curve shifts over all four Census regions
		     do idumyr = curiyr,curiyr + PPCCS(idummy) - 1 ! just as with foresight calculations of fuel costs 
			                                               ! we compute COC shifts only over the
			                                               ! read-in payback period PPCCS; payback period includes
                                                           ! curiyr hence the "- 1" in this do loop
!               compute fuel shift based on foresight natural gas and electricity costs computed in previous section
		        fuelshiftCOC(idummy,idumyr) = (currentnaturalgascost(idummy,idumyr) + currentelectricitycost(idummy,idumyr)) - &
		          (baselinenaturalgascost(idummy) + baselineelectricitycost(idummy)) ! get fuel price Shift
		                                                                             ! which is computed relative to the baseline
																			         ! fuel costs which employ 2018 fuel prices

!               total shift in supply/cost curve due to fuel cost shift + capital cost adder (2018 $)
!               note that in this totalshiftCCS equation below, 
!               CapAdderCCS(idummy,curiyr) instead of CapAdderCCS(idummy,idumyr)
!               is employed. This is because in the foresight calculation we assume just one interest rate (that of the 
!               current year curiyr) is employed when deciding the investment return over the payback period PPCCS
                totalshiftCOC(idummy,idumyr) = fuelshiftCOC(idummy,idumyr) + CapAdderCCS(idummy,curiyr)

		        yinterceptshiftCCS(idummy,idumyr) = log((slopeCCS(idummy) * halfofintCCS(idummy)) / &
		          (exp(slopeCCS(idummy) * (meanofdistCCS(idummy) + totalshiftCOC(idummy,idumyr))) - &
			       exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCOC(idummy,idumyr)))) )
			 end do ! loop over years within payback period PPCCS
		  end do ! loop over Census Regions
       end if ! only for years in which the 45Q is obtained will be do the foresight
...
! write out foresighted shifted y-intercept
       if (curcalyr.eq.LastConsYrCCSint) then
	    continue
!       if (curcalyr.ge.CCSstartyearint .and. curcalyr.le.LastConsYrCCSint) then
          do idummy = 1, inumreg
		     do idumyr = CCSstartyearint - 1989, curiyr
                write (5525,*) 'region and year = ', idummy, curcalyr
		        write (5525,*) 'base y-intercept ', idumyr+1989,yinterceptCCS(idummy)
		        write (5525,*) 'shift i-intercept ', idumyr+1989,yinterceptshiftCCS(idummy,idumyr)
                write (5525,*) 'baselinenaturalgas cost = ', baselinenaturalgascost(idummy)
				write (5525,*) 'baseelectricity cost = ', baselineelectricitycost(idummy)
                write (5525,*) 'check current natural gas cost = ', currentnaturalgascost(idummy,idumyr)
                write (5525,*) 'check current electricity cost = ', currentelectricitycost(idummy,idumyr)	
             end do
		  end do
         write (5525,*) 'shifty '
         write (5525,*)
...
! check on ***TOTAL*** CO2 available with and without shift; these are not necessary to run IDM`
! but they provide a check in the total available CO2 available from cement retrofits
! as dictated by NETL's cement kiln database 
! again, the calculations below are just a check and will be overriden
! NOTE: these ***TOTAL*** CO2 values both with and without the shift should be the same
! if the formualtion is correct
       do idummy = 1,inumreg
	      maxpossibleCCS(idummy) = normCCS(idummy) * (exp(yinterceptshiftCCS(idummy,curiyr)) / slopeCCS(idummy)) * &
			    (exp(slopeCCS(idummy) * (highestCCS(idummy) + totalshiftCOC(idummy,curiyr))) - &
				 exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCOC(idummy,curiyr))))
          write (5525,*) 'total CO2/CCS capacity with shift ', idummy, maxpossibleCCS(idummy)
	      maxpossibleCCS(idummy) = normCCS(idummy) * (exp(yinterceptCCS(idummy)) / slopeCCS(idummy)) * &
			    (exp(slopeCCS(idummy) * highestCCS(idummy)) - &
				 exp(slopeCCS(idummy) * lowestCCS(idummy)))
          write (5525,*) 'total CO2/CCS capacity without shift ', idummy, maxpossibleCCS(idummy)
!         note that the previous two write statements for economicalCCS which set the maximum theoretical capacity of 
!         cement plants that can add CCS should be equal
!         *** Note that economicalCCS and maxpossibleCCS are in units of thousand metric tons (M tonnes) ***
       end do

! print out maxpossibleCCS
!       do idummy = 1,inumreg
!	      write (5525,*) 'year ', 'region ', 'maxpossibleCCS '
!	      write (5525,*) curcalyr, idummy, maxpossibleCCS(idummy)
!	   end do

! ################# now let's do the calculation of if and how much CCS retrofit capacity to add #############################
	   
	   if (curcalyr.lt.CCSstartyearint) then    ! too soon to add CCS retrofit capacity
          continue
	      do idummy = 1,inumreg                 ! too early for CCS so we have no new CCS CO2 retrofit capacity
	         economicalCCS(idummy,curiyr) = 0.0 ! no CCS capacity added before CCSstartyear
	         testtotalCCS(idummy) = 0.0  ! initialize accumulated added CCS capacity
		  end do
       else if (curcalyr.ge.CCSstartyearint .and. curcalyr.le.LastConsYrCCSint) then ! we can consider implementing 
	                                                                                 ! NEW cement retrofit CCS with 45Q support
																					 ! since the model year (curcalyr)
																					 ! is within the interval when a CCS
																					 ! retrofit will qualify for the 45Q credit

          write (5525,*) 'Allow new 45Q-SUPPORTED CCS in year ', curcalyr ! yes we can (but not guaranteed we will) add CCS equipment
          do idummy = 1,inumreg  ! loop over all Census Regions since we must determine additional CCS equipment by Census Region

             CCSretrofitscalar(idummy,curiyr) = (maxpossibleCCS(idummy) - testtotalCCS(idummy)) / maxpossibleCCS(idummy)
			 ! CCSretrofitscalar has a range from zero to 1.0 and decreases as more available capacity is retrofitted
			 ! this prevents the new added retrofit capacity from exceeding availble initial 
			 ! kiln capacity which could be retrofitted
			 write (5525,*) 'year ', 'region ', 'CCSretrofitscalar '
			 write (5525,*) curcalyr, idummy, CCSretrofitscalar(idummy,curiyr)

             economicalCCS(idummy,curiyr) = 0.0 ! initialize new CCS retrofit capacity which will be accumulated
			                                    ! over the payback period starting from the current model year curiyr

             do idumyr = curiyr,curiyr + PPCCS(idummy) - 1  ! loop over payback period for determining profibility 
			                                                ! over the entire payback period using perfect foresight for 
												            ! natural gas and electricity prices

                if ((-1.0)*CO2_PRC_REG_45Q(idummy,idumyr) * MC_JPGDP(29).lt.lowestCCS(idummy) + totalshiftCOC(idummy,idumyr)) then
			       economicalCCS(idummy,curiyr) = economicalCCS(idummy,curiyr) + 0.0 ! if CCATS price < lower limit
				                                                                     ! of retrofit COC then new
				                                                                     ! installed retrofit CCS capacity is zero
                else if ((-1.0)*CO2_PRC_REG_45Q(idummy,idumyr) * MC_JPGDP(29).gt.highestCCS(idummy) + totalshiftCOC(idummy,idumyr)) then
!                  CCATS price > highest COC therefore all new retrofit CCS is economical
	               economicalCCS(idummy,curiyr) = economicalCCS(idummy,curiyr) + CCSretrofitscalar(idummy,curiyr) * &
				   normCCS(idummy) * (exp(yinterceptshiftCCS(idummy,idumyr)) / slopeCCS(idummy)) * &
			      (exp(slopeCCS(idummy) * (highestCCS(idummy) + totalshiftCOC(idummy,idumyr))) - &
				   exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCOC(idummy,idumyr))))			    
                else ! CCATS price is somewhere in between the lowest (shifted) COC and highest (shifted) COC
			         ! so we must compute what fraction of the remaining kiln capacity is economically 
				     ! available for new CCS retrofits
			       economicalCCS(idummy,curiyr) = economicalCCS(idummy,curiyr) + CCSretrofitscalar(idummy,curiyr) * &
				      normCCS(idummy) * (exp(yinterceptshiftCCS(idummy,idumyr)) / slopeCCS(idummy)) * &
			         (exp(slopeCCS(idummy) * (-1.0)*CO2_PRC_REG_45Q(idummy,idumyr) * MC_JPGDP(29)) - &
				      exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCOC(idummy,idumyr))))
!				      write (5525,*) 'region and economicalCCS after shift = ', idummy, economicalCCS(idummy,curiyr)
                end if
             end do ! end loop over payback period for region idummy
			 economicalCCS(idummy,curiyr) = economicalCCS(idummy,curiyr) / (1.0 * PPCCS(idummy)) ! take average of 
			                                                                                     ! accumulated economical CCS over
																								 ! payback period to get actual
																								 ! added CCS retrofit capacity
																								 ! in curcalyr
             write (5525,*) 'added CCS capacity based on average economical CCS over payback period PPCCS = ' 
			 write (5525,*) economicalCCS(idummy,curiyr)																								 ! new CCS capacity added
          end do ! finish loop over all regions

       else ! consider new CCS retrofit builds after 45Q eligible support dies (up to 2032 based on current policy)
          do idummy = 1,inumreg
             economicalCCS(idummy,curiyr) = 0.0 ! for now, no one will build CCS retrofit without a current tax credit
			                                    ! which means if curcalyr > LastConsYrCCSint then no added CCS retrofits
          end do
       end if
       write (5525,*) 'CCSretrofitscalar = ', CCSretrofitscalar(1:4,curiyr)

!############################################################################################################################


! now apply penetration rate (which is an ironstl.xlsx input file parameter)
! to get actual CCS equipment installed and NEW CO2 captured in curiyr
! remember in a given curiyr, economicalCCS(idummy,curiyr) is the aomunt of NEW CCS equipment installed to 
! capture NEW CO2 and does NOT include CO2 captured in curiyr for CCS retrofit equipment
! that has been PREVIOUSLY installed. The computation of CO2 captured from previously installed CCS equipment 
! (by "previous equipment" we mean CCS equipment installed before curiyr) is computed later 
! and summed up in the accumulateNTC and accumulate45Q variables
       testtotalCCS(1:INUMREG) = 0.0 ! initialize for total CCS equipment capacity installed
       do idummy = 1,inumreg
	      maxeconomicalCCS(idummy,curiyr) = economicalCCS(idummy,curiyr) ! store maximum ECONOMICAL retrofit capacity
		                                                                 ! i.e., capacity for which it is economically viable
																		 ! based on current year variable costs to
																		 ! install new CCS equipment

! Note that here is where we apply the "soft" technical potential factor techpotenCCS(inumreg) to
! dampen the new CCS build rate based solely on economic terms
	      economicalCCS(idummy,curiyr) = maxeconomicalCCS(idummy,curiyr) * techpotenCCS(idummy)

! check to see if we have added too much CCS equipment, that is, check
! to see if the recent addition of CCS in curiyr accumulated above the maximum 
! theoretical CCS allowed (defined by the maximum capturable CO2 = maxpossibleCCS(region,year)
! this will likely never happen given the "techpotenCCS" factor which greatly dampens the
! economic potential for installing CCS equipment but we want to be sure that
! new additional CCS retrofits never exceeed the maximum possible kiln-based retrofit
! capacity based on NETL data.
! To do this we need to accumulate through the current projection year all installed CCS retrofit capacity
! this variable testtotalCCS(region) will be used in CCSretrofitscalar(region,curiyr) to make certain
! that retrofitted CCS capacity never exceeds total available NETL kiln capacity
          do idumyr = CCSstartyearint-1989, curiyr
		     testtotalCCS(idummy) = testtotalCCS(idummy) + economicalCCS(idummy,idumyr)
          end do
		  write (5525,*) 
		  write (5525,*) 
		  write (5525,*) 
		  write (5525,*) 
		  write (5525,*) 'for region ', idummy
   	      write (5525,*) 'year and economicalCCS ', curcalyr, economicalCCS(idummy,curiyr)
		  write (5525,*) 'the accumulated CCS capacity installed ', testtotalCCS(idummy)


! to accumulate captured CO2 in the current year (curiyr) first
! initialize variable which will accumulate CO2 from all previous retrofits up to and 
! including CO2 captured by the current year's (curiyr) newly installed retrofit CCS equipment
		     accumulate45Q(idummy) = 0.0       ! initialize accumulated captured CO2 in curiyr that is 45Q eligible
			 accumulateNTC(idummy) = 0.0       ! initialize accumulated captured CO2 in curiyr that is NOT 45Q eligible
	   end do

! print out summary of new retofit CCS capacity
       write (5525,*)
       write (5525,*) 'New retrofit capacity in M tonnes'
	   write (5525,*) 'year = ', curcalyr
       do idummy = 1,inumreg
          write (5525,*) 'max economic ',maxeconomicalCCS(idummy,curiyr)
          write (5525,*) 'actual ',economicalCCS(idummy,curiyr)
	   end do
       write (5525,*) 'New retrofit capacity '
       write (5525,*)

! before computing the actual CO2 captured by all previously installed CCS equipment (including 
! CCS installed in the current model year curiyr), we need to compute the COC curve shift so we can
! compare the economics of any given CCS build year (from CCSstartyear to LastConsYrCCS) to the Capture
! economics of the current model year curiyr. This is especially important after the 45Q tax credit expires 
! for a given CCS build year and after the payback period is over (when capital costs have been paid off)
       write (5527,*)
       write (5527,*) 'fuelshift year ', curiyr, curcalyr
       write (5527,*) 'fuelshift year ', curiyr, curcalyr
       do idummy = 1,inumreg ! set up fuel shift parameter for current model year (curiyr) COC curve
	      fuelshiftCO2(idummy) = prcx(3,idummy,2) * MC_JPGDP(29) * NatGasUseCCS + &
	         prcx(1,idummy,2) * MC_JPGDP(29) * ElectricUseCCS - &
		     (baselinenaturalgascost(idummy) + baselineelectricitycost(idummy))
			 write (5527,*) idummy,fuelshiftCO2(idummy),fuelshiftCOC(idummy,curiyr)
       end do

! set up range of years of over which the accumulated CO2 is computed
! by "range" we mean those years in which CCS equipment was installed
! the goal here is to compute the total CO2 captured in any model year from 
! each of the previous years' installed CCS equipment.

! First compute end of range over the CCS-installation years we are coverering  
       if (curcalyr.le.LastConsYrCCSint) then ! If in curiyr (curcalyr) we have not yet reached the end of the period
	      endyrCCS = curiyr  ! allowing for 45Q supported CCS retrofits builds then loop through all build years
	   else                  ! up to and including curiyr for the purposes of computing total CO2 captured
	      endyrCCS = LastConsYrCCSint - 1989  ! Otherwise loop only through the entire range of all 
	   end if                                 ! 45Q eligible years as defined by the inclusive 
			                                  ! range of CCSstartyear and LastConsYrCCS
 	   write (5525,*)'curiyr curcalyr endyr ', curiyr, curcalyr, endyrCCS

       do idummy = 1,inumreg  ! one Census Region at a time...

		  do idumyr = CCSstartyearint-1989, endyrCCS   ! loop over all years for which 
			                                           ! CCS retrofit equipment has been
		                                               ! previously installed (or can be installed 
													   ! under the 45Q tax credit). We are evaluating the
													   ! CO2 captured from each of these installed 
													   ! CCS capacities year by year to see how much CO2
													   ! they capture in the current model year (curiyr)

             if (curiyr-idumyr .lt. PPCCS(idummy)) then ! for simplicity's sake we will assume that
			                                            ! if curiyr (the current model year) is within the payback period PPCCS
      	                                                ! of a given year's (idumyr) installed CCS capacity,
													    ! then all of th CO2 is captured, i.e., the CCS retrofit
													    ! is running at 100% utilization
													    ! NOTE that PPCCS will always be <= (usually =) to
													    ! the credit duration CreditDurCCS (which is 12 years under
													    ! the 45Q tax regulation)
													   
               accumulate45Q(idummy) = accumulate45Q(idummy) + economicalCCS(idummy,idumyr) ! CO2 captured = 
			                                                                                ! CCS retrofit capacity
			                                                                                ! if within payback period PPCCS
			 else ! we are at a model year beyond the payback period of a given year's (idumyr) CCS installation
			    continue

!               Since we are past the payback period PPCCS, we need to redefine the COC curve in
!               curcalyr (or equivalently curiyr) so that it reflects a cheaper COC by shifting the COC Curve
!               to the left by removing the BASE capital cost and not adding the CapAdderCCS:
                totalshiftCO2(idummy) = fuelshiftCO2(idummy) - BaseCapCCS(idummy)  ! re-establish curiyr's
				                                                                   ! COC shift because we need to  
                                                                                   ! compute CO2 captured in curiyr;
                                                                                   ! note that after the payback
				                                                                   ! period PPCCS has passed the 
																				   ! totalshift no longer has the 
																				   ! CapAdderCCS added to it
																				   ! but moreover removes
																				   ! the Base Capital Cost BaseCapCCS

		        yinterceptshiftCO2(idummy) = log((slopeCCS(idummy) * halfofintCCS(idummy)) / &
		         (exp(slopeCCS(idummy) * (meanofdistCCS(idummy) + totalshiftCO2(idummy))) - &
			      exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCO2(idummy)))) )


				write (5527,*) 'total and y-intercept shifts'
				write (5527,*) idummy,idumyr+1989,curiyr+1989
				write (5527,*) totalshiftCO2(idummy), totalshiftCOC(idummy,curiyr)
				write (5527,*) yinterceptshiftCO2(idummy), yinterceptshiftCCS(idummy,curiyr)

                if (curiyr-idumyr .ge. CreditDurCCSint) then ! first deal with CCS equipment that
                                                             ! was previously installed and is NO LONGER
                                                             ! still eligible for 45Q tax credit
															 ! for now (see ironstl.xlsx) we assume that the 
															 ! Payback Period PPCCS(region) is = to the 
															 ! 45Q tax credit duration CreditDurCCS
                   continue
				   write (5525,*) 'NTC here'
                   if (maxeconomicalCCS(idummy,idumyr).lt.0.0000001) then ! if CCS retrofit were uneconomic to build in the first,
				      accumulateNTC(idummy) = accumulateNTC(idummy) + 0.0 ! place, then no accumlation of CO2 from that year
				   else        ! there was CCS build in idumyr; let us see to CO2 is still being captured even
				      continue ! though the tax credit has expired the economics could still favor CO2 capture
                      if ((-1.0)*CO2_PRC_REG_NTC(idummy,curiyr) * MC_JPGDP(29).lt.lowestCCS(idummy) + totalshiftCO2(idummy)) then
			             accumulateNTC(idummy) = accumulateNTC(idummy) + 0.0 ! if NTC CCATS price < lower limit then no CO2 added
                      else if ((-1.0)*CO2_PRC_REG_NTC(idummy,curiyr) * MC_JPGDP(29).gt.highestCCS(idummy) + totalshiftCO2(idummy)) then
!                        NTC CCATS price > highest COC therefore all CO2 from previously retrofited CCS is economical
                         accumulateNTC(idummy) = accumulateNTC(idummy) + economicalCCS(idummy,idumyr)
                      else ! NTC CCATS price is somewhere in between the lowest (shifted) COC and highest (shifted) COC
			             testaccumulate = CCSretrofitscalar(idummy,idumyr) * techpotenCCS(idummy) * &
						   normCCS(idummy) * (exp(yinterceptshiftCO2(idummy)) / slopeCCS(idummy)) * &
			              (exp(slopeCCS(idummy) * (-1.0)*CO2_PRC_REG_NTC(idummy,curiyr) * MC_JPGDP(29)) - &
				           exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCO2(idummy))))
                         if (testaccumulate.gt.economicalCCS(idummy,idumyr)) then
						    accumulateNTC(idummy) = accumulateNTC(idummy) + economicalCCS(idummy,idumyr) ! capture economics in
							                                                                            ! curiyr better than when
																										! CCS was built in idumyr
						 else
						    accumulateNTC(idummy) = accumulateNTC(idummy) + testaccumulate      ! capture economics in
							                                                                    ! curiyr not as good as when
																								! CCS was built in idumyr
                            write (5525,*)							                                                                            ! curiyr not as good as when
						    write (5525,*) 'testaccumulate and economical CCS = '
						    write (5525,*) idummy, idumyr+1989, testaccumulate, economicalCCS(idummy,idumyr)
						    write (5525,*) 'woohoo NTC'																				! CCS was built in idumyr

						 end if ! end conditional to test economics of CO2 price occurring within COC curve
              	      end if ! end conditional to test economics of previous CCS build in current model year curiyr
                   end if ! end conditional to see if there was even ANY CCS retrofit equipment in a previous year
                else ! now deal with CCS equipment that was previously installed and is eligible still for 45Q credit
                   continue
				   write (5525,*) '45Q here'
                   if (maxeconomicalCCS(idummy,idumyr).lt.0.0000001) then ! if CCS retrofit were uneconomic, then
				      accumulate45Q(idummy) = accumulate45Q(idummy) + 0.0 ! no accumlation of CO2 from that previous year
				   else ! some CCS equipment was built in idumyr
				      continue
                      if ((-1.0)*CO2_PRC_REG_45Q(idummy,curiyr) * MC_JPGDP(29).lt.lowestCCS(idummy) + totalshiftCO2(idummy)) then
			             accumulate45Q(idummy) = accumulate45Q(idummy) + 0.0 ! if 45Q CCATS price < lower limit then no CO2 added 
                      else if ((-1.0)*CO2_PRC_REG_45Q(idummy,curiyr) * MC_JPGDP(29).gt.highestCCS(idummy) + &
					     totalshiftCO2(idummy)) then
!                        45Q CCATS price > highest COC therefore all previously retrofited CCS is economical
                         accumulate45Q(idummy) = accumulate45Q(idummy) + economicalCCS(idummy,idumyr)
                      else ! 45Q CCATS price in curiyr is somewhere in between the lowest (shifted) COD and highest (shifted) COC
			             testaccumulate = CCSretrofitscalar(idummy,idumyr) * techpotenCCS(idummy) * &
						 normCCS(idummy) * (exp(yinterceptshiftCO2(idummy)) / slopeCCS(idummy)) * &
			            (exp(slopeCCS(idummy) * (-1.0)*CO2_PRC_REG_45Q(idummy,curiyr) * MC_JPGDP(29)) - &
				         exp(slopeCCS(idummy) * (lowestCCS(idummy) + totalshiftCO2(idummy))))
                         if (testaccumulate.gt.economicalCCS(idummy,idumyr)) then
						    accumulate45Q(idummy) = accumulate45Q(idummy) + economicalCCS(idummy,idumyr) ! capture economics in
							                                                                            ! curiyr better than when
																										! CCS was built in idumyr
						 else
						    accumulate45Q(idummy) = accumulate45Q(idummy) + testaccumulate   ! capture economics in
							                                                                 ! curiyr not as good as when
																							 ! CCS was built in idumyr
                            write (5525,*)							                                                                            ! curiyr not as good as when
						    write (5525,*) 'testaccumulate and economical CCS = '
						    write (5525,*) idummy, idumyr+1989, testaccumulate, economicalCCS(idummy,idumyr)
						    write (5525,*) 'woohoo 45Q'																				! CCS was built in idumyr
                         end if ! end conditional to test economics of CO2 price occurring within COC curve`   
              	      end if ! end conditional to test economics of previous CCS build in current model year curiyr
                   end if ! end conditional to see if there was even ANY CCS retrofit equipment in a previous year
                end if ! end conditional to see if 45Q has expired 
             end if ! end conditional if curiyr is within PPCCS payback period of year we are testing (idumyr) for CO2 capture
          end do ! loop over all CCS retrofit install years up through curiyr if curiyr <= LastConsYrCCS
       end do ! loop over all Census Regions

!       write (5527,*) 'SUMMARY CO2 '
!       write (5527,*) 'CCS installed and accumulated CO2 in M tonnes'
!	   write (5527,*) 'year = ', curcalyr
!          write (5527,*) 'max economic ',maxeconomicalCCS(1:4,curiyr)
!          write (5527,*) 'current CCS equipment CO2 ', economicalCCS(1:4,curiyr)
!          write (5527,*) '45Q ', accumulate45Q(1:4)
!          write (5527,*) 'NTC ', accumulateNTC(1:4)
!       write (5527,*) 'SUMMARY CO2 '

     
       do idumyr = CCSstartyearint-1989, curiyr
          write (5525,*) 'curcalyr and economicalCCS', idumyr+1989, economicalCCS(1:4,idumyr)
       end do
       write (5525,*) 'ccchhheeeccckkk'
! for writing and testing purposes only
       do idummy = 1,inumreg + 1
	      natgasprice(idummy) = prcx(3,idummy,2)
		  elecprice(idummy) = prcx(1,idummy,2)
	   end do

!       write (126,*) 'for year = ',curcalyr
!	   do idummy = 1,inumreg + 1 ! do the "+ 1" here to include both Census Regional and national price check printout 
!	      write (126,*) 'electricity: region (2018$) price ', idummy, prcx(1,idummy,2) * MC_JPGDP(29) ! 2018 $
!	      write (126,*) 'natural gas: region (2018$) price ', idummy, prcx(3,idummy,2) * MC_JPGDP(29) ! 2018 $
!	      write (126,*) 'electricity: region (1987$) price ', idummy, prcx(1,idummy,2) ! 1987 $
!	      write (126,*) 'natural gas: region (1987$) price ', idummy, prcx(3,idummy,2) ! 1987 $
!      end do

! fill in CO2 captured for CCATS in tonnes CO2 (not M tonnes CO2)
       do idummy = 1,inumreg
          SUP_CMT_45Q(idummy,curiyr) = accumulate45Q(idummy) * 1000.0 ! CO2 captured from kilns that are 45Q eligible (in tonnes)
          SUP_CMT_NTC(idummy,curiyr) = accumulateNTC(idummy) * 1000.0 ! CO2 volumes from kilns with no tax credit (in tonnes)		 
	   end do

! fill current model year's consumption of fuel for CCS operations
! fill as trillion Btu`(need 0.000001 factor because fuel use rates are in MMBtu/tonne CO2 captured)
       do idummy = 1,inumreg
          electricityconsumedCCS(idummy,curiyr) = 0.000001 * &
		     (SUP_CMT_45Q(idummy,curiyr) + SUP_CMT_NTC(idummy,curiyr)) * ElectricUseCCS
          naturalgasconsumedCCS(idummy,curiyr) = 0.000001 * &
		     (SUP_CMT_45Q(idummy,curiyr) + SUP_CMT_NTC(idummy,curiyr)) * NatGasUseCCS
       end do

	   end subroutine cementCCS
...
!Anode Production
      ...
      integer ii
      real AnodeReq(7) ! KiloTonnes of Anode Required PB_2,PB_3,PB_4,IA_1,PB_PM,PB_EX,sum(1:3,5)
      real FuelInput(2,2) !Fuel Input: (Electricity,Natural Gas: PBA_NG1,PBA_NG2)
      real ProcInput(2,2) !Process Input: (Petroleum Coke,Petroleum Pitch: SBA_NG1,PBA_NG1,PBA_NG2)
      real baseline_prod          ! baseline anode Production,  all Pre-Bake, assume produced using PBA_NG1 technology
      ...
      stepnum=2       ! base smelting
      SmeltStep=1
!******************************************************************
      ! PB_EX base year base smelting

      if (curiyr.le.ICURIYR) then
          baseline_prod=sumprodcur(inddir,SmeltStep)
          else
          baseline_prod = MIN(sumprodcur(inddir,SmeltStep),is_surviving_cap(inddir))    ! surviving base smelting capacity still in use; PB_EX
           !   baseline_prod = MIN(sumprodcur(inddir,stepnum),is_surviving_cap(inddir))*IS_BASE_TECH_SHARE(inddir,1,1+is_numtech(inddir,SmeltStep))
          if (baseline_prod.lt.0.0) baseline_prod=0.0
      endif
      
      DO ii=1,is_numtech(inddir,SmeltStep)
          tot_production_tech(inddir,ii) = MAX((sumprodcur(inddir,SmeltStep) - is_surviving_cap(inddir)),0.0) * is_base_tech_share(inddir,SmeltStep,ii)
      ENDDO
      
      tot_production_tech(inddir,is_numtech(inddir,SmeltStep)+1) = baseline_prod       ! PB_EX smelting output
      
 !*********************************************************************************************

!Anode required (kT)
      AnodeReq(is_numtech(inddir,SmeltStep)+1)=tot_production_tech(inddir,is_numtech(inddir,SmeltStep)+1)*AnodeRatio(6)   ! PB_EX anode required (base year smelting)

      do ii=1,is_numtech(inddir,SmeltStep)                                                                                ! PB_2, PB_3, PB_4, IA_1=0, PB_PM (new-built smelting)
                    AnodeReq(ii)=tot_production_tech(inddir,ii)*AnodeRatio(ii)
      enddo

!Anode Requirement by Type (kT) 
!AnodeReq(7)=sum AnodeReq(1:3,5) (PB_2, PB_3, PB_4, PB_PM)

      AnodeReq(7)=0.0  
      
      do ii=1,is_numtech(inddir,SmeltStep)        ! number of smelting technolgies
          AnodeReq(7)=AnodeReq(7)+AnodeReq(ii)    ! AnodeReq for ii=4 (IA_1) is 0, because AnodeRatio is 0 for IA_1 (inert anode)
      enddo

!Fuel Input
! Electricity (MMBtu) and natural gas (MMBtu)
! existing base capacity fuel use for anode production (all PBA_NG1 anode production tech to supply anodes for use in PB_EX smelting)
      FuelInput(1,1)=IS_FUEL_USE(inddir,stepnum,1,1)*AnodeReq(1+is_numtech(inddir,SmeltStep))        ! electricity
      FuelInput(2,1)=IS_FUEL_USE(inddir,stepnum,2,1)*AnodeReq(1+is_numtech(inddir,SmeltStep))        ! natural gas

! new capacity fuel use for anode production (assume all PBA_NG2 anode production tech to supply anodes for smelting techs other than PB_EX and IA_1; SBA_PM anode production tech not in use for now)
      FuelInput(1,2)=IS_FUEL_USE(inddir,stepnum,1,2)*AnodeReq(7)                                      ! electricity
      FuelInput(2,2)=IS_FUEL_USE(inddir,stepnum,2,2)*AnodeReq(7)                                      ! natural gas

! Process inputs (petcoke and other petroleum)
! existing base capacity nonfuel use for anode production (all PBA_NG1 anode production tech to supply anodes for use in PB_EX smelting)
      ProcInput(1,1)=IS_FUEL_USE(inddir,stepnum,6,1)*AnodeReq(1+is_numtech(inddir,SmeltStep)) ! pet coke
      ProcInput(2,1)=IS_FUEL_USE(inddir,stepnum,7,1)*AnodeReq(1+is_numtech(inddir,SmeltStep)) ! other petroleum
! new capacity nonfuel use for anode production (assume all PBA_NG2 anode production tech to supply anodes for smelting techs other than PB_EX and IA_1; SBA_PM anode production tech not in use for now)
      ProcInput(1,2)=IS_FUEL_USE(inddir,stepnum,6,2)*AnodeReq(7) !pet coke
      ProcInput(2,2)=IS_FUEL_USE(inddir,stepnum,7,2)*AnodeReq(7)  !other pet
      
! Note other petroleum is assumed 0 in MECS 2018, and the petcoke nonfuel consumption is approximated as fuel consumption given it is a) small and b) anode consumption is kind of similar to combustion anyways

!Anode Production Energy Summary (MMBtu)

       ElecCons(stepnum,curiyr)=FuelInput(1,1)+FuelInput(1,2) ! electricity
       NGCons(stepnum,curiyr)=FuelInput(2,1)+FuelInput(2,2)   ! natural gas
       PetCons(stepnum,curiyr)=ProcInput(1,1)+ProcInput(1,2)  ! pet coke
       OPetCons(stepnum,curiyr)=ProcInput(2,1)+ProcInput(2,2) ! other petroleum
       return
       end
...
! glass industry submodule
	   ...
	   DO ii=1,3		! 1=flat, 2=container, 3=specialty; as of AEO2025, only container has a non-0 value
		  cullet_recycle_share(ii,curiyr)=cullet_recycle_share_start(ii)+(cullet_recycle_share_final(ii)-cullet_recycle_share_start(ii))/(float(ijumpyr-icuriyr))*(curiyr-icuriyr)
	   ENDDO
	   
	   container_fuel_reduction(curiyr)=0.025/0.10*cullet_recycle_share(2,curiyr)	! container furnaces energy consumption is reduced by about 2.5% for each 10% of input that is recycled cullet

       do ii=1,4
          Gl_add_elec(ii)=gl_cryo*gl_oxy(2,ii)*0.907185*1000.0
       enddo
	   
       call GL_Shipping
	   
	CALL GL_OxyFuel(1)	! flat glass
       do ii=1,4
         call TECH_STEP(ii)
       enddo
       
	CALL GL_OxyFuel(2) 	! container glass
       do ii=5,7
         call TECH_STEP(ii)
       enddo
	   
    CALL GL_OxyFuel(3)	! blown glass
       do ii=8,11
         call TECH_STEP(ii)
       enddo

	CALL GL_OxyFuel(4)	! specialty glass (fiber)   
       do ii=12,14
         call TECH_STEP(ii)
       enddo
       ...
       IMPLICIT NONE
        ...
        END SELECT
         ...
         do i=1, nfurn
			...
			! Hardcode here because the glass formula occasionally doesn't play nice with the Python wrapper--
			! if there was a slight rounding errror or something.
      ...
      ! initialization block
	   ...
	   TotBliq = 0.0
        ...
        if (inddir == 10) then  !glass
			...
			Glass_Furnace_Process_Emissions(1)=sumprodcur(inddir,4)*CO2_Glass_Process_Emission_Factor/1000.		! flat glass
			Glass_Furnace_Process_Emissions(2)=sumprodcur(inddir,7)*CO2_Glass_Process_Emission_Factor/1000.		! container glass (recyled cullet already subtracted out)
			Glass_Furnace_Process_Emissions(3)=sumprodcur(inddir,11)*CO2_Glass_Process_Emission_Factor/1000.	! blown glass
			Glass_Furnace_Process_Emissions(4)=sumprodcur(inddir,14)*CO2_Glass_Process_Emission_Factor/1000.	! fiber/specialty glass
...
1666    if (curiyr.eq.IS_FISYR(inddir)) numiter=numiter+1        ! Ensure first-year tech shares match history
        ...
        DO inumtech=1,is_numtech(inddir,stepnum)                 ! For each technology
			...
			scale_comp = scale_fctr2(is_logit_coeff(inddir,:,stepnum),   & ! Function to calculate the un-exponentiated logit component
                                ...
                                is_alpha(inddir,stepnum,inumtech),logit_steam(inumtech),obsoleteyr(inddir,stepnum,inumtech),stepnum,particulate_dummy,inumtech)
        ...
        ELSE                                                   ! Otherwise, don't bother adjusting
           scale_fac = 0.0
        ENDIF


        if (inddir.eq.12 .and. stepnum.ge.6 .and. stepnum.le.7) then
           scale_min = 0.0; scale_fac = 0.0
           scale_adj = 30.0                                         ! Additive Adjustment Factor
           do inumtech=1,is_numtech(inddir,stepnum)                 ! For each technology
		   	  scale_comp = scale_fctr2(is_logit_coeff(inddir,:,stepnum),   & ! Function to calculate the un-exponentiated logit component
                        ...
                        IS_fuel_use(inddir,stepnum,:,inumtech), &   
                        IS_emiss(inddir,stepnum,inumtech) *      &
  				        emtax(curiyr) * (MC_JPGDP(20) / MC_JPGDP(-2)) * (12.011/44.01) * 1000.0,      &
                        is_co2penalty(inddir,curiyr),           &
                        is_alpha_decay(inddir,:),               &
                        is_alpha(inddir,stepnum,inumtech),logit_steam(inumtech),obsoleteyr(inddir,stepnum,inumtech),stepnum,particulate_dummy,inumtech)
              IF (scale_comp.lt.scale_min) scale_min = scale_comp   ! Select the smallest value generated by the various technologies
...
!***** Pass the Technology Characteristics to the Logit Calculation Function *********
        DO inumtech=1,is_numtech(inddir,stepnum)                            ! For Each Competing Technology
            logit_comp(inumtech) = logit_calc(is_logit_coeff(inddir,:,stepnum),                  &
                                          ...
                                          scale_fac,logit_steam(inumtech),obsoleteyr(inddir,stepnum,inumtech),stepnum,particulate_dummy,inumtech)
        ENDDO
...
!***** In Base Year, Calibrate Alternate-Specific Constants ***************
		...
		IF((inddir.eq.12).and.(stepnum.ge.6).and.(stepnum.le.7).and.(curcalyr.le.(SedsLastYr+1)))THEN
			logit_comp(3)=0.0		! No H2 DRI before SEDS+1 (2023 for AEO2025)
			CALL shares_calc(logit_comp,shares2,is_numtech(inddir,stepnum))
		ENDIF
!***** Calculate Total Process Output from Each Technology *******************
      ! set base year shares variable from inputs for reporting only
        ...
        DO inumtech=1,is_numtech(inddir,stepnum)         ! For Each Technology
 	    ...
 	    tot_production_tech(inddir,inumtech) = MIN(is_basecap(inddir,stepnum),is_surviving_cap(inddir)) * is_base_tech_share(inddir,stepnum,inumtech) &  ! Capacity from Baseline Technologies
                                      + surviving_added_capacity(inumtech)                                                                                ! + surviving added capacity
          ...
          if ((inddir.eq.13).and.(stepnum.eq.1)) then    ! "extra" step for anode base smelting
              tot_production_tech(inddir,is_numtech(inddir,stepnum) +1) = MIN(sumprodcur(inddir,stepnum),is_basecap(inddir,stepnum))  ! surviving base capacity only for base smelting
              tot_production_tech(inddir,inumtech) = surviving_added_capacity(inumtech)                                               ! surviving added capacity only for non-base smelting
          endif
 ...
 !!! Calculate SOA_Adj here for testing
            ...
            do i = 1,is_numtech(inddir,stepnum)  !!this won't do anything to alum anode since num technologies is set to zero
                 SOA_adj(inddir,stepnum,i) =  1.0+ ( is_rei(inddir,stepnum,i)- 1.0)*(1.0-1.0/(1+((MAX((curcalyr-ibyr-SOA_Offset),0))/(SOA_Inflection-SOA_Offset))**SOA_Beta))
...
!glass steps 3,6,10,13 are furnace
				...
				IF(stepnum.eq.6) IS_tot_energy_use(ifuel,inumtech) = IS_tot_energy_use(ifuel,inumtech)*(1-container_fuel_reduction(curiyr))
                if (ifuel == 2) then  ! natural gas
                    SELECT CASE (stepnum)
                      CASE (3)
...
!***** Special Case: Aluminum  ***************
                 ...
                 IF ((stepnum == 1).or.(stepnum == 3)) then
                    IS_tot_energy_use(ifuel,inumtech) = tot_production_tech(inddir,inumtech)       &           ! Total Production (kT)
                                                * IS_fuel_use(inddir,stepnum,ifuel,inumtech) &
                                                * SOA_adj(inddir,stepnum,inumtech)
                 endif
...
! some, but not all consumption, is=0 in trills 
        if (inddir > 8) then
          if (inddir == 12) then !steel
			...
			H2FeedCons(stepnum,curiyr) = SUM(IS_tot_energy_use(5,1:is_numtech(inddir,stepnum)))/1000000.0        ! H2 feedstock (formerly the index for met coal; only used for steel as of 1/24)	NSK H2
		  else if ((inddir == 13).and.(stepnum == 1)) then !Aluminum
             ElecCons(stepnum,curiyr)  = SUM(IS_tot_energy_use(1,1:is_numtech(inddir,stepnum)+1))
          else
...
! Calls to: none
      ...
      prod_IBYR = is_production(inddir,stepnum)
      ...
      if (inddir.eq.12) write(IUNITBUG,'(A,3F12.3,2I6,3I4)') &
                'is_production, production,prodcurrpt, ibyr2, CURCALYR, ICURIYR2, inddir, stepnum', &
                is_production(inddir,stepnum),production,prodcurrpt(stepnum,ibyr2-1989), &
...
! Calculate total existing capacity (existing base plus existing builds since base)      
      IF (curiyr.le.FISYR) THEN
          is_surviving_cap(inddir)=IBYR_SurvCap(inddir,stepnum) ! no added capacity for base year or earlier
      ELSE
          sumup=0.0
          ...
          enddo
          is_surviving_cap(inddir)=is_basecap(inddir,stepnum)+sumup  ! total surviving capacity    
      ENDIF
                     
      needed_cap(inddir,stepnum,curiyr)=production-is_surviving_cap(inddir)       ! calculate needed capacity to meet the year's production 
      prod_adjust_fact = 1.0			! initialize a production adjustment factor
      if (needed_cap(inddir,stepnum,curiyr).lt.0.0) then
             needed_cap(inddir,stepnum,curiyr)=0.0  ! Don't need to add more capacity
             prod_adjust_fact = production/is_surviving_cap(inddir)	! This factor should proportionately reduce production
      endif
...
! Calls to: none
    ...
    function logit_calc(logit_coeff,tot_cost,lfuel_use,lemiss,co2penalty,alpha_decay,alpha,scale_fac,logit_steam,Obsolete_Yr,stepnum,cement_particulates,inumt)
    use i_
    implicit none
    ...
    if (int(Obsolete_Yr) .LE. curcalyr) then
		...
		logit_calc =  exp(calibrationc(1) * tot_cost &
        ...
        if (inddir.eq.11) then     ! cement has different logit coefficients
			...
			logit_calc =  exp(  calibrationc(1) * tot_cost &             							! Fixed costs
        ...
        if ((inddir.eq.13) .and. (stepnum.ne.5)) then	! aluminum has different logit coefficients
		    if (int(Obsolete_Yr) .LE. curcalyr) then
				logit_calc = 0.0
    		else
    			logit_calc =  exp(  calibrationc(1) * tot_cost 									&
                    ...
                    +  calibrationc(2) * (lfuel_use(3) * PRLIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)+ & ! Residual Fuel Oil
                                          lfuel_use(4) * PPCIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)) & ! Petcoke
                                          !lfuel_use(5) * PMCIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)) & ! Really petroleum pitch; rolled into petcoke
...
! Calls to: none
    ...
    function scale_fctr2(logit_coeff,tot_cost,IS_fuel_use2,IS_emiss2,co2penalty,alpha_decay,alpha,logit_steam,Obsolete_Yr,stepnum,cement_particulates,inumt)
    use i_
    implicit none
    ...
    real IS_fuel_use2(IS_MAXFUEL)         ! Fuel use 1=Electricity 2=NG 3=HFO 4=Bit.Coal 5=H2
    ...
    if (int(Obsolete_Yr) .LE. curcalyr) then
		...
		scale_fctr2 = calibrationc(1) * tot_cost     &
					+ calibrationc(2) * (IS_fuel_use2(1) * PELIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)+ &		! Electricity - Convert to base year dollars
										   ...
										   IS_fuel_use2(5) * PH2IN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)) & 	! H2 heat and power		NSK H2
					+ calibrationc(2) * logit_steam  &		! steam uses fuel logit factor
					+ calibrationc(3) * IS_emiss2 * co2penalty       &
					+ alpha*exp(-alpha_decay(1)*float(curcalyr-ibyr)*ln2/alpha_decay(2))
		...
		if ((inddir.eq.13) .and.(stepnum.ne.5))then	! aluminum has different logit coefficients
			scale_fctr2 = calibrationc(1) * tot_cost  &
						...
						+                   IS_fuel_use2(4) * PPCIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)) & ! Petcoke
						!+                   IS_fuel_use2(5) * POTIN(MNUMCR,curiyr)*MC_JPGDP(ICURIYR2)) & ! Petroleum pitch; rolled into petcoke in ironstlx
						+ calibrationc(5) * logit_steam   &
						+ calibrationc(3) * IS_emiss2 *  co2penalty   &
						+ alpha*exp(-alpha_decay(1)*float(curcalyr-ibyr)*ln2/alpha_decay(2))
		endif
...
! Calls: none
	  ...
	  !CALL GETRNGR('CO2PEN          ',IS_CO2PENALTY(inumind,:),41,1,1)
...
! PAPER INPUTS
      CALL ReadRngXLSX(WKUNIT,'paper')   !read range names & corresponding data
...
inumind=8
      ...
      do ij=1,PP_MAXSTPS
		...
		if (ij.eq.25) CALL GETRNGR(rangen,PP_Hog(ij,1:IS_NUMTECH(inumind,ij)),1,IS_NUMTECH(inumind,ij),1)
          ibyr_paper_cap_ratio(ij)=ibyr_survcap(inumind,ij)/is_production(inumind,ij)
	  enddo
      ...
      CALL GETRNGR('mech_share2     ',mech_share(17:31),1,15,1)	!mechanical share for splitting therm+mech prodcur years 2006-2020
      CALL GETRNGR('paper_share     ',paper_share(1:5,17:31),5,15,1)	!shares to split prodcur among paper types years 2006-2020
      ...
      CALL GETRNGR('Pa_MECS         ',MECS_Data(inumind,1:5,1:10),5,10,1)                 ! row, col, group     
...
! Aluminum INPUTS
      ...
      CALL GETRNGR('ABASETECHSHR1   ',IS_BASE_TECH_SHARE(inumind,1,1:IS_NUMTECH(inumind,1)),1,IS_NUMTECH(inumind,1),1)
      ...
      CALL GETRNGR('a_FUEL_USE1     ',IS_FUEL_USE(inumind,1,1:5,1:IS_NUMTECH(inumind,1)+1),5,IS_NUMTECH(inumind,1)+1,1)! +1 for anode base smelting
      CALL GETRNGR('a_FUEL_USE2     ',IS_FUEL_USE(inumind,2,1:3,1:3),3,3,1) !find paramters TD
      CALL GETRNGR('a_FUEL_USE2a    ',IS_FUEL_USE(inumind,2,6:7,1:3),2,3,1) !Pet Coke, Pet Pitch (find parameters - td)
	...
	CALL GETRNGR('a_EMISS5        ',IS_EMISS(inumind,5,1:IS_NUMTECH(inumind,5)),1,IS_NUMTECH(inumind,5),1)
      ...
      CALL GETRNGR('a_REI2          ',IS_REI(inumind,1,1:3),1,3,1) !no number of technologies for anode in ironstlx so hardwire 3
      ...
      CALL GETRNGR('AnodeRatio      ',AnodeRatio(1:6),1,6,1)
      CALL GETRNGR('al_pf_fuel      ',al_pf_fuel(1:2),1,2,1)
      CALL GETRNGR('al_ecalib       ',is_ecalib(inumind,1:5),1,5,1)
      !CALL GETRNGR('AL_CO2PEN       ',IS_CO2PENALTY(inumind,:),41,1,1)
      CALL GETRNGR('Al_MECS         ',MECS_Data(inumind,1:5,1:10),5,10,1)                 ! row, col, group
...
! Glass INPUTS
    ...
    CALL GETRNGR('Gl_MECS         ',MECS_Data(inumind,1:5,1:10),5,10,1)                 ! row, col, group
	CALL GETRNGR('Cul_Shr_Start   ',cullet_recycle_share_start(1:3),1,3,1)
	CALL GETRNGR('Cul_Shr_Final   ',cullet_recycle_share_final(1:3),1,3,1)
...
! Lime INPUTS
    ...
    CALL GETRNGR('Li_MECS         ',MECS_Data(9,1:5,1:10),5,10,1)                 ! row, col, group
...
! read in fuel rates for cement retrofit CCS (natural gas and purchased electricity)
    CALL GETRNGR('NatGasUseCCS    ',NatGasUseCCS,1,1,1)
    CALL GETRNGR('ElectricUseCCS  ',ElectricUseCCS,1,1,1)
	
! read in exponential distribution parameters
    CALL GETRNGR('slopeCCS        ',slopeCCS(1:4),1,4,1)
    CALL GETRNGR('yinterceptCCS   ',yinterceptCCS(1:4),1,4,1)
    CALL GETRNGR('normCCS         ',normCCS(1:4),1,4,1)
    CALL GETRNGR('halfofintCCS    ',halfofintCCS(1:4),1,4,1)
    CALL GETRNGR('lowestCCS       ',lowestCCS(1:4),1,4,1)
    CALL GETRNGR('highestCCS      ',highestCCS(1:4),1,4,1)
    CALL GETRNGR('meanofdistCCS   ',meanofdistCCS(1:4),1,4,1)

! read in costs
    CALL GETRNGR('BaseCapCCS      ',BaseCapCCS(1:4),1,4,1)
!    CALL GETRNGR('CapAdderCCS     ',CapAdderCCS(1:4),1,4,1)
    CALL GETRNGR('OplusMfixCCS    ',OplusMfixCCS(1:4),1,4,1)
    CALL GETRNGR('OplusMvarCCS    ',OplusMvarCCS(1:4),1,4,1)

! read in Payback Period
    CALL GETRNGR('PPCCS           ',PPCCS(1:4),1,4,1)

! read in technical potential
    CALL GETRNGR('techpotenCCS    ',techpotenCCS(1:4),1,4,1)
	
! read in 45Q Parameters
    CALL GETRNGR('LastConsYrCCS   ',LastConsYrCCS,1,1,1)
	CALL GETRNGR('CreditDurCCS    ',CreditDurCCS,1,1,1)
	CALL GETRNGR('CCSstartyear    ',CCSstartyear,1,1,1)
!********************************************************************
...
! Calls: none
     ...
     subroutine TECHSHR_RPT
      ...
      CHARACTER*35 TECHNAME(27)               ! hot roll
    ...
    ! aluminum step names
       ...
       DATA AINDSTEPNAME /"Primary Production", &
                          "Anode Production", &
                          "Primary Alumina", &
                          "Boilers", &
      ...
      CHARACTER*35 ATECHNAME(19)              ! smelting
	  DATA ATECHNAME /"   --PB_2", &
					  "   --PB_3", &
					  "   --PB_4", &
					  "   --IA_1", &
     ...
     ! paper technology names
	  CHARACTER*65 PTECHNAME(121)           ! wood prep
	  DATA PTECHNAME /"   --Debarker: Ring", &
                        ...
                        "   --Dry press felt", &
                        "   --UNC_DRY_PM",&
                                              ! uncoated paper (finishing)
                        "   --Electric ", &
                        "   --Electric w/ nip press", &
                        "   --Electric, High Efficiency", &
                        "   --Electric, High Efficiency w/ nip press", &
                        "   --UNC_FIN_PM",&
                                              ! uncoated paper (stock prep)
                        "   --Conical Refining", &
                        "   --Disk Refining", &
                        "   --NC_PREP_PM",&
                                              ! coated paper (drying)
                        ...
                        "   --Dry press felt", &
                        "   --CTD_DRY_PM",&
                                              ! coated paper (finishing)
                        "   --Electric ", &
                        "   --Electric w/ nip press", &
                        "   --Electric, High Efficiency", &
                        "   --Electric, High Efficiency w/ nip press", &
                        "   --CTD_FIN_PM",&
                                              ! coated paper (stock prep)
                        "   --Disk Refining", &
                        ...
                        "   --Dry press felt", &
                        "   --TIS_DRY_PM",&
                                              ! tissue paper (finishing)
                        "   --Electric", &
                        "   --Electric, High Efficiency", &
                        "   --TIS_FIN_PM",&
                                              ! tissue paper (stock prep)
                        "   --Conical Refining", &
                        "   --Disk Refining", &
                        "   --TIS_PREP_PM",&
                                              ! black liquor evaporators
                        ...
                        "   --Evaporator w/ graphene oxide membrane concentrator", &    
                        "   --EVAP_PM",&
                                              ! lime kilns
                        "   --NG", &
                        "   --NG w/ flash dryer", &
      ...
      CHARACTER*35 CTECHNAME(24)                                                   !technology names
...
!**********************************************************
	  ...
	  IROW = IROW + 1
      icount=1
      do jj=ibyr,IEYR !ENDYR  POT_2050 2040 to IEYR
	  IROW=8
      ...
      DO ISTEP=1,numstep
        if ((IS_NUMTECH(12,istep).gt.0).and.(istep.ne.2)) then
            ...
            if((jj.eq.ibyr).and.(istep.ne.7)) CALL ckstr(I15,IROW,1,LINE,1,0,1)
			if (istep.ne.7) CALL cknum(I15,IROW,icol,dble(prodcurrpt(ISTEP,jj-1989)),0,4)     ! POT_2050NDIGIT 2 to 0
			if(istep.eq.6) then
			   IROW=IROW+1
			   if(jj.eq.ibyr)LINE = '  ' // TECHNAMESUB(1)
			  if(jj.eq.ibyr) CALL ckstr(I15,IROW,1,LINE,1,0,1)

			endif
			if(istep.eq.7) then
			   IROW=IROW+1
			   if(jj.eq.ibyr) LINE = '  ' // TECHNAMESUB(2)
			   if(jj.eq.ibyr) CALL ckstr(I15,IROW,1,LINE,1,0,1)

			endif

            DO IF=1,IS_NUMTECH(12,istep)
			    if(jj.eq.ibyr) LINE= techname(icount)
				icount=icount+1
               IROW = IROW + 1
               if(jj.eq.ibyr)CALL ckstr(I15,IROW,1,LINE,1,0,1)
        ...
        DO ISTEP=1,numstep
          if (IS_NUMTECH(8,istep).gt.0) then
            ...
            DO IF=1,IS_NUMTECH(8,istep)
			    if(jj.eq.ibyr) LINE= ptechname(icount)
				icount=icount+1
               IROW = IROW + 1
               jcount = jcount + 1
               if(jj.eq.ibyr)CALL ckstr(I15,IROW,1,LINE,1,0,1)
      ...
      jcount=0
      IROW = IROW + 1
      do jj=ibyr,IEYR !LASTYR  POT_2050 2040 to IEYR
        IROW=IROW-jcount ! Return to first row for this industry by subtracting jcount
        jcount=0
        CALL ckint(I15,IROW,icol,jj,0) !write year
        DO ISTEP=1,numstep
          if (IS_NUMTECH(13,istep).gt.0) then
			...
			IF(jj.eq.IBYR .and. ISTEP.eq.1) then
				CALL cknum(I15,IROW,icol,dble(al_prodcurrpt(ISTEP,jj-1989)*primprod_percent(IBYR-1989)),0,4)
			ELSEIF(jj.eq.IBYR .and. ISTEP.eq.3) then
				CALL cknum(I15,IROW,icol,dble((al_prodcurrpt(1,jj-1989)*primprod_percent(IBYR-1989)/al_mass_loss) * (1.0+al_non_met) *(1 - alumina_percent(IBYR-1989))),0,4)
			ELSEIF(jj.eq.IBYR .and. ISTEP.eq.4) then
				CALL cknum(I15,IROW,icol,dble((al_prodcurrpt(1,jj-1989)*primprod_percent(IBYR-1989)/al_mass_loss) * (1.0+al_non_met) *(1 - alumina_percent(IBYR-1989))*2.47),0,4)
			ELSEIF(jj.eq.IBYR .and. ISTEP.eq.5) then
				CALL cknum(I15,IROW,icol,dble(al_prodcurrpt(1,jj-1989)*(1-primprod_percent(IBYR-1989))),0,4)
			ELSE
				CALL cknum(I15,IROW,icol,dble(al_prodcurrpt(ISTEP,jj-1989)),0,4)      ! POT_2050NDIGIT 2 to 0
			ENDIF

            DO IF=1,IS_NUMTECH(13,istep)
			    if(jj.eq.ibyr) LINE= atechname(icount)
				icount=icount+1
                IROW = IROW + 1
                jcount = jcount + 1
                if(jj.eq.ibyr)CALL ckstr(I15,IROW,1,LINE,1,0,1)
        ...
        DO ISTEP=1,numstep
          if (IS_NUMTECH(10,istep).gt.0) then
            ...
            DO IF=1,IS_NUMTECH(10,istep)
               if(jj.eq.ibyr) LINE= gtechname(icount)
				icount=icount+1
                IROW = IROW + 1
                jcount = jcount + 1
                if(jj.eq.ibyr)CALL ckstr(I15,IROW,1,LINE,1,0,1)
        ...
        DO ISTEP=1,numstep+1 !add lime here
          if (IS_NUMTECH(11,istep).gt.0) then
            ...
            DO IF=1,IS_NUMTECH(11,istep)
               if(jj.eq.ibyr) LINE= ctechname(icount)
				icount=icount+1
                IROW = IROW + 1
                jcount = jcount + 1
                if(jj.eq.ibyr)CALL ckstr(I15,IROW,1,LINE,1,0,1)
...
! This subroutine is common to the paper and steel industries
      subroutine IAMatrix(IPRODX,irx)
      use i_
      implicit none

      REAL IPRODX(inumreg)               ! Regional production
...
! this will require some thinking for the exceptions. OK to leave as is?--maybe, if only process flow industries called
...
! Invert the input-output matrices.
      DO IS=1,MPASTP+1                                      !  keeping mpastp because it is dimension dependent Initialize inverted matrices as the identity matrix
        IAold_inv(is,is)=1.
      enddo
...
!****** Variables for EAF/BOF Adjustment ********
	  ...
	  REAL exp_factor						  ! this is exp(IS_ALPHA_FURNACE*(PriceDelta(7) - 1))
      ...
      !***** Calculate the Annualized Cost  *************************
        ...
        DO inumtech=1,2  ! EAF, BOF
			scale_comp = scale_fctr2(is_logit_coeff(inddir,:,stepnum),                 & ! Function to calculate the un-exponentiated logit component
                                ...
                                is_co2penalty(inddir,curiyr),           &
                                is_alpha_decay(inddir,:),                  &
                                is_alpha(inddir,stepnum,inumtech),logit_steam,obsoleteyr(inddir,stepnum,inumtech),stepnum,1,inumtech)
        ...
        DO inumtech=1,2  ! EAF, BOF
            logit_comp(inumtech) = logit_calc(is_logit_coeff(inddir,:,stepnum),                  &
                                          ...
                                          is_alpha_decay(inddir,:),               &
                                          is_alpha(inddir,stepnum,inumtech),      &
                                          scale_fac,logit_steam,obsoleteyr(inddir,stepnum,inumtech),stepnum,1,1)
        ENDDO
...
!		   PriceDelta(ifx) = AveFprc(irx,curiyr-prcprm,curiyr,ifx)   ! if this works, let's use (simplified via PGR 07/2022)
    	...
    	if (curcalyr.eq.ibyr) then                              ! Save the Lagged value of BOF_Fact last MECS year
           ...
           if (testyear(irx).eq.curcalyr) then
             if (BOF_Fact_Lag(irx).lt.0.10) then  ! BF/BOF percent cannot keep declining if below a minimum threshold
				BOF(irx) = BOF_Fact_Lag(irx)      ! this 0.10 should be dehardcoded as it represents a fixed minimum threshold for BF/BOF
				EAF(irx) = EAF_Fact_Lag(irx)
             else
			    continue
				exp_factor = max(0.25,1.0 - exp(IS_Alpha_Furnace(irx)*(PriceDelta(7) - 0.95))) 
				   if (curcalyr.gt.ibyr) then
				      BOF(irx) = BOF_Fact_Lag(irx) * (1.0 - 0.02 * (exp_factor))
                                                             ! the extra maximum decline due to increasing coal price
															 ! needs to be dehardcoded
                   else
				      BOF(irx) = BOF_Fact_Lag(irx) * 0.9825 ! BOF percentage must decline by the minimum
					                                      ! this minimum decline needs to be dehardcoded 
                   end if 
...
!             DRI_Tot(irx) = 0.0                                                 ! Initialize
             IF (curcalyr.ge.Phase1_YR(irx) .and. curcalyr.lt.Phase2_YR(irx)) DRI_Tot(irx) = DRI_Tot_Phase1(irx) * 1000.0   ! Specified initial capacity (convert to 1000 Tonnes)
             if (curcalyr.eq.Phase2_YR(irx)) DRI_Tot(irx) = DRI_Tot_Phase2(irx) * 1000.0
             if (curcalyr.gt.Phase2_YR(irx)) then
                continue
                if (DRI_Tot_Phase2(irx).eq.0.0) then ! if DRI production is zero from the last historical year then we expect none in the future
                   DRI_Tot(irx) = 0.0
                else
                   DRI_Tot(irx) = DRI_Tot(irx) * (2.0 - (BOF_Fact(irx)/BOF_Fact_Lag(irx))) ! YOY increment of DRI dependent on 
                                                                        				   ! YOY decrement of BF/BOF
!                   DRI_Tot(irx) = DRI_Tot(irx) ! test constant DRI in projections
                end if
             end if

		     testyear(irx) = curcalyr + 1
           end if ! avoid accumulation of iterations to results end of testyear loop.
...
!******** Calculate Coke PRODFLOW Elements **************
        ...
        if (curcalyr.eq.ibyr) Coke_Fact(irx) = Base_Coke(irx)
...
!******* Calculate regional shares of output by process step.  Used to allocate fuel demand later ***************
      ...
      DO IS=1,MPASTP
	   ...
	   !write (5525,*) 'produrrpt '
	   !write (5525,*) IS, prodcurrpt(IS,curiyr)
      ENDDO
...
!Calls: q_interp2
      ...
      do ii=2,12
        forecast=q_interp2(shrfinal(1),shrstart(1),shrfinal(ii),shrstart(ii),REAL(curcalyr))
...
! Calls: q_interp2
      ...
      do ii=1,4
        consumption(ii) =steel_output*cc_ngas(ii)/1000000.0
      enddo
...
! Calls: none
	  ...
	  REAL netcog  !net coke oven gas (COG) production
...
!***** Calculate Coke Production Required to Meet Demand **************
      ...
      !write (5525,*) 'Net_Coke Coke_Conversion DRI_BOF_TOT(curiyr)'
      !write (5525,*) Net_Coke, Coke_Conversion, DRI_BOF_TOT(curiyr)
...
!* Variables for Adjusting PRODFLOW Elements
    ...
    DATA WPrep_Initial / 1.075, 1.429, 1.818, &     ! Mechanical Pulp, Semi-Chemical Pulp, Kraft Pulp
                          2010,  2010,  2010 /
    DATA WPrep_Final / 1.053, 1.176, 1.818, &       ! Same order
                        2020,  2020,  2020 /


!******** Interpolate Wood Prep Factors between Initial and Final Values *************** Not sure I did this correctly; NSK: this is linearly EXTRAPOLATING after 2020; do we want that?
...
! Calls: none
    ...
    REAL paper_temp(5)
	integer prx,ii
	INTEGER pp_proxy_iyr
	PARAMETER (pp_proxy_iyr=31)	! 2020; the (index) year ironstlx.xlsx has paper share data out to
 ...
 ! fill in gap between read in data and ibyr if there is one
    ...
    IF (curiyr .gt. pp_proxy_iyr) THEN   ! For years after paper share data
        DO prx = 1,5
			Paper_Temp(prx) = PP_ProdNDX(prx,curiyr)*Paper_Share(prx,pp_proxy_iyr)          ! Calculate Un-normalized Shares
        ENDDO
        DO prx = 1,5
...
! Calls: q_interp2
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    NetSteamConv=max(0.0,netproc-HOGFSteam) ! KPE PAPER_STEAM_GRS(26,curiyr) NOT subtracted   no step 26 in netprc from conventional fuels GJ  (KPE more accurate nonhog steam)
! net steam is positive tdtdtdtd
	! interpolate chp shares
    do ii=1,17
     if ((ibyr).ge.curcalyr) then
  ...
  !  cogenx=abs(NetSteamConv)*share(2) !GJ   KPE confirmed -these two equations are fossil cogenx and boiler steam in GJ. 
     cogenx=NetSteamConv*share(2) !tdtd GJ   KPE confirmed -these two equations are fossil cogenx and boiler steam in GJ.  tdtdtdtdtd
 !   boilersx=abs(NetSteamConv)*share(1)  !GJ
    boilersx=NetSteamConv*share(1)  !GJ  
...
! all net hogfuel per documentation is chp, 	
        ...
        PP_ElecGen(4)=PP_ElecGen(4)+ABS(pp_stmfuel_chp(7,1)*HOGFSteam)  ! hog fuel renewables GJ electric/GJ steam tdtdtdtdtdamico
	
    ! what is left is for fossil fuels; elec is represented as a negative number in the input file so abs is used to avoid confusion
...
! Calls: none
    ...
    integer incap  !switch to use ibyr weighted average or ibyr2+2 weighted average
...
! used for paper
	   	 renewchpfac = 4.0    !6.190613661
		 natgaschpfac = 1.0 ! 4.82629345
...
!paper uses this
    ...
    if (curcalyr.eq.ibyr) then
         incap=1 !use ibyr weighted average
    else
         incap=2 !hold ibyr+1 weighted average constant after ibyr
    endif
...
common/ind_easy/INDETTCBUS, INDC4TCBUS, INDPRTCBUS, INDPPTCBUS, INDDFTCBUS, INDRFTCBUS, INDMGTCBUS, &
    ...
    Real INDOWICBUS(mnumyr)		! Industrial sector consumption of non-wood waste biomass energy (municipal solid waste)
    Real INDPCEPCONB(mnumyr)	! Electricity sector consumption of petcoke
	REAL INDPMTCPUS(MNUMYR)		! Marketeable petcoke (includes industrial and power sector, but not refining), million b/d
...
!      INDC3TCPUS = C3TCPUS*1000
	  ...
	  INDPMTCPUS=PMTCPUS*PCTCZUS*ZSAJQUS	! (million b/d)*(MMBtu/b)*(d/yr) gives tBtu
  RETURN
...
Subroutine Read_coal (YEARS_QCR, ANINPUTFILE, dataqcr, MNUMYRX, NUMINDX, IBYRX, ICURIYRXX, qcrcokeplntx, qcrindchpx, qcrindnchpx)
    
    Implicit None

    Integer, INTENT(IN) :: YEARS_QCR, MNUMYRX, NUMINDX, IBYRX, ICURIYRXX,  ANINPUTFILE
    Integer IYEARCOAL/0/, countx/0/, i/0/, j/0/			                        ! IYEAR, countx initialized to 0 using /<number>/
    Real*8  COKEPLNT, INDCHP, INDNCHP, FOOD, PAPER, BULKCHEM, GLASS, CEMENTLIME, IRONSTEEL, ALUMINUM, FABMETALS, MACHINERY, &
            COMPUTERS, TRANSEQUIP, ELECEQUIP, WOODPROD, PLASTIC, LTCHEM, OTHRNMM, OTHRPRIM, MISCFIN ! THESEse values are a mix of SEDS data, MER data, STEO projections, and estimates from real-world research
    Real, intent(out), dimension (YEARS_QCR) :: qcrcokeplntx            ! Coke Plants
    Real, intent(out), dimension (YEARS_QCR) :: qcrindchpx           ! Other Industrial CHP
    Real, intent(out), dimension (YEARS_QCR) :: qcrindnchpx           ! Other Industrial Non-CHP
    Real, intent(out), dimension (MNUMYRX,NUMINDX,5) :: dataqcr          ! array of data across IDM classifications and census regions
    
	Read(ANINPUTFILE, '()' )                          ! Skip first line (header)
    ...
    Do while (countx < YEARS_QCR)         				! Go from last year in MECS to last year in QCR
		
		Read(ANINPUTFILE,*) IYEARCOAL, COKEPLNT, INDCHP, INDNCHP, FOOD, PAPER, BULKCHEM, GLASS, CEMENTLIME, IRONSTEEL, ALUMINUM, FABMETALS,&
                            MACHINERY, COMPUTERS, TRANSEQUIP, ELECEQUIP, WOODPROD, PLASTIC, LTCHEM, OTHRNMM, OTHRPRIM, MISCFIN

        countx = IYEARCOAL-IBYRX+1  		! Go from start year (last MECS year) to last year (newest QCR year)

        qcrcokeplntx(COUNTX)=COKEPLNT                           ! Pass output values to WEXOG arrays
        qcrindchpx(COUNTX)=INDCHP
        qcrindnchpx(COUNTX)=INDNCHP
		dataqcr(countx+IBYRX-1989-1,7,5)=FOOD
		dataqcr(countx+IBYRX-1989-1,8,5)=PAPER
		dataqcr(countx+IBYRX-1989-1,9,5)=BULKCHEM
		dataqcr(countx+IBYRX-1989-1,10,5)=GLASS
		dataqcr(countx+IBYRX-1989-1,11,5)=CEMENTLIME
		dataqcr(countx+IBYRX-1989-1,12,5)=IRONSTEEL
		dataqcr(countx+IBYRX-1989-1,13,5)=ALUMINUM
        dataqcr(countx+IBYRX-1989-1,14,5)=FABMETALS
		dataqcr(countx+IBYRX-1989-1,15,5)=MACHINERY
        dataqcr(countx+IBYRX-1989-1,16,5)=COMPUTERS
		dataqcr(countx+IBYRX-1989-1,17,5)=TRANSEQUIP
        dataqcr(countx+IBYRX-1989-1,18,5)=ELECEQUIP
        dataqcr(countx+IBYRX-1989-1,19,5)=WOODPROD
        dataqcr(countx+IBYRX-1989-1,20,5)=PLASTIC
		dataqcr(countx+IBYRX-1989-1,21,5)=LTCHEM
		dataqcr(countx+IBYRX-1989-1,22,5)=OTHRNMM
		dataqcr(countx+IBYRX-1989-1,23,5)=OTHRPRIM
		dataqcr(countx+IBYRX-1989-1,24,5)=MISCFIN
    End do

End Subroutine Read_coal

Subroutine Read_electric (YEARS_ASM, ANINPUTFILE, dataasmx, MNUMYRX, NUMINDX, IBYRX, ICURIYRX)
    
    Implicit None

    Integer, INTENT(IN) :: YEARS_ASM, MNUMYRX, NUMINDX, IBYRX, ICURIYRX, ANINPUTFILE
    Integer IYEARELEC/0/, countx/0/, i/0/, j/0/	                        ! IYEAR, countx initialized to 0 using /<number>/
    Real*8  FOOD, PAPER, BULKCHEM, GLASS, CEMENTLIME, IRONSTEEL, ALUMINUM, &
	       FABMETALS, MACHINERY, COMPUTERS, TRANSEQUIP, ELECEQUIP, WOODPROD, &
            PLASTIC, LTCHEM, OTHRNMM, OTHRPRIM, MISCFIN       ! IDM industrial classifications
    Real, intent(out), dimension (MNUMYRX,NUMINDX,5) :: dataasmx          ! array of data across IDM classifications and census regions
    ...
    Do while (COUNTX < YEARS_ASM)         				! Go from last year in MECS to last year in QCR
		
		Read(ANINPUTFILE,*) IYEARELEC, FOOD, PAPER, BULKCHEM, GLASS, CEMENTLIME, IRONSTEEL, ALUMINUM, FABMETALS, MACHINERY, COMPUTERS, &
                            TRANSEQUIP, ELECEQUIP, WOODPROD, PLASTIC, LTCHEM, OTHRNMM, OTHRPRIM, MISCFIN

        COUNTX = IYEARELEC-ibyrx+1  		! Go from start year (currently 2006) to last year (currently newest SEDS year plus 4)
		dataasmx(countx+IBYRX-1989-1,7,5)=FOOD
		dataasmx(countx+IBYRX-1989-1,8,5)=PAPER
        dataasmx(countx+IBYRX-1989-1,9,5)=BULKCHEM
		dataasmx(countx+IBYRX-1989-1,10,5)=GLASS
		dataasmx(countx+IBYRX-1989-1,11,5)=CEMENTLIME
		dataasmx(countx+IBYRX-1989-1,12,5)=IRONSTEEL
		dataasmx(countx+IBYRX-1989-1,13,5)=ALUMINUM
		dataasmx(countx+IBYRX-1989-1,14,5)=FABMETALS
		dataasmx(countx+IBYRX-1989-1,15,5)=MACHINERY
		dataasmx(countx+IBYRX-1989-1,16,5)=COMPUTERS
		dataasmx(countx+IBYRX-1989-1,17,5)=TRANSEQUIP
		dataasmx(countx+IBYRX-1989-1,18,5)=ELECEQUIP
		dataasmx(countx+IBYRX-1989-1,19,5)=WOODPROD
		dataasmx(countx+IBYRX-1989-1,20,5)=PLASTIC
		dataasmx(countx+IBYRX-1989-1,21,5)=LTCHEM
		dataasmx(countx+IBYRX-1989-1,22,5)=OTHRNMM
		dataasmx(countx+IBYRX-1989-1,23,5)=OTHRPRIM
		dataasmx(countx+IBYRX-1989-1,24,5)=MISCFIN
	End do
	
End Subroutine Read_electric

Subroutine Read_carbshrca (CARBSHRCAINPUT, datacarbshrca, NUMINDX)
    
    Implicit None

    Integer, INTENT(IN) :: NUMINDX,  CARBSHRCAINPUT
    Integer i/0/		                        ! countx initialized to 0 using /<number>/
    Real*8  carbonshare(2*NUMINDX) 							! Values are to be California's reported GHG emissions
    Real, intent(out), dimension (NUMINDX) :: datacarbshrca          ! array of data across IDM classifications and constant conversions
    
	Read(CARBSHRCAINPUT, '()' )                          ! Skip first line (header)
	...
	Read(CARBSHRCAINPUT,*) carbonshare
	
	i=0
	Do while (i < NUMINDX)
		i = i + 1
		datacarbshrca(i) = carbonshare(2*i)
	End do
	
	
End Subroutine Read_carbshrca
...
! These are dummy variables whose values are passed to H2_chem_supply_reg, H2_chem_demand_reg, and H2_ref_supply_div, respectively
	...
	CHARACTER dum2
 ! base year H2 feedstock consumption for 6 chemical subsectors: industrial gases (really supply), other inorganics, petrochemicals,other organics, resins, and ag chemicals (H2_7x are refining)
	REAL H2_11,H2_12,H2_13,H2_14,H2_21,H2_22,H2_23,H2_24,H2_31,H2_32,H2_33,H2_34,H2_41,H2_42,H2_43,H2_44, &
			H2_51,H2_52,H2_53,H2_54,H2_61,H2_62,H2_63,H2_64,H2_71,H2_72,H2_73,H2_74,H2_75,H2_76,H2_77,H2_78,H2_79
	REAL, intent(out), dimension (6,4) :: H2_chem_supply_regx  	! H2 produced by industrial chemicals subsector (first index) and census region (second index)
	REAL, intent(out), dimension (6,4) :: H2_chem_demand_regx  	! H2 consumed by industrial chemicals subsector (first index) and census region (second index)
    REAL, intent(out), dimension (MNUMCR) :: H2_ref_supply_divx	! H2 produced by refineries, by census division
	
	READ(IUNITH2, '()' )                          ! Skip first line (header)
	  
    DO WHILE (countx < 2)        ! Two lines of data (as of AEO2025)
		Read(IUNITH2,*) dum1, dum2, H2_11,H2_12,H2_13,H2_14,H2_21,H2_22,H2_23,H2_24,H2_31,H2_32,H2_33,H2_34,H2_41,H2_42,H2_43,H2_44, &
							H2_51,H2_52,H2_53,H2_54,H2_61,H2_62,H2_63,H2_64,H2_71,H2_72,H2_73,H2_74,H2_75,H2_76,H2_77,H2_78,H2_79
 
		countx = countx+1
		
		IF (countx.eq.1) THEN	! base year production in the chemical subsectors and refineries
			H2_chem_supply_regx(1,:)=(/H2_11,H2_12,H2_13,H2_14/)	! industrial gases
			H2_chem_supply_regx(2,:)=(/H2_21,H2_22,H2_23,H2_24/)	! other inorganics
			H2_chem_supply_regx(3,:)=(/H2_31,H2_32,H2_33,H2_34/)	! petrochemicals
			H2_chem_supply_regx(4,:)=(/H2_41,H2_42,H2_43,H2_44/)	! other organics
			H2_chem_supply_regx(5,:)=(/H2_51,H2_52,H2_53,H2_54/)	! resins
			H2_chem_supply_regx(6,:)=(/H2_61,H2_62,H2_63,H2_64/)	! ag chemicals
			H2_ref_supply_divx(1:9)=(/H2_71,H2_72,H2_73,H2_74,H2_75,H2_76,H2_77,H2_78,H2_79/)	! Refining base year SMR H2 production
		ELSEIF (countx.eq.2) THEN ! base year production in the chemical subsectors
			H2_chem_demand_regx(1,:)=(/H2_11,H2_12,H2_13,H2_14/)	! industrial gases
			H2_chem_demand_regx(2,:)=(/H2_21,H2_22,H2_23,H2_24/)	! other inorganics
			H2_chem_demand_regx(3,:)=(/H2_31,H2_32,H2_33,H2_34/)	! petrochemicals
			H2_chem_demand_regx(4,:)=(/H2_41,H2_42,H2_43,H2_44/)	! other organics
			H2_chem_demand_regx(5,:)=(/H2_51,H2_52,H2_53,H2_54/)	! resins
			H2_chem_demand_regx(6,:)=(/H2_61,H2_62,H2_63,H2_64/)	! ag chemicals
		ENDIF
	ENDDO
...
!bench one year after MECS using MECS actual data from ironstlx and MECS year model results
    use i_
    IMPLICIT NONE
    real con(NUM_CON_FUELS,5)
    real mecsbench(10)
    integer ii,iff,ifuel

     ! this if block looks OK.  it computes the mecsbench factors in ibyr+1 as the ratio of 
	 ! mecs data to the con variables-- then recomputes the con variables in ibyr+1
	 ! the problem is after ibyr+1 (ibyr2+2)-- not the code, the 
	if ((CURCALYR==(ibyr+1)).and.(curitr.eq.1)) then
        ...
        if (con(3,indreg).ne.0.0) mecsbench(4)=MECS_Data(inddir,INDREG,4)/CON(3,indreg)     !steam coal
        if (con(7,indreg).ne.0.0) mecsbench(5)=MECS_Data(inddir,INDREG,5)/CON(7,indreg)     !distillate
        if (con(8,indreg).ne.0.0) mecsbench(6)=MECS_Data(inddir,INDREG,6)/CON(8,indreg)     !propane
        if (con(11,indreg).ne.0.0) mecsbench(7)=MECS_Data(inddir,INDREG,7)/CON(11,indreg)   !pet coke
        if (con(18,indreg).ne.0.0) mecsbench(8)=MECS_Data(inddir,INDREG,8)/CON(18,indreg)   !renewables
        if (con(4,indreg) .ne.0.0) mecsbench(9)=MECS_Data(inddir,INDREG,9)/CON(4,indreg)    !met coal
        if (con(15,indreg).ne.0.0) mecsbench(10)=MECS_Data(inddir,INDREG,10)/CON(15,indreg) !other petroleum          
        ...
        con(15,indreg)=con(15,indreg)*mecsbench(10)   !other petroleum
        ...
        if (indreg.eq.4) then !retally totals
           ...
           con(15,5)=SUM(con(15,1:4))                 !other petroleum
...
380			CONTINUE
	...
	!the next function maps regions to divisions
	! stolem from mecsless860
	
!	FUNCTION reg2div(H, Q)
!    use i_
!	implicit none
!	integer H, Q
!	integer divs(2)
!! Establish the range of census divisions for the current region using an innovative approach works because integer division means numbers are truncated
!      H=areg*2+areg/4-1  ! obviously, L will be 1,3,5, and 8 for areg=1..4
!      Q=areg*2+areg/3    !            M will be 2,4,7, and 9 for areg=1..4
!	  
!	  reg2div = [H, Q]
!	
!	return
!	END function reg2div