...
*  BrzEthExpNonUs(Period)                                         M BBLs per day of ethanol exported from Brazil to satisfy non-US demand
   ...
   WACFS_Pet_to_Bio(WACFS_PetCategory,Period)                         WACFS Transfer Petroleum Deficit to Bio-Liquid Relief
   WACFS_Pet_Cst(WACFS_PetCategory,Period)                            WACFS Cost For Petroleum Deficit in Price Period only
   WACFS_Bio_Sub(WACFS_PetCategory,Period)                            WACFS Subsidy For Bio-Liquid Relief in Price Period only
   WACFSSafety(WACFS_PetCategory,Period)
   WACFS_LandfillGas(WACFS_PetCategory,Period)                        WACFS Landfill Gas Demand
   WACFS_AltVehicles(WACFS_PetCategory,WACFS_Vehicle_Types,Period)      WACFS Alternate Vehicle Demand
   WACFS_landfillGas(WACFS_PetCategory,Period)                        WACFS landfill gas allowed to go to either PetCategory
...
*   UtilSupCurve(RefReg,Utility,Step,Period)                         Enforce bounds on ethanol import supply steps
   ...
   WACFS_Petroleum(CFP_PetCategory,Period)                          WACFS Constraint - Petroleum Deficit
   WACFS_BioFuel(CFP_PetCategory,Period)                            WACFS Constraint - BioFuel Relieve
   WACFS_LandfillGasConstr(Period)                                  WACFS Constraint - allow landfill gas credit to both PetCategories
...
*  npv_EthBrazilPrc(EthStream,Step,ActivePeriod)*ETH_BRAZIL(EthStream,Step,ActivePeriod)  ) +
  ...
  sum((WACFS_PetCategory,PrcPeriod(ActivePeriod)), npv_WACFS_Price(WACFS_PetCategory,ActivePeriod) * WACFS_Pet_CST(WACFS_PetCategory,PrcPeriod) ) +
  sum((WACFS_PetCategory,PrcPeriod(ActivePeriod)), -npv_WACFS_Price(WACFS_PetCategory,ActivePeriod) * WACFS_Bio_Sub(WACFS_PetCategory,PrcPeriod) ) +
...
CFP_Petroleum(CFP_PetCategory,ActivePeriod)..
   ...
   sum((CFP_RefReg,RefType,CFP_PetStreams)$(CFP_PetStreams('DSUout')),
     ...
     RecipetoProd(CFP_RefReg,RefType,CFP_PetStreams,ActivePeriod)) -
   ...
   sum((RefRegA,CFP_RefReg,TranMode,CFP_PetStreams)$((REFtoREFTranCost(RefRegA,CFP_RefReg,TranMode,CFP_PetStreams)) and (CFP_PetStreams('DSUout'))),
     npv_CFP_Pet_CFactor(CFP_PetCategory,CFP_RefReg,CFP_PetStreams,ActivePeriod) *
      RefRefTRAN(RefRegA,CFP_RefReg,TranMode,CFP_PetStreams,ActivePeriod)) -
   ...
   sum((CFP_RefReg,CFP_PetStreams)$(CFP_PetStreams('DSUout')),
     npv_CFP_Pet_CFactor(CFP_PetCategory,CFP_RefReg,CFP_PetStreams,ActivePeriod) *
      IMPORTS(CFP_PetStreams,CFP_RefReg,ActivePeriod)) -
...
CFP_BioFuel(CFP_PetCategory,ActivePeriod)..
   ...
   sum(SupTypMode(CFP_RefReg,RefType,CFP_BioProcess,CFP_BioMode),
    ...
    ProcMode(CFP_RefReg,RefType,CFP_BioProcess,CFP_BioMode,ActivePeriod)) -
   sum((RefRegA,CFP_RefReg,TranMode,CFP_BioStreams(EthStream))$REFtoREFTranCost(RefRegA,CFP_RefReg,TranMode,CFP_BioStreams),
     npv_CFP_Eth_CFactor(CFP_PetCategory,CFP_BioStreams,ActivePeriod) *
      RefRefTRAN(RefRegA,CFP_RefReg,TranMode,CFP_BioStreams,ActivePeriod)) +
   sum((RefRegA,CFP_RefReg,TranMode,CFP_BioStreams(EthStream))$REFtoREFTranCost(CFP_RefReg,RefRegA,TranMode,CFP_BioStreams),
     npv_CFP_Eth_CFactor(CFP_PetCategory,CFP_BioStreams,ActivePeriod) *
     RefRefTRAN(CFP_RefReg,RefRegA,TranMode,CFP_BioStreams,ActivePeriod)) -
   ...
   sum((CFP_RefReg,RefType),
     npv_CFP_BiodImp_CFactor(CFP_PetCategory,CFP_RefReg,ActivePeriod) *
      BIODIMPref(CFP_RefReg,RefType,ActivePeriod)) -

   sum((CFP_RefReg,RefType),
     npv_CFP_RenewDImp_CFactor(CFP_PetCategory,CFP_RefReg,ActivePeriod) *
      RENEWDIMPref(CFP_RefReg,RefType,ActivePeriod)) -
...
* esh: adding in WACFS constraints
* structured the same way as LCFS constraints
*  For ActivePeriod 1 assin WACFS Price from Previous Forecast Year Solution - Else Transfer from Petroleum Deficit to BioFuel Relieve
WACFS_Petroleum(CFP_PetCategory,ActivePeriod)..
   CFP_Pet_to_Bio(CFP_PetCategory,ActivePeriod) +
   sum(PrcPeriod(ActivePeriod),CFP_Pet_Cst(CFP_PetCategory,PrcPeriod)) -
   sum((CFP_RefReg,RefType,CFP_PetStreams)$(CFP_PetStreams('DSUout')),
     npv_CFP_Pet_CFactor(CFP_PetCategory,CFP_RefReg,CFP_PetStreams,ActivePeriod) *
...
WACFS_BioFuel(CFP_PetCategory,ActivePeriod)..
   CFP_Pet_to_Bio(CFP_PetCategory,ActivePeriod) +
   sum(PrcPeriod(ActivePeriod),CFP_Pet_Cst(CFP_PetCategory,PrcPeriod)) -
   sum((CFP_RefReg,RefType,CFP_PetStreams)$(CFP_PetStreams('DSUout')),
     npv_CFP_Pet_CFactor(CFP_PetCategory,CFP_RefReg,CFP_PetStreams,ActivePeriod) *
...
*  such that total in each year/period is not exceeded
*  trillion BTU per year
WACFS_LandfillGasConstr(Period)..
   sum(WACFS_PetCategory, WACFS_LandfillGas(WACFS_PetCategory,Period)) -
   npv_WACFS_LandfillGas_total(Period)
   =L= 0;
