...
*   Check if the file LFMM_report.gdx exists and if it does, load it
    ...
    if(errorlevel=0,
      ...
      execute_load   repActivity, repUtilization,
        ...
        repRefFuelUse, repRefElecPurch, repTotEnergyUse, repRefH2Use, repRFSCredits, repCO2Emissions,
...
*   Coal total, Ethanol plants
    ...
    WRENEW_QCLETH(MNUMYR,'11_United_States')   =            124.711 ;

    WRENEW_QCLETH(MNUMYR,'03_East_North_Central')   =          34.919079  ;
    WRENEW_QCLETH(MNUMYR,'04_West_North_Central')   =          89.791924  ;
    WRENEW_QCLETH(MNUMYR,'06_East_South_Central')   =          0  ;
    WRENEW_QCLETH(MNUMYR,'07_West_South_Central')   =          0 ;
    WRENEW_QCLETH(MNUMYR,'08_Mountain')   =                    0 ;
    WRENEW_QCLETH(MNUMYR,'09_Pacific')   =                     0 ;
...
* Dump cat coke into QPCRF for now, coker coke cannot be burned for fuel in the LFMM
    ...
    QMORE_QH2RF(MNUMCR,MNUMYR) =
      sum((DomRefReg,RefType,RefInputStr('H2'),ActiveDem)$crN2L(ActiveDem,MNUMCR),
         RefReg_to_CenDiv_ACU_Frac(DomRefReg,ActiveDem)*REFPURused.l(DomRefReg,RefType,RefInputStr,Period)* CONVFACT_CFRSQ * 365 / 1000) ;

        QMORE_QH2RF('11_United_States',MNUMYR)   =
          sum((ActiveDem,MNUMCR)$crN2L(ActiveDem,MNUMCR), QMORE_QH2RF(MNUMCR,MNUMYR) ) ;
...
* CCATS co2 volumes from ethanol production eligible for tax credit
CCATSDAT_SUP_ETH_45Q(MNUMCR,MNUMYR) =
      sum((ActiveDem)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)), CCATSDAT_SUP_ETH_45Q_lfmm(ActiveDem,t)) ;
*    CCATSDAT_SUP_ETH_45Q(ActiveDem,t)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)) =  CCATSDAT_SUP_ETH_45Q(MNUMCR,MNUMYR) ;
*    CCATSDAT_SUP_ETH_45Q(MNUMCR,MNUMYR) = CCATSDAT_SUP_ETH_45Q(MNUMCR,MNUMYR) ;

* CCATS co2 volumes from ethanol production not eligible for tax credit
CCATSDAT_SUP_ETH_NTC(MNUMCR,MNUMYR) =
      sum((ActiveDem)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)), CCATSDAT_SUP_ETH_NTC_lfmm(ActiveDem,t)) ;
*    CCATSDAT_SUP_ETH_NTC(ActiveDem,t)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)) =  CCATSDAT_SUP_ETH_NTC(MNUMCR,MNUMYR) ;
*    CCATSDAT_SUP_ETH_NTC(MNUMCR,MNUMYR) = CCATSDAT_SUP_ETH_NTC(MNUMCR,MNUMYR)  ;
*      sum((MNUMPR,DomRefReg,ActiveDem,ProcessMode,Process,RefType)$(RefReg2MNUMPR(MNUMPR,DomRefReg) and (ord(MNUMPR)<9) and crN2L(ActiveDem,MNUMCR)),
*         ProcessTable('CO2','ETHCCS',ProcessMode)*PROCMODE.l(DomRefReg,RefType,Process,ProcessMode,Period) );

**CCATS fixed investment and variable O&M costs for carbon capture from ethanol retrofits, $/tonne CO2
CCATSDAT_CST_ETH_INV(MNUMCR,MNUMYR) =
      sum((ActiveDem)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)), CCATSDAT_CST_ETH_INV_lfmm(ActiveDem,t)) ;

CCATSDAT_CST_ETH_OM(MNUMCR,MNUMYR) =
      sum((ActiveDem)$(crN2L(ActiveDem,MNUMCR) and tMNUM(t,MNUMYR)), CCATSDAT_CST_ETH_OM_lfmm(ActiveDem,t)) ;
...
* 2/18/20: adding LFMMOUT_RenewDIMP and LFMMOUT_RenewDImpPD
    ...
    LFMMOUT_RenewDEXP(MNUMCR,MNUMYR)             = 0.0 ;
    LFMMOUT_RenewDEXP('11_United_States',MNUMYR) =
      sum((Step), RENEWDEXP.l(Step,Period) ) ;

    PMMRPT_BIODEXP(MNUMCR,MNUMYR) = 0 ;
    PMMRPT_BIODEXP('11_United_States',MNUMYR) =
      sum((Step), BIODEXP.l(Step,Period) ) ;
...
* TEMP TEMP TEMP TEMP
* for accounting, now add hydrogen purch from market( Bill bfoe/day ) to NG,H2 to refinery (RFHCXH2IN, MMbfoe/cd)

    PMMFTAB_RFHCXH2IN(MNUMPR,MNUMYR) = PMMFTAB_RFHCXH2IN(MNUMPR,MNUMYR) +
      sum((DomRefReg,RefType,RefInputStr('H2'))$(RefReg2MNUMPR(MNUMPR,DomRefReg)),REFPURused.l(DomRefReg,RefType,RefInputStr,Period)) /1000. ;
...
*SEW hard code for coal use in ethanol plants
    LFMMOUT_REF_UTL('48_M57',MNUMPR,MNUMYR) =  1 ;

* Zero out old PMM variables
*== PMMRPT_RFDSTCAP(MNUMPR,MNUMYR) = 0 ;
*== PMMRPT_RFDSTUTL(MNUMPR,MNUMYR) = 0 ;

* Report process unit capacity builds for MAM job module
    LFMMOUT_BUILDS(REFREG,M57,MNUMYR)= 0;
    LFMMOUT_BUILDS(REFREG,M57,MNUMYR) = sum((RefType,Process)$M57_2_Process(M57,Process),PriorBuilds(RefReg,RefType,Process,t));
...
repRefH2Use(RefReg,RefType,Period,RunYears) = 0 ;
repRefH2Use(RefReg,RefType,Period,RunYears) =
    sum(RefInputStr('H2'), REFPURused.l(RefReg,RefType,RefInputStr,Period) )* CONVFACT_CFRSQ * 365 / 1000.0 ;
...
Loop(MNUMYR,
  ...
  Loop(MNUMCR,
   ...
   CheckValue(LFMMOUT_RenewDEXP(MNUMCR,MNUMYR));
   CheckValue(PMMRPT_BIODIMP(MNUMCR,MNUMYR));
   CheckValue(PMMRPT_BIODPRICE(MNUMCR,MNUMYR));
   CheckValue(PMMRPT_CLLETHCD(MNUMCR,MNUMYR));

   CheckValue(CCATSDAT_SUP_ETH_45Q(MNUMCR,MNUMYR));
   CheckValue(CCATSDAT_SUP_ETH_NTC(MNUMCR,MNUMYR));
   CheckValue(CCATSDAT_CST_ETH_INV(MNUMCR,MNUMYR));
   CheckValue(CCATSDAT_CST_ETH_OM(MNUMCR,MNUMYR));
   ...
   CheckValue(QMORE_QH2RF(MNUMCR,MNUMYR));
   ...
   Loop(M57,
    Loop(REFREG,
         CheckValue(LFMMOUT_BUILDS(REFREG,M57,MNUMYR));
    );
   );
);/* End_of MNUMYR_loop */
...
Execute_Unload 'LFMM_to_NEMS.gdx',
   ...
   QMORE_QPRRF, QMORE_QBURF, QMORE_QH2RF, QMORE_QISRF, QMORE_QPYRF,
   ...
   LFMMOUT_BIODIMPPD, LFMMOUT_RenewDIMP, LFMMOUT_RenewDImpPD, LFMMOUT_RenewDEXP, LFMMOUT_BIODEXPPD,
   ...
   LFMMOUT_RFSDSRS, LFMMOUT_REF_CAP, LFMMOUT_REF_UTL, LFMMOUT_BUILDS,
   ...
   CCATSDAT_SUP_ETH_45Q,CCATSDAT_SUP_ETH_NTC, CCATSDAT_CST_ETH_INV, CCATSDAT_CST_ETH_OM ;
...
* Keep this code, though, in case lfmm_p.gdx changes in the future
   ...
   repRefRefTranModeCapUtz,
   repREFCDTranCap, repRefCDTranMode, repRefCDTranModeCapUtz,
   repRefFuelUse, repRefH2Use, repRefElecPurch, repTotEnergyUse, repRFSCredits, repCO2Emissions,
   repTotalCO2, repMargCrdPrice, repCrudeUse, repCrudeRAC, repSpecProdPrice,
