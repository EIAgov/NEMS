...
def write_aimms_input(my_module, pyfiler, my_var, base_year):
    """Write AIMMS input file comprising multiple composite tables.
    ...
    # TODO: only if parquet switch is ON ?

    #write_parquet = False
    if my_module == "hmm":
        os.mkdir(f"toAIMMS/GlobalDataToAIMMS_{str(y)}_{str(iter)}")
        try:
            os.mkdir(f"toAIMMS/GlobalDataToAIMMS_{str(y)}_{str(iter)}")
        except:
            pass

        # TODO: delete old parquet files?
        ...
        for k, v in my_var.items():
            ...
            if k == 'COALEMM.EMM_CL_UNITS':
                ...
                my_df = pd.DataFrame.from_dict(temp)
                ...
                # Filter to only MNUMYR NEMS is running in if MNUMYR is one of the indexes
                if (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v):
                    L = [[y_int, x[y_int-1]]]
                ...
                # Filter to only MNUMYR NEMS is running in if MNUMYR is the last index
                if (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v) and ("MNUMYR" == v[-1]):
                    L = [[i1 + 1, y_int , x[i1, y_int-1]]
                         for i1 in range(x.shape[0])]
                    my_df = pd.DataFrame(L)
                ...
                if (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v) and ("MNUMYR" == v[-1]):
                    L = [[i1 + 1, i2 + 1, y_int, x[i1, i2, y_int-1]]
                         for i1 in range(x.shape[0])
                         for i2 in range(x.shape[1])]
                    my_df = pd.DataFrame(L)
                ...
                if (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v) and ("MNUMYR" == v[-1]):
                    L = [[i1 + 1, i2 + 1, i3 + 1, y_int, x[i1, i2, i3, y_int-1]]
                         for i1 in range(x.shape[0])
                         for i2 in range(x.shape[1])
                         for i3 in range(x.shape[2])]
                    my_df = pd.DataFrame(L)
                    ...
                    # Special handling for QELGEN_VINT since it is large, but does not have MNUMYR as the last index
                elif (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v) and ("QELGEN_VINT" in k):
                    L = [[i1 + 1, i2 + 1, y_int, i4 + 1, x[i1, i2, y_int - 1, i4]]
                         for i1 in range(x.shape[0])
                         for i2 in range(x.shape[1])
                ...
                if (my_module == "coal" or my_module == "hmm") and (y > base_year) and ("MNUMYR" in v) and ("MNUMYR" == v[-1]):
                    L = [[i1 + 1, i2 + 1, i3 + 1, i4 + 1, y_int, x[i1, i2, i3, i4, y_int-1]]
                         for i1 in range(x.shape[0])
                         for i2 in range(x.shape[1])
                         for i3 in range(x.shape[2])
                         for i4 in range(x.shape[3])]
                    ...
                    my_df = my_df.sort_values(
                        [my_df.columns[4], my_df.columns[3], my_df.columns[2], my_df.columns[1], my_df.columns[0]])
                    my_df.columns = v + [k.replace(".", "_")]
                else:
            ...
            if not (my_table == "nil"):
                my_df.to_parquet(f"toAIMMS/GlobalDataToAIMMS_{y}_{iter}/{my_table}.parquet")

    # end of "if write_parquet"
    # TODO: check parquet files for nans and nulls
    ...
    if my_module != "hmm":
        ...
        for k, v in my_var.items():
            ...
            if k == 'COALEMM.EMM_CL_UNITS':
                ...
                f.write("\nCOMPOSITE TABLE:\n")
                temp = {my_prefix[my_module] + 'M1': [1], my_prefix[my_module] + my_col: [str(x, encoding='utf-8').strip()]}
                my_col = my_prefix[my_module] + my_col
                temp[my_col][0] = '"' + temp[my_col][0] + '"'
                ...
                f.write("\nCOMPOSITE TABLE:\n")
                if "CONVFACT" in k:
                    temp = {my_prefix[my_module] + 'M1': [1], my_prefix[my_module] + my_col: [float(x)]}
                else:
                    temp = {my_prefix[my_module] + 'M1': [1], my_prefix[my_module] + my_col: [int(x)]}
                f.write(tabulate(temp, v + [my_prefix[my_module] + k.replace(".", "_")], tablefmt="plain"))
                f.write('\n;')
                ...
                if (my_module == "coal" or my_module == "hmm" or my_module == "ngas") and (y > base_year) and ("MNUMYR" in my_df.columns):
                    my_df = my_df[my_df["MNUMYR"] == y_int]

                my_df.columns = my_prefix[my_module] + my_df.columns
                f.write("\nCOMPOSITE TABLE:\n")
                ...
                # Filter to only MNUMYR NEMS is running in if MNUMYR is the last index
                if (my_module == "coal" or my_module == "hmm" or my_module == "ngas") and (y > base_year) and ("MNUMYR" in v) and ("MNUMYR" == v[-1]):
                    L = [[i1 + 1, y_int , x[i1, y_int-1]]
                         for i1 in range(x.shape[0])]
                    my_df = pd.DataFrame(L)
                    my_df = my_df.sort_values([my_df.columns[1], my_df.columns[0]])
