...
The optimization operates as follows:
    ...
    1. :meth:`~opmodels.ccats_optimization.OptimizationModel.setup` is called from :ref:`module`.Optimization specific options are set based on inputs
       from *opt_setup.csv*.

    2. :meth:`~opmodels.ccats_optimization.OptimizationModel.run` is called from :ref:`module`.
...
* :meth:`~opmodels.ccats_optimization.OptimizationModel.__init__` - Constructor to initialize Class (called by :meth:`module.Module.setup`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.setup` - Setup function using *opt_setup.csv* (called by :meth:`module.Module.setup`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.run` - runs main model functions in :class:`~opmodels.ccats_optimization.OptimizationModel` (called by :meth:`module.Module.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo` - creates the pyomo model (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.prepare_data_for_blocks` - creates dictionaries of data indexed by block (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_blocks` - calls most declare_* functions to set-up the optimization problem, assigning data by block (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_sets` - creates sets (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_parameters` - creates parameters (variables held fixed) in Pyomo (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_variables` - creates variables in Pyomo (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_constraints` - creates constraints in Pyomo (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_objective` - creates objective function for a single block (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.instantiate_pyomo`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_constraints_across_blocks` - creates constraints across blocks (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.declare_objective_across_blocks` - creates multi-block objective (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.solve_optimization` - solve the optimization problem (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)
* :meth:`~opmodels.ccats_optimization.OptimizationModel.solve_linearized_optimization` - re-solve the optimization as a linear problem to get duals, if originally solved non-linear (called by :meth:`~opmodels.ccats_optimization.OptimizationModel.run`)

CCATS Optimization: Output Debug Files
______________________________________
    
* **ccats_main_opt_log.log** - Optimization log file
...
class OptimizationModel(subccats.Submodule):
    ...
    def setup(self, setup_filename):
        ...
        self.debug_opt_log : Boolean
            If True, output solver status to nohup (integrated runs only) and ccats_main_opt_log.log (standalone & integrated runs)

        self.xpress_path : str
            Path that specifies the location of the xpress solver license 
        ...
        # Read in switches/paths/options
        self.threads                 = int(self.parent.threads)                                       # number of threads for solving
        self.xpress_path             = str(self.setup_table.at['xpress_path', 'value'])               # path to xpress license 
        self.soltimelimit            = int(self.setup_table.at['soltimelimit', 'value'])              # solve time limit
        self.maxmemorysoft           = int(self.setup_table.at['maxmemorysoft', 'value'])             # MB available for solve
        self.optimalitytol           = float(self.setup_table.at['optimalitytol', 'value'])           # sets the relative precision for objective optimality
        self.feastol                 = float(self.setup_table.at['feastol', 'value'])                 # sets the maximum deviation for constraints feasibility
        self.opt_check_solver_status = self.setup_table.at['opt_check_solver_status', 'value'].upper() == 'True'.upper() # check the solver status
        self.troubleshoot_datatypes  = self.setup_table.at['troubleshoot_datatypes', 'value'].upper() == 'True'.upper()  # log datatypes to assist with troubleshooting
        self.debug_opt_log           = self.setup_table.at['debug_opt_log', 'value'].upper() == 'True'.upper()           # Output solver status to nohup (integrated runs only) and ccats_main_opt_log.log (standalone & integrated runs)
    ...
    def prepare_data_for_blocks(self):
        ...
        ### Endogenized sources of CO2 (source): parameters that vary by block
        
        self.m.cost_invest_source =       {0: self.preproc.cost_invest_source_b0.copy(),
                                           1: self.preproc.cost_invest_source_b1.copy(),
                                           2: self.preproc.cost_invest_source_b2.copy()}
        
        self.m.cost_variable_source =     {0: self.preproc.cost_variable_source_b0.copy(),
                                           1: self.preproc.cost_variable_source_b1.copy(),
                                           2: self.preproc.cost_variable_source_b2.copy()}
        
        self.m.build_limit_source_node =  {0: self.preproc.build_limit_source_node_b0.copy(),
                                           1: self.preproc.build_limit_source_node_b1.copy(),
                                           2: self.preproc.build_limit_source_node_b2.copy()}
        
        self.m.build_limit_source_total = {0: self.preproc.build_limit_source_total_b0,
                                           1: self.preproc.build_limit_source_total_b1,
                                           2: self.preproc.build_limit_source_total_b2}
        
        self.m.discount_invest_source   = {0: self.preproc.discount_invest_source_b0,
                                           1: self.preproc.discount_invest_source_b1,
                                           2: self.preproc.discount_invest_source_b2}
        
        self.m.policy_eligibility_source_exist = {0: self.preproc.policy_eligibility_source_exist_b0,
                                                  1: self.preproc.policy_eligibility_source_exist_b1,
                                                  2: self.preproc.policy_eligibility_source_exist_b2}
        
        self.m.policy_eligibility_source_new = {0: self.preproc.policy_eligibility_source_new_b0,
                                                1: self.preproc.policy_eligibility_source_new_b1,
                                                2: self.preproc.policy_eligibility_source_new_b2}
    ...
    def instantiate_blocks(self, mb, t):
        ...
        # Endogenous sources of CO2 (source)
        data.nodes_source                    = self.preproc.nodes_source
        data.kinds_source                    = self.preproc.kinds_source
        data.annual_capacity_exist_source    = self.preproc.annual_capacity_exist_source
        data.cost_invest_source              = self.m.cost_invest_source[self.t]
        data.cost_variable_source            = self.m.cost_variable_source[self.t]
        data.build_limit_source_node         = self.m.build_limit_source_node[self.t]
        data.build_limit_source_total        = self.m.build_limit_source_total[self.t]
        data.discount_invest_source          = self.m.discount_invest_source[self.t]
        data.policy_eligibility_source_exist = self.m.policy_eligibility_source_exist[self.t]   
        data.policy_eligibility_source_new   = self.m.policy_eligibility_source_new[self.t]
        ...
        if self.troubleshoot_datatypes:
            ...
            attributes = ['co2_supply','co2_demand', 'co2_demand_cost_net',
                          ...
                          'policy_cost',
                          'annual_capacity_exist_source', 
                          'cost_invest_source', 'cost_variable_source', 
                          'build_limit_source_node', 'build_limit_source_total', 'discount_invest_source',
                          'policy_eligibility_source_exist','policy_eligibility_source_new']
            ...
            for key in d.keys():
                ...
                try:
                    ...
                    if x.index.nunique() != len(x):
                        print(key + ' has a nonunique index!')
                    if x.isna().any():
                        print(key + ' contains na values!')
                    if x.isnull().any():
                        print(key + ' contains null values!')
                except:
                    print('Could not check:' + str(key))
    ...
    def declare_sets(self, data):
        ...
        ### Nodes
        # Declare nodes
        self.b.s_nodes_supply = pyo.Set(initialize=sorted(data.nodes_supply)) # Exogenous to CCATS
        self.b.s_nodes_source = pyo.Set(initialize=sorted(data.nodes_source)) # Endogenous to CCATS
        self.b.s_nodes_trans_ship = pyo.Set(initialize=sorted(data.nodes_trans_ship))
        self.b.s_nodes_demand = pyo.Set(initialize=sorted(data.nodes_demand))
        self.b.s_nodes_sequester = pyo.Set(initialize=sorted(data.nodes_sequester))
        self.b.s_nodes_storage = pyo.Set(initialize=sorted(data.nodes_storage))
        self.b.s_nodes = pyo.Set(initialize=sorted(data.nodes))
        ...
        self.b.s_arcs = pyo.Set(initialize=tuple(sorted(self.b.s_arcs)))

        # Declare transport options
        transport_options = data.pipeline_lookup_opt['pipe_segment'].unique().tolist()
        self.b.s_transport_options = pyo.Set(initialize=sorted(transport_options))

        # Declare policy options
        policy_options = [0,1,2]
        self.b.s_policy_options = pyo.Set(initialize=sorted(policy_options))

        ## Sources (Endogenous)
        # Source technologies (Endogenized CO2 sources)
        self.b.s_kinds_source = pyo.Set(initialize=data.kinds_source)
        # Source vintages
        self.b.s_vintages_source = pyo.Set(initialize=data.vintages_source)
    ...
    def declare_parameters(self, data):
        ...
        ### Instantiate source parameters
        self.b.p_annual_capacity_exist_source       = pyo.Param(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_vintages_source, initialize=data.annual_capacity_exist_source, mutable=True, default=0.0)
        self.b.p_total_capacity_exist_source        = pyo.Param(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_vintages_source, initialize=data.total_capacity_exist_source, mutable=True, default=0.0)
        self.b.p_policy_eligibility_source_exist    = pyo.Param(self.b.s_kinds_source, self.b.s_policy_options, self.b.s_vintages_source, initialize=data.policy_eligibility_source_exist, mutable=True, default=0)
        self.b.p_policy_eligibility_source_new      = pyo.Param(self.b.s_kinds_source, self.b.s_policy_options, initialize=data.policy_eligibility_source_new, mutable=True, default=0)
        self.b.p_cost_invest_source                 = pyo.Param(self.b.s_kinds_source, self.b.s_nodes_source,initialize=data.cost_invest_source, mutable=True, default=9E14)
        self.b.p_cost_variable_source               = pyo.Param(self.b.s_kinds_source, self.b.s_nodes_source, initialize=data.cost_variable_source, mutable=True, default=9E14)
        self.b.p_build_limit_source_node_annual     = pyo.Param(self.b.s_kinds_source, initialize=data.build_limit_source_node, mutable=True, default=0.0)
        self.b.p_build_limit_source_node_cumulative = pyo.Param(self.b.s_kinds_source, self.b.s_nodes_source, initialize=data.build_limit_source_node_cumulative, mutable=True, default=0.0)
        self.b.p_build_limit_source_total_annual    = pyo.Param(self.b.s_kinds_source, initialize=data.build_limit_source_total, mutable=True, default=0.0)
        self.b.p_discount_invest_source             = pyo.Param(initialize=data.discount_invest_source, mutable=True, default=0.00)
    ...
    def declare_variables(self):
        ...
        self.b.va_excess_demand_eor : Pyomo Var
        ...
        self.b.va_excess_demand_eor = pyo.Var(self.b.s_nodes_demand, bounds=slack_demand_rule)
        ...
        ### Endogenous sources of CO2 (source)
        self.b.v_supply_source_exist = pyo.Var(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_policy_options, self.b.s_vintages_source, domain=NonNegativeReals, initialize=0, bounds=(0, self.b.p_M_supply))
        self.b.v_supply_source_new   = pyo.Var(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_policy_options, domain=NonNegativeReals, initialize=0, bounds=(0, self.b.p_M_supply))
        self.b.v_capacity_source_new = pyo.Var(self.b.s_kinds_source, self.b.s_nodes_source, domain=NonNegativeReals, initialize=0, bounds=(0, self.b.p_M_supply))
    ...
    def declare_constraints(self):
        ...
        ### Endogeneous sources of CO2 (source)

        # Source - existing unit supply by policy
        @self.b.Constraint(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_policy_options, self.b.s_vintages_source)
        def c_supply_source_exist_by_policy_vintage(m, k, n, p, v):
            return m.v_supply_source_exist[k, n, p, v] <= m.p_annual_capacity_exist_source[k, n, v] * m.p_policy_eligibility_source_exist[k, p, v]

        # Source - existing unit total supply
        @self.b.Constraint(self.b.s_kinds_source, self.b.s_nodes_source, self.b.s_vintages_source)
        def c_supply_source_exist(m, k, n, v):
            return sum(m.v_supply_source_exist[k, n, p, v] for p in self.b.s_policy_options) <= m.p_annual_capacity_exist_source[k, n, v]

        # Flow balance at source nodes
        @self.b.Constraint(self.b.s_nodes_source, self.b.s_policy_options)
        def c_flow_balance_supply_source(m, n, p):
            return (sum(m.v_flow_by_policy[a, p] for a in m.s_arcs_in[n]) - sum(m.v_flow_by_policy[a, p] for a in m.s_arcs_out[n])
                    == - (sum(m.v_supply_source_exist[k, n, p, v]  
                                for k in self.b.s_kinds_source
                                for v in self.b.s_vintages_source)
                         + sum(m.v_supply_source_new[k, n, p]
                                for k in self.b.s_kinds_source)
                         )
                    )

        # Limit annual source build-out at individual nodes
        @self.b.Constraint(self.b.s_kinds_source, self.b.s_nodes_source)
        def c_build_limit_source_node_rule(m, k, n):
            return m.v_capacity_source_new[k, n] <= m.p_build_limit_source_node_annual[k]

        # Limit annual source build-out nationwide
        @self.b.Constraint(self.b.s_kinds_source)
        def c_build_limit_source_total_rule(m,k):
            return (sum(m.v_capacity_source_new[k, n] 
                        for n in self.b.s_nodes_source) <= m.p_build_limit_source_total_annual[k])
    ...
    def declare_objective(self):
        ...
        # Summation of flow for arcs_out
        def sum_flow_arcs_out_rule(m, n):
            return sum(m.v_flow[a] for a in m.s_arcs_out[n])
        self.b.e_sum_flow_arcs_out = pyo.Expression(self.b.s_nodes, rule=sum_flow_arcs_out_rule)
        ...
        if self.parent.linear_model_switch: # Linear Program - does not use vb_transport_investment/p_capex_transport_base
            def sum_costs_investment_rule(m):
                return (
                        m.p_discount_invest_source      * sum(m.p_cost_invest_source[k, n] * m.v_capacity_source_new[k, n]
                                                            for k in m.s_kinds_source
                                                            for n in m.s_nodes_source)
            ...
            def sum_costs_investment_rule(m):
                return (
                        m.p_discount_invest_source * sum(m.p_cost_invest_source[k, n] * m.v_capacity_new_source[k, n]
                                                            for k in m.s_kinds_source
                                                            for n in m.s_nodes_source)
        ...
        def sum_costs_variable_rule(m):
            return (m.p_discount_variable
                    * (  sum(m.p_cost_variable_source[k, n] * (m.v_supply_source_exist[k, n, p, v] + m.v_supply_source_new[k, n, p])
                             for k in m.s_kinds_source
                             for n in m.s_nodes_source
                             for p in m.s_policy_options
                             for v in m.s_vintages_source)
        ...
        def sum_excess_rule(m):
            return (sum(m.va_excess_supply[n,p] * m.p_M_excess
                ...
                + sum((m.va_excess_ts_in[n,p] + m.va_excess_ts_out[n,p]) * m.p_M_excess
                      ...
                      for p in m.s_policy_options)
                + sum(m.va_excess_demand_eor[n]  * m.p_M_excess
                      for n in m.s_nodes_demand)
                    )
    ...
    def declare_constraints_across_blocks(self):
        ...
        c_storage_cumulative_nat_co2 : Pyomo Constraint
            Pyomo constraint asserting that natural co\ :sub:`2` volumes <=  maximum formation reserves.
        ...
        for t in self.m.s_time:
            ...
            # Track cumulative source supplies remaining
            def c_source_cumulative_resources(m, k, n, v):
                return (sum((m.blocks[t_star].v_supply_source_new[k,n,p] + m.blocks[t_star].v_supply_source_exist[k,n,p,v]) * m.blocks[t_star].p_duration
                            for t_star in self.m.s_time if t_star <= t for p in m.blocks[t_star].s_policy_options) <=
                        m.blocks[t].p_total_capacity_exist_source[k,n,v])
            rule_name = "c_source_cumulative_resources"
            index1 = "s_kinds_source"
            index2 = "s_nodes_source"
            index3 = 's_vintages_source'
            command = "self.m.c_" + rule_name + "_t%i = pyo.Constraint(self.m.blocks[%i]." + index1 + ", self.m.blocks[%i]." + index2 + ", self.m.blocks[%i]." + index3 + ", rule=" + rule_name + ")"
            exec(command % (t, t, t, t))
            ...
            # Source - new unit supply by policy
            def c_source_supply_new_by_policy(m, k, n, p):
                return ( m.blocks[t].v_supply_source_new[k, n, p] <= m.blocks[t].p_policy_eligibility_source_new[k, p] *
                        sum(m.blocks[t_star].v_capacity_source_new[k, n]
                                                                        for t_star in self.m.s_time 
                                                                        if t_star <= t - 1))
            rule_name = "c_source_supply_new_by_policy"
            index1 = "s_kinds_source"
            index2 = "s_nodes_source"
            index3 = "s_policy_options"
            command = "self.m.c_" + rule_name + "_t%i = pyo.Constraint(self.m.blocks[%i]." + index1 + ", self.m.blocks[%i]." + index2 + ", self.m.blocks[%i]." + index3 + ", rule=" + rule_name + ")"
            exec(command % (t, t, t, t))
            
            # Source - new unit supply
            def c_source_supply_new(m, k, n):
                return ( sum(m.blocks[t].v_supply_source_new[k, n, p] for p in m.blocks[t].s_policy_options) 
                        <= sum(m.blocks[t_star].v_capacity_source_new[k, n]
                                for t_star in self.m.s_time 
                                if t_star <= t - 1))
            rule_name = "c_source_supply_new"
            index1 = "s_kinds_source"
            index2 = "s_nodes_source"
            command = "self.m." + rule_name + "_t%i = pyo.Constraint(self.m.blocks[%i]." + index1 + ", self.m.blocks[%i]." + index2 + ", rule=" + rule_name + ")"
            exec(command % (t, t, t))

            # Source - cumulative new builds
            def c_source_supply_cumulative_new_builds(m, k, n):
                return (
                        sum(m.blocks[t].p_annual_capacity_exist_source[k,n,v]
                            for v in m.blocks[t].s_vintages_source
                            )
                         + sum(m.blocks[t_star].v_capacity_source_new[k, n]
                                for t_star in self.m.s_time 
                                if t_star <= t) 
                        <= m.blocks[t].p_build_limit_source_node_cumulative[k,n]
                        )
            rule_name = "c_source_supply_cumulative_new_builds"
            index1 = "s_kinds_source"
            index2 = "s_nodes_source"
            command = "self.m." + rule_name + "_t%i = pyo.Constraint(self.m.blocks[%i]." + index1 + ", self.m.blocks[%i]." + index2 + ", rule=" + rule_name + ")"
            exec(command % (t, t, t))
    ...
    def solve_optimization(self):
        ...
        opt = pyo.SolverFactory("xpress_persistent")  # current solver options: "amplxpress", "appsi_highs"
        opt.options['THREADS'] = self.threads
        opt.options['SOLTIMELIMIT'] = self.soltimelimit
        opt.options['MAXMEMORYSOFT'] = self.maxmemorysoft
        opt.options["OPTIMALITYTOL"] = self.optimalitytol
        opt.options["FEASTOL"] = self.feastol
        opt.set_instance(self.m)

        # Write Model to .mps and .lp
        if self.parent.debug_switch == True:
            file_name = self.output_path + 'model_file\\' + 'ccats_model_' + str(self.year_current) + '_c' + str(self.parent.cycle_current) 
            self.m.write(file_name + '.mps', io_options={"symbolic_solver_labels":True})
            self.m.write(file_name + '.lp', io_options={"symbolic_solver_labels":True})
            self.m.rc = pyo.Suffix(direction=pyo.Suffix.IMPORT)

        # Solve Model
        self.logger.info('Start solve.')
        try:
            if self.debug_opt_log == True:
                results = opt.solve(self.m, tee=True, load_solutions=False, keepfiles=True, logfile="ccats_main_opt_log.log")
                pyo.check_optimal_termination(results)
                solver_exception = None
            else:
                results = opt.solve(self.m, tee=False, load_solutions=False, keepfiles=True, logfile="ccats_main_opt_log.log")
                pyo.check_optimal_termination(results)
                solver_exception = None
        ...
        if com.check_results(results, TerminationCondition, SolutionStatus):
            ...
            com_bug.compute_infeasibility_explanation(self.m, solver=pyo.SolverFactory("xpress_direct"))
    ...
    def solve_linearized_optimization(self):
        ...
        self.logger.info('Declare new solver instance')
        opt_linear = pyo.SolverFactory("xpress_persistent")  # current solver options: "amplxpress", "appsi_highs"
        ...
        opt_linear.options["OPTIMALITYTOL"] = self.optimalitytol
        opt_linear.options["FEASTOL"] = self.feastol
        ...
        try:
            if self.debug_opt_log == True:
                results = opt_linear.solve(self.m, tee=True, load_solutions=False, keepfiles=True, logfile="ccats_main_opt_log.log")
                pyo.check_optimal_termination(results)
                solver_exception = None
            else:
                results = opt_linear.solve(self.m, tee=False, load_solutions=False, keepfiles=True, logfile="ccats_main_opt_log.log")
                pyo.check_optimal_termination(results)
                solver_exception = None
        except Exception as e:
            results = None
        ...
        if com.check_results(results, TerminationCondition, SolutionStatus):
            name = "noclass!"
            self.logger.info(f"[{name}] Solve failed")
            com_bug.compute_infeasibility_explanation(self.m_linear, solver=pyo.SolverFactory("xpress_direct"))
            if results is not None:
                self.logger.info("status=" + str(results.solver.status))
                self.logger.info("TerminationCondition=" + str(results.solver.termination_condition))
