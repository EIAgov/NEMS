# $Header: M:/default/scripts/RCS/runitnew.sh,v 1.8 2013/10/25 19:36:35 dsa Exp $
# Script to launch job from runit.f
# needs 6 arguments
#
datec=`date`
echo "Runitnew.cygwin.sh launched at $datec" > nohup.out
...
cd $outdir
echo "Job Dispatched by Runitnew.cygwin.sh" >> nohup.out
...
# lower case
  ...
  if [ "$cdstatus" = "0" ] ; then
    ...
    if [ $? = 0 ] ; then
      ...
      echo "error during copying, so will just use output directory" >> $outdir/nohup.out
    fi
  fi
else
  cd $outdir
...
if [ "$copied" = "1" ] ; then
  ...
  if [ -d $workdir2/p1 ] ; then
    ...
    cp "$cp_flg" -f $workdir2/p1/* $outdir/p1
    cpretp1=$?
    echo "cygwin cp p1 return code=$cpretp1" >> nohup.out
    if [ $cpretp1 -eq 0 ] ; then
      rm -rf $workdir/p1
    fi
  else 
    cpretp1=0
  ...
  if [ -d $workdir2/p2 ] ; then
    ...
    cp "$cp_flg" -f $workdir2/p2/* $outdir/p2
    cpretp2=$?
    echo "cygwin cp p2 return code=$cpretp2" >> nohup.out
    if [ $cpretp2 -eq 0 ] ; then
      rm -rf $workdir/p2
    fi
  else 
    cpretp2=0
  ...
  if [ -d $workdir2/p3 ] ; then
    ...
    cp "$cp_flg" -f $workdir2/p3/* $outdir/p3
    cpretp3=$?
    echo "cygwin cp p3 return code=$cpretp3" >> nohup.out
    if [ $cpretp3 -eq 0 ] ; then
      rm -rf $workdir/p3
    fi
  else 
    cpretp3=0
  fi
  echo "command:  cp $cp_flg2 $workdir2/* $outdir" >> nohup.out
  cp "$cp_flg2" $workdir2/* $outdir 
  cpret=$?
  echo "cygwin cp return code=$cpret" >> $outdir/nohup.out
  echo "cygwin cp return code=$cpret" >> $workdir2/nohup.out
