! $Header: m:/default/scripts/RCS/runit.f,v 1.51 2019/11/04 17:23:57 pkc Exp $
!****************************************************************
 PROGRAM runit
...
98     continue
   ...
   do i=1,numjob
     ...
     if(update) then
      if(J(i)%Status.eq.'Running'.or.J(i)%Status.eq.'dying'.or.J(i)%Status.eq.'Cleanup')then
        J(i)%Status='MIA'
       endif
       j(i)%command=' '
...
!======================================================================== 
 ...
 if(CurJ%Status.eq.'Running'.or.CurJ%Status.eq.'dying'.or.CurJ%Status.eq.'Cleanup' &
                           .or. CurJ%Command.eq.'kill') then ! last status update should be no longer than a few seconds ago
     call date_and_time(DATE=curdat,VALUES=vals)
     hr=vals(5)
     min=vals(6)
     sec=vals(7)
     ...
     delay= 14400  ! after 4 hr delay, figure job has gone missing in action
...
! and returns 1 if most recent file is over "seconds" seconds
 ...
 integer*8 handle       ! integer*4 for 32-bit; integer*8 for 64-bit, won't compile if *4
 integer*4 iret 
 integer*4 packtime
 integer freshness
 character*(*) filen
 ...
 if (lexist) then
   handle=file$first
   iret=getfileinfoqq(filen,filedata,handle)
