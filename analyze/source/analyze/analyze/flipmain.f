...
C LOCAL
      ...
      CHARACTER(len=128) :: first_arg
      CHARACTER(len=128) :: second_arg
      LOGICAL FIRST_ARG_BOOL
      LOGICAL SECOND_ARG_BOOL
      CHARACTER(LEN=128) :: FILE_NAME
      COMMON /SKIP_PROMPT/ SECOND_ARG_BOOL
      
      
      FIRST_ARG_BOOL = .FALSE.
      SECOND_ARG_BOOL = .FALSE.
      
C read in command line args
...
500   CONTINUE
      ...
      IF (FIRST_ARG_BOOL .EQ. .TRUE.  .AND.
     &    SECOND_ARG_BOOL .EQ. .TRUE.) THEN
           CLOSE(456)
           STOP
      END IF
...
501      FORMAT(1X,A8,'...')
         EXENUM = 0
         IF( LEN_TRIM(FIRST_ARG) .ne. 0 .AND. 
     &         FIRST_ARG_BOOL .EQ. .FALSE.) THEN
            FILE_NAME = "MATRIX_"//TRIM(FIRST_ARG)//".DAT"
            OPEN (UNIT = 456, FILE = FILE_NAME, STATUS = "UNKNOWN")
            CTEMP = 'READ P ' // TRIM(FIRST_ARG) 
            FIRST_ARG_BOOL = .TRUE.
            GOTO 510
         ELSE IF( LEN_TRIM(SECOND_ARG) .ne. 0 .AND. 
     &         SECOND_ARG_BOOL .EQ. .FALSE.) THEN
            CTEMP = 'LIST *,*' 
            SECOND_ARG_BOOL = .TRUE.
            GOTO 510
         END IF
