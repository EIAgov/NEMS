...
200   CONTINUE
        ...
        PRINT *,'   f> Formatted Matrix (MPS), such as from EKKBCDO'
...
C   Set Log frequency to every 100 iterations
      ...
      IF( FileType.EQ.'MATRIX' )THEN
         ...
         ELSE
            CALL EKKMPS(RTCOD,DSPACE,FILEIN,2,0)
         ENDIF
      ELSE
...
1090  CONTINUE
      ...
      IF( SWMSG )PRINT *,' Opening PACKED file ',PCKNAM(:LPCKN)
      CLOSE( FILEOUT )
